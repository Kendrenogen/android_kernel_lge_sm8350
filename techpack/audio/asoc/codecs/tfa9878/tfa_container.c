	...
	struct tfa_file_dsc *file, int vstep_idx, int vstep_msg_idx)
{
	...
	if ((type == msg_hdr)
		...
		if ((subversion > 0)
			...
			&& ((hdr->customer[2]) == 'I')
			&& ((hdr->customer[3]) == 'V'))) {
			pr_debug("%s: msg/volstep subversion %d, custom %d.%d.%d\n",
 				__func__, subversion,
 				hdr->customer[4],
 				hdr->customer[5],
 				hdr->customer[6]);
	...
	/* at initial device only: to write files only once */
	if (tfa->ext_dsp != 0) {
...
enum tfa98xx_error tfa_cont_write_files(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dsc_set_input_select
			...
			/* at initial device only: to write cmd only once */
			if (tfa->ext_dsp != 0) {
				if (tfa_nr_device_set(tfa) > 1
					|| tfa_nr_config_done(tfa) > 0)
					continue;
	...
	uint16_t bitfield)
{
	...
	pr_debug("%s: dev %d, prof %d, bitfield 0x%04x\n", __func__,
		tfa->dev_idx, tfa->profile, bitfield);
	...
	int prof_idx, int vstep_idx)
{
	...
	/* TODO?: put SetRe25C message to indicate all messages are sent */
	if (tfa->ext_dsp)
	...
	int length, const char *buffer)
{
	...
	if (length == -1) {
		...
		if (tfa->convert_dsp32) {
			if (total_len <= 0xffff) {
				blob[idx][2] = (uint8_t)((total_len >> 8) & 0xff);
		...
		for (i = total_len; i < (total_len + len_word_in_bytes); i++) {
			blob[idx][i] = 0;
		}
	...
	if (post_len > tfadsp_max_msg_size) {
		pr_debug("%s: set buffer full for blob (index %d): %d >= max %d, current length: %d\n",
			__func__, idx, post_len, tfadsp_max_msg_size, total[idx]);
		return TFA98XX_ERROR_BUFFER_TOO_SMALL;
	}

	/* add length field (length in words) to the multi message */
	if (tfa->convert_dsp32) {
		*blobptr[idx]++ = (uint8_t)((length / len_word_in_bytes) & 0xff);			/* lsb */
		*blobptr[idx]++ = (uint8_t)(((length / len_word_in_bytes) & 0xff00) >> 8);	/* msb */
		*blobptr[idx]++ = 0x0;
		*blobptr[idx]++ = 0x0;
	} else {
		*blobptr[idx]++ = 0x0;
	...
	if (idx == BLOB_INDEX_INDIVIDUAL) {
		pr_debug("%s: set last message for individual call: cmd=%d (index %d)\n",
			__func__, cmd, idx);
	...
	if ((cmd == FW_PAR_ID_GET_MEMORY && buf[1] == 0x80)
		...
		pr_debug("%s: found last message - sending: cmd=%d (index %d)\n",
			__func__, cmd, idx);
		return 1; /* 1 means last message is done! */
	}
	...
	if ((cmd == SB_PARAM_GET_ALGO_PARAMS && buf[1] == 0x81)
		...
		|| (cmd == SB_PARAM_GET_PARM_STEREO_BOOST && buf[1] == 0x81)
		...
		|| (cmd == SB_PARAM_GET_DISTO_DYNAMICS && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_SIBILANCE_LB_FILT && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_SIBILANCE_B1_FILT && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_SIBILANCE_B2_FILT && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_SIBILANCE_B3_FILT && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_SIBILANCE_GAIN_FILT && buf[1] == 0x81)
		|| (cmd == SB_PARAM_GET_STATE && buf[1] == 0x81)) {
		// || (cmd == SB_PARAM_GET_XMODEL && buf[1] == 0x81)
		// || (cmd == SB_PARAM_GET_TAG && buf[1] == 0x81)) {
		pr_debug("%s: found last message - sending: cmd=%d CC=%d (index %d)\n",
			__func__, cmd, cc, idx);
