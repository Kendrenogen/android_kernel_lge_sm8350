...
tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	err = tfa_dev_config_pcm_format(tfa98xx->tfa,
		tfa98xx_device_count, tfa98xx->sample_size, tfa98xx->slot_size);
#endif
...
static int tfa98xx_run_calibration(struct tfa98xx *tfa98xx)
{
	...
	if (!tfa98xx->calibrate_done) {
		...
		if (value > 0) {
			...
			if (tfa98xx->tfa->reset_mtpex) {
				pr_debug("[0x%x] resetting MTPEX is pending - need calibration\n",
					tfa98xx->i2c->addr);
				tfa98xx->calibrate_done = 0;
			}
		} else {
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		// MTPEX <reset to force to calibrate>
		ret = tfa_dev_mtp_set(tfa98xx->tfa, TFA_MTP_RE25, 0);
		// EXT_TEMP
	...
	if (cal_err) {
		pr_info("[0x%x] Calibration start failed!\n",
			tfa98xx->i2c->addr);
		return -EIO;
	} else {
			...
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				...
				if (failed) {
					...
					if (tfa98xx->tfa->spkr_damaged) {
						...
						int cur_tdmdspkg
							= TFA7x_GET_BF(tfa98xx->tfa, TDMSPKG);

						if (cur_tdmdspkg < TFA_REDUCED_TDMSPKG_IN_BYPASS) {
							dev_info(&tfa98xx->i2c->dev,
								"reduce TDMSPKG (%d to %d) in bypass!\n",
								cur_tdmdspkg,
								TFA_REDUCED_TDMSPKG_IN_BYPASS);

							/* set TDMSPKG as 7 dB for safety */
							TFA7x_SET_BF(tfa98xx->tfa,
								TDMSPKG, TFA_REDUCED_TDMSPKG_IN_BYPASS);
...
#if defined(TFA_BLACKBOX_LOGGING)
		if (tfa98xx->tfa->dev_idx == 0) /* master device only */
			if (tfa98xx->pstream
				&& tfa98xx->tfa->blackbox_enable) {
				/* get logging once before shutting down stream */
...
#define TFA_MSP_CAL_CHECK_STORE

#ifdef TFA_MSP_CAL_CHECK_STORE
#define TFA_CALI_MSP_FILE  "/mnt/vendor/persist-lg/msp/audio.dat"

static int msp_mtp[2] = {0};
static void tfa98xx_check_msp_cali_store(struct tfa98xx *tfa98xx, int mtp)
{
	char buf[50] = {0};
	struct file *fp = NULL;
	loff_t pos = 0;

	if (tfa98xx->calibrate_done && tfa98xx->i2c->addr == 0x34) {
		if (msp_mtp[0] != mtp)
			msp_mtp[0] = mtp;
	}
	else if (tfa98xx->calibrate_done && tfa98xx->i2c->addr == 0x35) {
		if (msp_mtp[1] != mtp) {
			msp_mtp[1] = mtp;
			//write file
			fp = filp_open(TFA_CALI_MSP_FILE, O_RDWR | O_CREAT, 0666);
			if (IS_ERR(fp)) {
				pr_err("%s:audio.dat open %s failed!\n", __func__, TFA_CALI_MSP_FILE);
				return;
			}
			snprintf(buf, PAGE_SIZE, "top_spk,%d,bottom_spk,%d", msp_mtp[0], msp_mtp[1]);
			__kernel_write(fp, buf, strlen(buf), &pos);
			pr_info("%s: store buf:%s\n", __func__, buf);
			filp_close(fp, NULL);
		}
	}
}
...
#ifdef TFA_MSP_CAL_CHECK_STORE
	tfa98xx_check_msp_cali_store(tfa98xx, mtp);
#endif
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((tfa0->stream_state & BIT_PSTREAM) != 0)
		err = tfa_update_log();
	...
	for (idx = 0; idx < ndev; idx++) {
		...
		if (ntfa == NULL)
			continue;
		addr = ntfa->slave_address;
		count = snprintf(buf, PAGE_SIZE,
			"%s[0x%02x] maxX %d um, maxT %d degC, countXmax %d, countTmax %d\n",
			buf, addr,
			tfa0->log_data[offset], tfa0->log_data[offset + 1],
			tfa0->log_data[offset + 2], tfa0->log_data[offset + 3]);
	}
#else
	...
	const struct i2c_device_id *id)
{
	...
	if (np) {
		...
		if (ret) {
			...
			// return ret; /* ser default range instead */
		}
	}
#endif /* LIMIT_CAL_FROM_DTS */
