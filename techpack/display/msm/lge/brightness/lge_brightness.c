...
int lge_update_backlight(struct dsi_panel *panel)
{
	...
	if (!rc) {
		pr_info("[%s] <--%pS unset=%d\n", panel->name, __builtin_return_address(0), panel->lge.bl_lvl_unset);
	}
...
int lge_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if ((panel->lge.lp_state == LGE_PANEL_LP2) || (panel->lge.lp_state == LGE_PANEL_LP1)) {
...
#if defined(CONFIG_LGE_DUAL_SCREEN)
		if (is_ds_connected())
			bl_type = LGE_BLMAP_HDR_DS_MODE_ENABLE;
#endif
		...
		bl_type = LGE_BLMAP_DS_MODE_ENABLE;
#endif
	} else {
	...
	if (panel->lge.allow_bl_update) { //TODO : Discuss condition state.
		...
		if (c_conn->ops.set_backlight) {
			...
			pr_info("[%s] BR:%d BL:%d %s\n", panel->name, brightness, bl_lvl, lge_get_blmapname(bl_type));
		}
...
static int lge_mipi_dsi_dcs_set_display_brightness_extension(struct dsi_panel *panel, u32 bl_lvl)
{
	char *bc_payload = NULL;
	int rc = 0;
	u32 count = 0;
	...
	bc_payload[3] = (bl_lvl & 0xff00) >> 8;
	bc_payload[4] = bl_lvl & 0x00ff;
	...
	u32 bl_lvl)
{
	...
	if (panel->lge.use_br_ctrl_ext &&
		...
		if (panel->lge.use_dcs_brightness_short) {
			...
			rc = lge_mipi_dsi_dcs_set_display_brightness_extension(panel, bl_lvl);
		}
	} else {
		if (panel->lge.use_dcs_brightness_short) {
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!dsi_panel_initialized(panel)) {
		pr_err("panel not yet initialized..\n");
		if (panel->lge.use_exscreen_control && panel->lge.ddic_ops->set_hl_mode_pending) {
			panel->lge.ddic_ops->set_hl_mode_pending(panel, input);
		}
		mutex_unlock(&panel->panel_lock);
		return -EINVAL;
	}
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->set_hl_mode) {
		if (panel->lge.use_exscreen_control &&
				panel->lge.ddic_ops->get_hl_mode_pending &&
				(panel->lge.ddic_ops->get_hl_mode_pending(panel) > 0)) {
			panel->lge.ddic_ops->set_hl_mode_pending(panel, -1);
		}
		panel->lge.ddic_ops->set_hl_mode(panel, input);
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[%s] request=%d\n", panel->name, panel->lge.br_offset);
...
static struct attribute *hl_mode_attrs[] = {
...
static const struct attribute_group hl_mode_attr_group = {
	.name	= "brightness",
	.attrs	= hl_mode_attrs,
};
...
int lge_brightness_create_sysfs(struct dsi_panel *panel, struct device *panel_sysfs_dev)
{
	...
	if (panel_sysfs_dev) {
		...
		if (panel->lge.use_hl_mode) {
			if ((rc = sysfs_merge_group(&panel_sysfs_dev->kobj, &hl_mode_attr_group)) < 0)
				pr_err("merge hl_mode group fail!\n");
		}
	}
	return rc;
}
