...
static int lge_dsi_panel_mode_set(struct dsi_panel *panel)
{
	...
	if ((panel->lge.use_fps_switch) && (panel->lge.ddic_ops) &&
			(panel->lge.ddic_ops->set_fps_cmds)) {
		panel->lge.ddic_ops->set_fps_cmds(panel, panel->lge.last_frame_rate, true);
	}
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (lp_mode == SDE_MODE_DPMS_ON && last_panel_power_mode == SDE_MODE_DPMS_OFF) {
		...
		if (panel->lge.notify_pending) {
			panel->lge.notify_pending = false;
			goto notify;
		}
		goto mode_set;
	} else if (lp_mode == SDE_MODE_DPMS_OFF) {
		if (panel->lge.use_hl_mode && panel->lge.ddic_ops->init_hl_mode) {
			panel->lge.ddic_ops->init_hl_mode(panel);
		}
...
static int dsi_panel_reset_off(struct dsi_panel *panel)
{
	...
	if (dsi_panel_full_power_seq(panel)) {
		if (panel->lp11_init && panel->lge.reset_keep_high_after_off_cmd)
			mdelay(panel->lge.reset_keep_high_after_off_cmd);
...
static void mark_notifier_pending_field(struct dsi_panel *panel)
{
	if (!panel)
		return;

	if (panel->lge.lp_state == LGE_PANEL_OFF) {
		pr_info("set notify pending\n");
		panel->lge.notify_pending = true;
	} else if (panel->lge.notify_pending &&
			(panel->lge.lp_state == LGE_PANEL_NOLP)) {
		pr_info("disable notify pending\n");
		panel->lge.notify_pending = false;
	}
}
...
error:
	...
	mark_notifier_pending_field(panel);
...
void lge_mdss_report_panel_dead()
{
	...
	if (display_sec && display_sec->panel && conn_sec) {
		...
		mutex_unlock(&conn_sec->lock);
		lge_mdss_panel_dead_notify(display_sec);
	}
}
...
static int lge_dsi_panel_parse_dt(struct dsi_panel *panel, struct device_node *of_node)
{
	...
	rc = of_property_read_u32(of_node, "lge,mdss-dsi-reset-keep-high-after-off-cmd", &tmp);
	if (rc && (tmp > 0)) {
		panel->lge.reset_keep_high_after_off_cmd = 0;
	} else {
		panel->lge.reset_keep_high_after_off_cmd = tmp;
		pr_info("reset_keep_high_after_offcmd=%d\n",
				panel->lge.reset_keep_high_after_off_cmd);
	}
	...
	panel->lge.use_fps_switch =
		of_property_read_bool(of_node, "lge,fps-switch-enabled");
	...
	panel->lge.use_exscreen_control = of_property_read_bool(of_node, "lge,use-expandscreen-control");
	pr_info("use exscreen control=%d\n", panel->lge.use_exscreen_control);
	...
	panel->lge.send_pu_to_hs
		= of_property_read_bool(of_node, "lge,send-partial-update-cmds-to-hs");
	pr_info("send pu to HS=%d\n", panel->lge.send_pu_to_hs);
