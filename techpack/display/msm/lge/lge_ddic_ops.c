...
extern struct lge_ddic_ops nt37800a_ops;
extern struct lge_ddic_ops nt37800a_rdk_ops;
extern struct lge_ddic_ops nt37701_ops;
...
static struct lge_ddic_match supported_ddic_list[] = {
	{"sw43103", &sw43103_ops},
	{"nt37800a", &nt37800a_ops},
	{"nt37800a_rdk", &nt37800a_rdk_ops},
	{"nt37701", &nt37701_ops},
};

extern char* get_ddic_name(void);
extern bool is_ddic_name(char *ddic_name);
extern bool is_ddic_name_matched(struct dsi_panel *panel, char *ddic_name);
...
extern int lge_cmdline_bootup_resolution;
...
void lge_ddic_ops_init(struct dsi_panel *panel)
{
	...
	for (i = 0; i < count; ++i) {
		if (is_ddic_name_matched(panel, supported_ddic_list[i].compatible)) {
...
void lge_ddic_feature_init(struct dsi_panel *panel)
{
	...
	if (panel->lge.use_hl_mode) {
		panel->lge.hl_mode = 0;
		if (panel->lge.ddic_ops && panel->lge.ddic_ops->init_hl_mode)
			panel->lge.ddic_ops->init_hl_mode(panel);
	}

	if (panel->lge.use_exscreen_control) {
		panel->lge.bootup_resolution = lge_cmdline_bootup_resolution;
	}

	if (panel->lge.use_fps_switch) {
		panel->lge.last_frame_rate = LGE_FPS_UNSET;
	}
}
...
char *lge_ddic_cmd_set_prop_map[LGE_DDIC_DSI_CMD_SET_MAX] = {
	...
	"lge,mdss-dsi-expandable-area-command",
	"lge,mdss-dsi-expandable-area-enable-command",
...
char *lge_ddic_cmd_set_state_map[LGE_DDIC_DSI_CMD_SET_MAX] = {
	...
	"lge,mdss-dsi-expandable-area-command-state",
	"lge,mdss-dsi-expandable-area-enable-command-state",
	"lge,mdss-dsi-switch-fps-ctrl-command-state",
};
	...
	struct device_node *of_node)
{
	...
	u32 i;
	u8 supp_list[LGE_DDIC_DSI_CMD_SET_MAX * 4] = {0x0,};
	u8 not_supp_list[LGE_DDIC_DSI_CMD_SET_MAX * 4] = {0x0,};
	ssize_t supp_len = 0, not_supp_len = 0;
	...
	for(i = 0; i < LGE_DDIC_DSI_CMD_SET_MAX; i++) {
		...
		if (rc) {
			not_supp_len += scnprintf(not_supp_list + not_supp_len, 4, "%02d,", i);
		} else {
			supp_len += scnprintf(supp_list + supp_len, 4, "%02d,", i);
		}
	}
	pr_info("support list : %s\n", supp_list);
	pr_info("unsupport list : %s\n", not_supp_list);
...
char* get_payload_addr(struct dsi_panel *panel, enum lge_ddic_dsi_cmd_set_type type, int position)
{
	...
	if ((cmd_set->count == 0) || (position >= cmd_set->count)) {
