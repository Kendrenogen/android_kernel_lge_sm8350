...
enum lge_ddic_dsi_cmd_set_type {
	...
	LGE_DDIC_DSI_EXPANDABLE_AREA,
	LGE_DDIC_DSI_EXPANDABLE_AREA_ENABLE,
	LGE_DDIC_DSI_SWITCH_FPS_CTRL,
...
struct lge_ddic_ops {
	...
	int (*get_hl_mode)(struct dsi_panel *panel);
	void (*set_hl_mode)(struct dsi_panel *panel, int mode);
	void (*set_hl_mode_pending)(struct dsi_panel *panel, int mode);
	int (*get_hl_mode_pending)(struct dsi_panel *panel);
	void (*init_hl_mode)(struct dsi_panel *panel);
	void (*set_exscreen_width)(struct dsi_panel *panel, int value, bool is_target);
	...
	/* For Display FPS */
	void (*set_fps_cmds)(struct dsi_panel *panel, u32 fps_input, bool resync);
...
struct lge_dsi_panel {
	...
	int reset_keep_high_after_off_cmd;
	bool is_incell;
...
//Display Bringup need to check is it need
	...
	bool send_pu_to_hs;
	void *expandable_handle;
	bool expandable_pending;
	int expandable_pending_mode;
	int expandable_mode;
	bool use_exscreen_control;
	int exscreen_req_width;
	int exscreen_cur_width;
	int bootup_resolution;
