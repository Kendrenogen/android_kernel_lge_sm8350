...
#define E2P_SLAVE_ADDR_8bit       0xA2	//SHMOON_201013 : IMX682/787 (1010001b)
	...
	struct cam_sensor_power_ctrl_t *power_info)
{
	...
	//power_info->power_setting_size = 1;	//SHMOON_201006 : power up RESET only
	...
	if (!power_info->power_setting)
		return -ENOMEM;
#if 1
	power_info->power_setting[0].seq_type = SENSOR_VIO_LDAF;
	power_info->power_setting[0].seq_val = CAM_VIO_LDAF;
	power_info->power_setting[0].config_val = 1;
	power_info->power_setting[0].delay = 0;

	power_info->power_setting[1].seq_type = SENSOR_VDD_LDAF;
	power_info->power_setting[1].seq_val = CAM_VDD_LDAF;
...
#else	//SHMOON_201006 : power up RESET only
	power_info->power_setting[0].seq_type = SENSOR_RESET;
	power_info->power_setting[0].config_val = 1;
	power_info->power_setting[0].delay = 0;
#endif

	power_info->power_down_setting_size = 3;
	//power_info->power_down_setting_size = 1;	//SHMOON_201006 : power up RESET only
...
#if 1	
	power_info->power_down_setting[0].seq_type = SENSOR_VIO_LDAF;
	power_info->power_down_setting[0].seq_val = CAM_VIO_LDAF;	
	power_info->power_down_setting[0].config_val = 0;
	power_info->power_down_setting[0].delay = 0;

	power_info->power_down_setting[1].seq_type = SENSOR_VDD_LDAF;
	power_info->power_down_setting[1].seq_val = CAM_VDD_LDAF;
...
*/
	...
	if(vl53l0_data == NULL){
		...
		//SHMOON_201111
		memset(vl53l0_data, 0, sizeof(struct stmvl53l0_data));
...
static int32_t stmvl53l0_platform_remove(struct platform_device *pdev)
{
	...
	stmvl53l0_cleanup(p_ctrl->vl53l0_data);	//SHMOON_201103
...
int32_t ldaf_i2c_e2p_read(uint32_t RegAddr, uint8_t *RegData, enum camera_sensor_i2c_type data_type)
{
	...
	uint32_t data = 0;
	uint16_t temp_sid = 0;
	struct v4l2_subdev *temp_cci_subdev = NULL;
	...
	temp_cci_subdev = local_cam_ldaf_t->io_master_info.cci_client->cci_subdev;
	local_cam_ldaf_t->io_master_info.cci_client->cci_subdev = cam_cci_get_subdev(0);
	...
	local_cam_ldaf_t->io_master_info.cci_client->cci_subdev = temp_cci_subdev;
...
int ldaf_verify_e2p_chksum(uint16_t slv_addr, uint32_t st_addr, uint32_t e2p_size, uint32_t chk_addr)
{
	...
	rc = ldaf_e2p_cnt_rd(slv_addr, st_addr, e2p_data, e2p_size);
	if (rc < 0) {
		vl53l0_dbgmsg("Verify e2p i2c failed");
		goto END;
	}

	for (i = 0; i < e2p_size; i++) partial_sum += e2p_data[i];

	rc = ldaf_e2p_cnt_rd(slv_addr, chk_addr, chksum, 4);
	if (rc < 0) {
		vl53l0_dbgmsg("Verify e2p i2c failed");
		goto END;
	}
	...
	if (partial_sum != (chksum[0] | chksum[1] << 8 | chksum[2] << 16 | chksum[3] << 24)) {
		vl53l0_dbgmsg("Verify e2p checksum failed partial sum: 0x%x checksum: 0x%x",
