...
#define FORCE_CRASH_ADDR_ADDR     0x6C
...
#define FORCE_CRASH_ADDR    (msm_imem_base + FORCE_CRASH_ADDR_ADDR)
static void *msm_imem_base;
static int dummy_arg;
static int subsys_crash_magic;

#define LAHAINA_PINCTRL_OFFSET          0x134000
#define LAHAINA_PINCTRL_ADDR       (msm_pinctrl_base + LAHAINA_PINCTRL_OFFSET)
static void __iomem *msm_pinctrl_base;
...
void lge_set_subsys_crash_reason(const char *name, int type)
{
	const char *subsys_name[] =
		{ "adsp", "modem", "wcnss", "slpi", "venus", "cdsp", "wlan"};
	int i = 0;
...
void lge_set_force_crash(unsigned int magic)
{
	writel_relaxed(magic, FORCE_CRASH_ADDR);
}
...
void lge_gen_key_panic(int key, int status)
{
	...
	if (key_crash_cnt >= 7) {
		...
		gen_key_panic = 1;
		lge_set_force_crash(CRASH_HANDLER_MAGIC_NUM);
...
extern void qcom_wdt_trigger_bark(void);
...
static int gen_noc_panic(const char *val, const struct kernel_param *kp)
{
	struct device_node *np;

	np = of_find_compatible_node(NULL, NULL, "qcom,lahaina-pinctrl");
	if (!np) {
		pr_err("unable to find DT pinctrl node\n");
		return -ENODEV;
	}

	if (of_get_address(np, 0, NULL, NULL)) {
		msm_pinctrl_base = of_iomap(np, 0);
		if (!msm_pinctrl_base) {
			pr_err("unable to map pinctrl\n");
			return -ENOMEM;
		}
	}

	writel_relaxed(1, LAHAINA_PINCTRL_ADDR);
	readl_relaxed(LAHAINA_PINCTRL_ADDR);
	mb();
	mdelay(5000);
...
module_param_call(gen_noc_panic, gen_noc_panic, param_get_bool,
		&dummy_arg, S_IWUSR | S_IRUGO | S_IWGRP);
