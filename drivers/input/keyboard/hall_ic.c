...
void lge_slide_status_notify(struct hallic_dev *hdev, int state)
{
	if (!strncmp(hdev->name, "slidedetect", strlen(hdev->name))) {
		if (hdev->state == 0) {
			pr_info("[Display][%s] LGE_SLIDE_OPEN(%d)\n", __func__, hdev->state);
			lge_panel_notifier_call_chain(LGE_SLIDE_DETECT, 0, LGE_SLIDE_OPEN);
		} else {
			pr_info("[Display][%s] LGE_SLIDE_CLOSED(%d)\n", __func__, hdev->state);
			lge_panel_notifier_call_chain(LGE_SLIDE_DETECT, 0, LGE_SLIDE_CLOSED);
		}
	}
}
EXPORT_SYMBOL(lge_slide_status_notify);
