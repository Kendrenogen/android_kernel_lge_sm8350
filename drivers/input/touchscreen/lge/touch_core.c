...
void touch_report_cancel_event(struct touch_core_data *ts)
{
	...
	for (i = 0; i < MAX_FINGER; i++) {
		if (old_mask & (1 << i)) {
			...
			hidden_coordinate(ts, i, CANCELED);
#endif
		}
	}
...
#if IS_ENABLED(CONFIG_MTK_PERFMGR_TOUCH_BOOST)
	if (new_mask)
		input_report_key(ts->input, BTN_TOUCH, 1);
	else
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_HIDDEN)
					hidden_coordinate(ts, i, PRESSED);
#endif
				}
#if IS_ENABLED(CONFIG_LGE_TOUCH_COMMON_CONTROL)
				if (ts->systrace_mode == SYS_PR)
					touch_trace_begin("%s", "main_touch");
#endif
			}
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_HIDDEN)
				hidden_coordinate(ts, i, RELEASED);
#endif
			}
#if IS_ENABLED(CONFIG_LGE_TOUCH_COMMON_CONTROL)
			if (ts->systrace_mode == SYS_PR)
				touch_trace_end("%s", "main_touch");
#endif
		}
	}
...
void touch_report_cancel_event_display_area(struct touch_core_data *ts, int display_area_id)
{
	int i = 0;
	int tcount_diff = 0;
	...
	for (i = 0; i < MAX_FINGER; i++) {
		if (ts->old_mask & (1 << i)) {
			// Calculate whether the coordinates are inside the area
			if (ts->tdata[i].x >= ts->flexible_display.area[display_area_id].area.left_top.x &&
					ts->tdata[i].x <= ts->flexible_display.area[display_area_id].area.right_bottom.x &&
					ts->tdata[i].y >= ts->flexible_display.area[display_area_id].area.left_top.y &&
					ts->tdata[i].y <= ts->flexible_display.area[display_area_id].area.right_bottom.y) {
				// Bitmask clear for release events
				ts->new_mask &= ~(0x1 << i);
				tcount_diff++;
	...
	// Send release events
	touch_report_event(ts);
	ts->tcount -= tcount_diff;
}
EXPORT_SYMBOL(touch_report_cancel_event_display_area);
...
irqreturn_t touch_irq_thread(int irq, void *dev_id)
{
	...
	if (ret >= 0) {
		...
		if (ts->intr_status & TOUCH_IRQ_LG_PAY_SWIPE_UP)
			touch_send_uevent(ts, TOUCH_UEVENT_LG_PAY_SWIPE_UP);

		if (ts->intr_status & TOUCH_IRQ_LG_PAY_SWIPE_LEFT)
			touch_send_uevent(ts, TOUCH_UEVENT_LG_PAY_SWIPE_LEFT);

		if (ts->intr_status & TOUCH_IRQ_LG_PAY_SWIPE_RIGHT)
			touch_send_uevent(ts, TOUCH_UEVENT_LG_PAY_SWIPE_RIGHT);

		/* Quick Tool Swipe */
		if (ts->intr_status & TOUCH_IRQ_QUICK_TOOL_SWIPE_LEFT)
			touch_send_uevent(ts, TOUCH_UEVENT_QUICK_TOOL_SWIPE_LEFT);

		if (ts->intr_status & TOUCH_IRQ_QUICK_TOOL_SWIPE_RIGHT)
			touch_send_uevent(ts, TOUCH_UEVENT_QUICK_TOOL_SWIPE_RIGHT);

		/* Fake Off Swipe */
		if (ts->intr_status & TOUCH_IRQ_FAKE_OFF_SWIPE)
			touch_send_uevent(ts, TOUCH_UEVENT_SWIPE);
		...
		if (ts->intr_status & TOUCH_IRQ_LONGPRESS_DOWN)
			touch_send_uevent(ts, TOUCH_UEVENT_LONGPRESS_DOWN);

		if (ts->intr_status & TOUCH_IRQ_LONGPRESS_UP)
			touch_send_uevent(ts, TOUCH_UEVENT_LONGPRESS_UP);

		if (ts->intr_status & TOUCH_IRQ_SINGLE_WAKEUP)
			touch_send_uevent(ts, TOUCH_UEVENT_SINGLE_WAKEUP);

		if (ts->intr_status & TOUCH_IRQ_FAKE_OFF_SINGLE_WAKEUP)
			touch_send_uevent(ts, TOUCH_UEVENT_FAKE_OFF_SINGLE_WAKEUP);
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_HIDDEN)
static void touch_hidden_work_func(struct work_struct *hidden_work)
{
	struct touch_core_data *ts =
		container_of(to_delayed_work(hidden_work),
				struct touch_core_data, hidden_work);
...
static int touch_check_driver_function(struct touch_core_data *ts)
{
	if (ts->driver->probe &&
			...
			(ts->driver->lpwg || ts->driver->display_area_status) &&
			ts->driver->lpwg_function_info &&
...
char *uevent_str[TOUCH_UEVENT_SIZE][3] = {
	{"TOUCH_GESTURE_WAKEUP=WAKEUP", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=PASSWORD", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWIPE_DOWN", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWIPE_UP", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWIPE_LEFT", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWIPE_RIGHT", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=WATER_MODE_ON", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=WATER_MODE_OFF", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=AI_BUTTON", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=AI_PICK", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=LONG_DOWN", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=LONG_UP", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SINGLE_WAKEUP", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=PEN_WAKEUP", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=PEN_WAKEUP_BTN", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=PEN_DETECTION", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWITCH_AES_BOTH", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWITCH_AES_TO_1", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWITCH_AES_TO_2", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=DS_UPDATE_STATE", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=SWIPE", NULL, NULL},
	{"TOUCH_GESTURE_WAKEUP=FAKE_OFF_SINGLE_WAKEUP", NULL, NULL},
};
...
void touch_send_uevent(struct touch_core_data *ts, int type)
{
	...
	if (type == TOUCH_UEVENT_WATER_MODE_ON || type == TOUCH_UEVENT_WATER_MODE_OFF ||
			type == TOUCH_UEVENT_LONGPRESS_UP || type == TOUCH_UEVENT_DS_UPDATE_STATE) {
		kobject_uevent_env(&device_uevent_touch.kobj, KOBJ_CHANGE, uevent_str[type]);
		TOUCH_I("%s\n",  uevent_str[type][0]);
	} else {
		...
		if (atomic_read(&ts->state.uevent) == UEVENT_IDLE ||
			...
			if (ts->lpwg_coord.count != 0) {
				buf = vzalloc(UEVENT_MSG_LEN);

				ret += touch_snprintf(buf + ret, UEVENT_MSG_LEN - ret, "TOUCH_GESTURE_COORDINATE=");
				for (i = 0; i < ts->lpwg_coord.count; i++) {
					ret += touch_snprintf(buf + ret, UEVENT_MSG_LEN - ret, "%d %d",
							ts->lpwg_coord.code[i].x, ts->lpwg_coord.code[i].y);
					if (i != ts->lpwg_coord.count - 1) {
						ret += touch_snprintf(buf + ret, UEVENT_MSG_LEN - ret, " ");
					}
				}
				...
				TOUCH_I("%s: generated data [%s]\n", __func__, buf);
				uevent_str[type][1] = buf;
...
EXPORT_SYMBOL(touch_send_uevent);
...
void touch_notify_temperature(int type)
{
	temperature = type;
	touch_atomic_notifier_call(NOTIFY_TEMPERATURE, &type);
}
EXPORT_SYMBOL(touch_notify_temperature);
void touch_restore_state(struct touch_core_data *ts) {
	...
	if (atomic_read(&ts->state.temperature) != temperature) {
		TOUCH_I("%s: need to restore temperature (%d)\n", __func__, temperature);
		touch_notify_temperature(temperature);
	}
}
				   ...
				   unsigned long event, void *data)
{
	...
	if (ts->driver->notify) {
		...
		case NOTIFY_TEMPERATURE:
			atomic_set(&ts->state.temperature, *(int *)data);
			ret = ts->driver->notify(ts->dev, event, data);
			break;
	...
	if (ts->driver->notify) {
		...
		case LGE_PANEL_EVENT_RECOVERY:
			...
			if (panel_data->state == LGE_PANEL_RECOVERY_DEAD) {
				...
				//touch_interrupt_control(ts->dev, INTERRUPT_DISABLE); //temp bringup
			}
			break;
		case LGE_PANEL_EVENT_FPS:
			TOUCH_D(TRACE, "EVENT_FPS : %d %d", panel_data->display_id,
					panel_data->state);
			if (panel_data->state == 30)
				display_notify_data = 0;
			else if (panel_data->state == 60)
				display_notify_data = 1;
			else if (panel_data->state == 120)
				display_notify_data = 2;
			ret = ts->driver->notify(ts->dev, LCD_EVENT_CHANGE_FPS,
					&display_notify_data);
			break;
		default:
				   ...
				   unsigned long event, void *data)
{
	...
	case NOTIFY_FM_RADIO:
		...
		break;
	case NOTIFY_TEMPERATURE:
		arr[ATOMIC_NOTIFY_TEMPERATURE].event = event;
		arr[ATOMIC_NOTIFY_TEMPERATURE].data = *(int *)data;
		break;
	default:
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_HIDDEN)
	INIT_DELAYED_WORK(&ts->hidden_work, touch_hidden_work_func);
#endif
...
#if IS_ENABLED(CONFIG_LGE_DD_TEMP_BRINGUP)
...
//		char buf[100] = {0};
	...
	for (i = 0; i < DISPLAY_AREA_ID_MAX; i++) {
		if (i == 0) {
			ts->display_area_status[i].lpwg.screen = SCREEN_ON;
		} else {
			ts->display_area_status[i].lpwg.screen = SCREEN_OFF;
		}
		ts->display_area_status[i].lpwg.mode = LPWG_NONE;
		ts->display_area_status[i].lpwg.sensor = PROX_FAR;
		ts->display_area_status[i].lpwg.qcover = HALL_FAR;
	}
...
#endif
#if IS_ENABLED(CONFIG_LGE_TOUCH_COMMON_CONTROL)
	cmds_adder(ts->dev, "core", core_cmd_lists);
#endif
	...
	atomic_set(&ts->state.temperature, 0xFFFF); //default set.

	ret = touch_init_uevent(ts);
	ret = touch_init_sysfs(ts);

#if IS_ENABLED(CONFIG_LGE_TOUCH_HIDDEN)
	queue_delayed_work(ts->wq, &ts->hidden_work, 0);
#endif
