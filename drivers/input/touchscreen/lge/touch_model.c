...
int touch_get_dts_base(struct touch_core_data *ts)
{
	...
	PROPERTY_U32(np, "flexible_display_info_update_frequency", ts->caps.flexible_display_info_update_frequency);
	...
	PROPERTY_BOOL(np, "hidden_coordi_enable", ts->role.hidden_coordi);
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_MODULE_DETECT)
