...
#include "touch_sw82908_abt.h"
#endif

static int dlkm_sw82908_lpwg_mode(struct device *dev);
static void dlkm_sw82908_lcd_mode(struct device *dev, u32 mode);
static int dlkm_sw82908_power(struct device *dev, int ctrl);
static int dlkm_sw82908_init(struct device *dev);
static int dlkm_sw82908_vsync_set(struct device *dev, int force_mode);
...
static void project_param_set(struct device *dev)
{
	...
	param->used_mode = (1<<LCD_MODE_U0)|(1<<LCD_MODE_U3)|(1<<LCD_MODE_U2_UNBLANK)|
		    (1<<LCD_MODE_U2)|(1<<LCD_MODE_U3_QUICKCOVER)|(1<<LCD_MODE_STOP);
}
...
int dlkm_sw82908_reg_read(struct device *dev, u16 addr, void *data, int size)
{
...
int dlkm_sw82908_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct touch_bus_msg msg = {0, };
...
static int dlkm_sw82908_drm_notifier_callback(struct notifier_block *self,
...
static int dlkm_sw82908_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
static int dlkm_sw82908_sw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0;
	...
	if (mode == SW_RESET) {
		...
		dlkm_sw82908_write_value(dev, SYS_RST_CTL, 2);
		touch_msleep(20);
		dlkm_sw82908_write_value(dev, SYS_RST_CTL, 0);
	} else if (mode == SW_RESET_CODE_DUMP) {
		/* [system reset] Touch F/W resister reset and do code flash dump */
		dlkm_sw82908_write_value(dev, SYS_RST_CTL, 1);
		touch_msleep(20);
		dlkm_sw82908_write_value(dev, SYS_RST_CTL, 0);
	} else {
...
int dlkm_sw82908_hw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
...
int dlkm_sw82908_only_hw_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
int dlkm_sw82908_power_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	dlkm_sw82908_power(ts->dev, POWER_OFF);
	dlkm_sw82908_power(ts->dev, POWER_ON);
...
int dlkm_sw82908_reset_ctrl(struct device *dev, int ctrl)
{
	int ret = 0;
	...
	case SW_RESET_CODE_DUMP:
		ret = dlkm_sw82908_sw_reset(dev, ctrl);
		break;

	case HW_RESET_ASYNC:
	case HW_RESET_SYNC:
		ret = dlkm_sw82908_hw_reset(dev, ctrl);
		break;
	case HW_RESET_ONLY:
		ret = dlkm_sw82908_only_hw_reset(dev);
		break;
	case HW_RESET_POWER:
		ret = dlkm_sw82908_power_reset(dev, ctrl);
		break;
	default:
...
static int dlkm_sw82908_power(struct device *dev, int ctrl)
{
	...
	case POWER_HW_RESET_ASYNC:
		TOUCH_I("%s, HW reset\n", __func__);
		dlkm_sw82908_reset_ctrl(dev, HW_RESET_ASYNC);
		break;
	case POWER_HW_RESET_SYNC:
		...
		break;
	case POWER_SW_RESET:
		dlkm_sw82908_reset_ctrl(dev, SW_RESET);
		break;
	case POWER_SLEEP:
	case POWER_WAKE:
	default:
...
static int dlkm_sw82908_ic_boot_check(struct device *dev, u32 *boot_st)
{
	u32 bootmode = 0;
	u32 rdata_crc = 0;
	u32 rdata_pass = ~0;
	...
	ret = dlkm_sw82908_read_value(dev, GDMA_CRC_RESULT, &rdata_crc);
	err |= (rdata_crc != CRC_FIXED_VALUE);

	ret = dlkm_sw82908_read_value(dev, GDMA_CRC_PASS, &rdata_pass);
	err |= (!rdata_pass)<<1;

	ret = dlkm_sw82908_read_value(dev, INFO_PTR_ADDR, &rdata_ptr);
	err |= (!rdata_ptr)<<2;
...
int dlkm_sw82908_ic_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	u32 bootmode = 0;
	...
	ret = dlkm_sw82908_reg_read(dev, CHIP_INFO + tc_version,
			&d->ic_info.version, sizeof(d->ic_info.version));

	ret = dlkm_sw82908_reg_read(dev, CHIP_INFO + tc_product_id1,
			&d->ic_info.product_id, 8);

#if !IS_ENABLED(__SW82908_STILL_BRING_UP)
	ret = dlkm_sw82908_reg_read(dev, PT_INFO + pt_info_channel,
			&d->ic_info.pt_info, sizeof(d->ic_info.pt_info));
#endif	/* __SW82908_STILL_BRING_UP */

	dlkm_sw82908_ic_boot_check(dev, &bootmode);
...
static void dlkm_sw82908_get_longpress_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	d->lpwg_longpress.area.x1 = 423;
	d->lpwg_longpress.area.y1 = 1726;
	d->lpwg_longpress.area.x2 = 652;
	d->lpwg_longpress.area.y2 = 1954;
...
static int dlkm_sw82908_longpress_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_lpwg_longpress_buf buf = {0, };
	struct sw82908_active_area area = {0, };
	int ret = 0;
	...
	if (enable) {
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LONG_PRESS_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
...
//			set_debug_reason(dev, LPWG_DEBUG_LONGPRESS);
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LONG_PRESS_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
...
static void dlkm_sw82908_get_onetap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
...
static int dlkm_sw82908_onetap_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_lpwg_onetap_buf buf = {0, };
	struct sw82908_active_area area = {0, };
	int ret = 0;
	...
	if (enable) {
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + ONE_TAP_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
			TOUCH_E("failed to write long press register (ret = %d)\n",
					ret);
		else {
...
//			set_debug_reason(dev, LPWG_DEBUG_LONGPRESS);
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + ONE_TAP_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
			TOUCH_E("failed to clear long press register (ret = %d)\n",
					ret);
...
static void set_debug_reason(struct device *dev, int type)
{
	...
	dlkm_sw82908_reg_write(dev, ABT_CMD + LPWG_FAILREASON_ON, &data, sizeof(data));
}

static void dlkm_sw82908_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82908_get_tci_data(struct device *dev, int count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	u8 i = 0;
	...
	if (!count || count > TAP_COUNT_KNOCK_ON) {
		TOUCH_I("Invalid coordinate count : %d\n", count);
		return 0;
	}
...
static int dlkm_sw82908_tci_active_area(struct device *dev,
		u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == HALL_NEAR)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_X1, &active_area[0], sizeof(u32));
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_Y1, &active_area[1], sizeof(u32));
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_X2, &active_area[2], sizeof(u32));
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_Y2, &active_area[3], sizeof(u32));

	return ret;
}

static int dlkm_sw82908_tci_control(struct device *dev, int type)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	...
	case ENABLE_CTRL:
		...
		lpwg_data = info1->enable;
		ret = dlkm_sw82908_reg_write(dev, addr, &lpwg_data, sizeof(lpwg_data));
		break;
	...
	case ACTIVE_AREA_CTRL:
		ret = dlkm_sw82908_tci_active_area(dev,
		 ...
		 *  addr = ABT_CMD + TCI_ENABLE;
		 *  lpwg_data = 0;
		 * ret = sw82908_reg_write(dev, addr, &lpwg_data, sizeof(lpwg_data));
		 * break;
		 */
...
static int dlkm_sw82908_tci_knock(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	//struct tci_info *info2 = &ts->tci.info[TCI_2];
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + TCI_ENABLE,
			&lpwg_data[0], sizeof(lpwg_data));

	ret = dlkm_sw82908_tci_control(dev, ACTIVE_AREA_CTRL);

	set_debug_reason(dev, LPWG_DEBUG_TCI_1);
	return ret;
}

static int dlkm_sw82908_tci_password(struct device *dev)
{
	set_debug_reason(dev, LPWG_DEBUG_TCI_2);
	return dlkm_sw82908_tci_knock(dev);
}

static void dlkm_sw82908_init_ai_pick_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
static void dlkm_sw82908_ai_pick_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	...
	if (enable) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor == PROX_NEAR || ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == HALL_NEAR) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE) {
			TOUCH_I("%s: enable ai_pick gesture & area (lpwg_mode : %d)\n",
					__func__, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);

			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				TOUCH_I("%s: force wake IC by ai_pick\n", __func__);
				dlkm_sw82908_sleep_ctrl(dev, IC_NORMAL);
				dlkm_sw82908_tc_driving(dev, LCD_MODE_U0);
			}

			info1->enable |= 0x01;
			info1->intr_delay = 0;
			info1->tap_distance = 10;
			dlkm_sw82908_tci_knock(dev);

			dlkm_sw82908_tci_active_area(dev,
					d->ai_pick.total_area.x1,
					d->ai_pick.total_area.y1,
					d->ai_pick.total_area.x2,
					d->ai_pick.total_area.y2);
		} else if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_PASSWORD_ONLY) {
			TOUCH_I("%s: enable ai_pick gesture (lpwg_mode : %d)\n",
					__func__, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);

			info1->enable |= 0x01;
			info1->intr_delay = 0; // ts->tci.double_tap_check ? 68 : 0;
		...
		if ((ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE) ||
				(ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_PASSWORD_ONLY)) {
			if (d->ai_pick.pre_enable) {
				TOUCH_I("%s: restore lpwg setting (lpwg_mode : %d)\n",
						__func__, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
				dlkm_sw82908_lpwg_mode(dev);
			}
		} else {
...
static void dlkm_sw82908_print_ai_pick_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
static bool dlkm_sw82908_check_ai_pick_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_active_area *area = &d->ai_pick.total_area;
	int i = 0;
...
static int dlkm_sw82908_lpwg_control(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	case LPWG_NONE:
		info1->enable = 0;
		ret = dlkm_sw82908_tci_control(dev, ENABLE_CTRL);
		if (d->ai_pick.enable)
			dlkm_sw82908_ai_pick_enable(dev, d->ai_pick.enable);
		break;
	case LPWG_DOUBLE_TAP:
		...
		ret = dlkm_sw82908_tci_control(dev, ACTIVE_AREA_CTRL);
		ret = dlkm_sw82908_tci_knock(dev);
		break;
	case LPWG_PASSWORD:
		info1->enable = 0x01 | (0x01 << 16);
		info1->intr_delay = 0; //ts->tci.double_tap_check ? 68 : 0;
		info1->tap_distance = 7;

		ret = dlkm_sw82908_tci_control(dev, ACTIVE_AREA_CTRL);
		ret = dlkm_sw82908_tci_password(dev);
		break;
	case LPWG_PASSWORD_ONLY:
		info1->enable = 0x01 << 16;
		info1->intr_delay = 0;
		info1->tap_distance = 10;
...
static int dlkm_sw82908_get_swipe_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	u32 rdata[3];
...
static void dlkm_sw82908_get_swipe_info(struct device *dev)
{
...
static int dlkm_sw82908_lpwg_abs_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_lpwg_abs_buf buf = {0, };
	struct sw82908_active_area area = {0, };
	int ret = 0;
	...
	if (enable) {
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LPWG_ABS_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
		...
		ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LPWG_ABS_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
...
static void dlkm_sw82908_get_lpwg_abs_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
static void dlkm_sw82908_print_lpwg_abs_info(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
static int dlkm_sw82908_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct swipe_ctrl *ctrl[SW82908_SWIPE_NUM] = {	/* L, R, U, D */
		&ts->swipe[SWIPE_L],
		&ts->swipe[SWIPE_R],
		&ts->swipe[SWIPE_U],
		&ts->swipe[SWIPE_D],
	};
	struct swipe_ctrl *ctrl2[SW82908_SWIPE2_NUM] = {	/* L2, R2 */
		&ts->swipe[SWIPE_L2],
		&ts->swipe[SWIPE_R2],
	};
	struct sw82908_swipe_buf buf = {0, };
	struct sw82908_swipe2_buf buf2 = {0, };
	struct active_area area[4] = {0, };
	struct active_area start_area[4] = {0, };
	int i = 0;
	int ret = 0;
	...
	TOUCH_I("%s: SWIPE L(%d),R(%d),U(%d),D(%d),L2(%d),R2(%d)\n", __func__,
		ctrl[SW82908_SWIPE_L]->enable, ctrl[SW82908_SWIPE_R]->enable,
		ctrl[SW82908_SWIPE_U]->enable, ctrl[SW82908_SWIPE_D]->enable,
		ctrl2[SW82908_SWIPE2_L]->enable, ctrl2[SW82908_SWIPE2_R]->enable);

	if (enable) {
		memset(&buf, 0, sizeof(buf));

		for (i = 0; i < SW82908_SWIPE_NUM; i++) {	/* L, R, U, D  */
		...
		buf.area_hori[0] = area[SW82908_SWIPE_L].x1;
		buf.area_hori[1] = area[SW82908_SWIPE_R].x1;
		buf.area_hori[2] = area[SW82908_SWIPE_L].y1;
		buf.area_hori[3] = area[SW82908_SWIPE_R].y1;
		buf.area_hori[4] = area[SW82908_SWIPE_L].x2;
		buf.area_hori[5] = area[SW82908_SWIPE_R].x2;
		buf.area_hori[6] = area[SW82908_SWIPE_L].y2;
		buf.area_hori[7] = area[SW82908_SWIPE_R].y2;

		buf.area_verti[0] = area[SW82908_SWIPE_U].x1;
		buf.area_verti[1] = area[SW82908_SWIPE_D].x1;
		buf.area_verti[2] = area[SW82908_SWIPE_U].y1;
		buf.area_verti[3] = area[SW82908_SWIPE_D].y1;
		buf.area_verti[4] = area[SW82908_SWIPE_U].x2;
		buf.area_verti[5] = area[SW82908_SWIPE_D].x2;
		buf.area_verti[6] = area[SW82908_SWIPE_U].y2;
		buf.area_verti[7] = area[SW82908_SWIPE_D].y2;

		buf.start_hori[0] = start_area[SW82908_SWIPE_L].x1;
		buf.start_hori[1] = start_area[SW82908_SWIPE_R].x1;
		buf.start_hori[2] = start_area[SW82908_SWIPE_L].y1;
		buf.start_hori[3] = start_area[SW82908_SWIPE_R].y1;
		buf.start_hori[4] = start_area[SW82908_SWIPE_L].x2;
		buf.start_hori[5] = start_area[SW82908_SWIPE_R].x2;
		buf.start_hori[6] = start_area[SW82908_SWIPE_L].y2;
		buf.start_hori[7] = start_area[SW82908_SWIPE_R].y2;

		buf.start_verti[0] = start_area[SW82908_SWIPE_U].x1;
		buf.start_verti[1] = start_area[SW82908_SWIPE_D].x1;
		buf.start_verti[2] = start_area[SW82908_SWIPE_U].y1;
		buf.start_verti[3] = start_area[SW82908_SWIPE_D].y1;
		buf.start_verti[4] = start_area[SW82908_SWIPE_U].x2;
		buf.start_verti[5] = start_area[SW82908_SWIPE_D].x2;
		buf.start_verti[6] = start_area[SW82908_SWIPE_U].y2;
		buf.start_verti[7] = start_area[SW82908_SWIPE_D].y2;

		memset(&buf2, 0, sizeof(buf2));

		for (i = 0; i < SW82908_SWIPE2_NUM; i++) {	/* L2, R2 */
		...
		buf2.area[0] = area[SW82908_SWIPE2_L].x1;
		buf2.area[1] = area[SW82908_SWIPE2_R].x1;
		buf2.area[2] = area[SW82908_SWIPE2_L].y1;
		buf2.area[3] = area[SW82908_SWIPE2_R].y1;
		buf2.area[4] = area[SW82908_SWIPE2_L].x2;
		buf2.area[5] = area[SW82908_SWIPE2_R].x2;
		buf2.area[6] = area[SW82908_SWIPE2_L].y2;
		buf2.area[7] = area[SW82908_SWIPE2_R].y2;

		buf2.start[0] = start_area[SW82908_SWIPE2_L].x1;
		buf2.start[1] = start_area[SW82908_SWIPE2_R].x1;
		buf2.start[2] = start_area[SW82908_SWIPE2_L].y1;
		buf2.start[3] = start_area[SW82908_SWIPE2_R].y1;
		buf2.start[4] = start_area[SW82908_SWIPE2_L].x2;
		buf2.start[5] = start_area[SW82908_SWIPE2_R].x2;
		buf2.start[6] = start_area[SW82908_SWIPE2_L].y2;
		buf2.start[7] = start_area[SW82908_SWIPE2_R].y2;


		if (atomic_read(&ts->state.sleep) == IC_NORMAL) {
			TOUCH_D(TRACE, "state.sleep : %d \n", atomic_read(&ts->state.sleep));
			ret = dlkm_sw82908_reg_write(dev, ABT_CMD + SWIPE_ON,
					&buf, sizeof(buf));
			if (ret < 0) {
			...
			ret = dlkm_sw82908_reg_write(dev, ABT_CMD + SWIPE2_ON,
					&buf2, sizeof(buf2));
			if (ret < 0) {
		...
		for (i = 0; i < SW82908_SWIPE2_NUM; i++)
...
#if IS_ENABLED(__SUPPORT_CLK_CTRL)
static int dlkm_sw82908_clock(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (onoff) {
		if (ts->bus_type == HWIF_SPI) {
			dlkm_sw82908_reg_write(dev, SPI_OSC_CTL, &onoff, sizeof(onoff));
			atomic_set(&ts->state.sleep, IC_NORMAL);
			touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
		...
		//pllosc_sel
		dlkm_sw82908_reg_read(dev, SYS_CLK_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(pllosc_sel);
		dlkm_sw82908_reg_write(dev, SYS_CLK_CTL, &reg_val, sizeof(reg_val));
		TOUCH_I("SYS_CLK_CTL(0x%x) value(0x%x) pllosc_sel(0x%x) \n", SYS_CLK_CTL, reg_val, pllosc_sel);
		touch_msleep(1);

		//xe_en
		dlkm_sw82908_reg_read(dev, SYS_PLL_CTL1, &reg_val , sizeof(reg_val));
		reg_val &= ~(xe_en);
		dlkm_sw82908_reg_write(dev, SYS_PLL_CTL1, &reg_val, sizeof(reg_val));
		TOUCH_I("SYS_PLL_CTL1(0x%x) value(0x%x) xe_en(0x%x) \n", SYS_PLL_CTL1, reg_val, xe_en);
		touch_msleep(1);

		//ldo15_bias_mute
		dlkm_sw82908_reg_read(dev, LDO15_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(r_ldo15_bias_mute);
		dlkm_sw82908_reg_write(dev, LDO15_CTL, &reg_val, sizeof(reg_val));
		TOUCH_I("LDO15_CTL(0x%x) value(0x%x) r_ldo15_bias_mute(0x%x) \n", LDO15_CTL, reg_val, r_ldo15_bias_mute);
		touch_msleep(1);

		//ldo_bias
		dlkm_sw82908_reg_read(dev, SYS_LDO_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(ldo_bias);
		dlkm_sw82908_reg_write(dev, SYS_LDO_CTL, &reg_val, sizeof(reg_val));
		...
		dlkm_sw82908_reg_write(dev, SPI_OSC_CTL, &reg_val, sizeof(reg_val));
...
static inline int dlkm_sw82908_clock(struct device *dev, u32 onoff)
{
	TOUCH_NOOP();
	return 0;
}
...
static void dlkm_sw82908_print_failreason(struct device *dev, int type, int count)
{
	u64 buf = 0;
	u8 data = 0;
	u8 fail_num = 0;
	...
	switch (type) {
	case LPWG_DEBUG_TCI_1:
		ret = dlkm_sw82908_reg_read(dev, ABT_CMD + TCI0_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_TCI_NUM;
		break;
	case LPWG_DEBUG_TCI_2:
		ret = dlkm_sw82908_reg_read(dev, ABT_CMD + TCI1_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_TCI_NUM;
		break;
	case LPWG_DEBUG_SWIPE:
		ret = dlkm_sw82908_reg_read(dev, ABT_CMD + SWIPE_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_SWIPE_NUM;
		break;
	case LPWG_DEBUG_LONGPRESS:
		ret = dlkm_sw82908_reg_read(dev, ABT_CMD + LONGPRESS_FAILREASON_BUF,
				&buf, sizeof(buf));
...
static void dlkm_sw82908_lpwg_failreason(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	u32 status = 0;
	int ret,i = 0;
	...
	ret = dlkm_sw82908_reg_read(dev, ABT_CMD + LPWG_FAILREASON_STS,
	...
	if (count[LPWG_DEBUG_TCI_1]) { /* Knock-on */
		dlkm_sw82908_print_failreason(dev, LPWG_DEBUG_TCI_1,
				count[LPWG_DEBUG_TCI_1]);
	}

	if (count[LPWG_DEBUG_TCI_2]) { /* Knock-code */
		dlkm_sw82908_print_failreason(dev, LPWG_DEBUG_TCI_2,
				count[LPWG_DEBUG_TCI_2]);
	}

	if (count[LPWG_DEBUG_SWIPE]) { /* Swipe */
		dlkm_sw82908_print_failreason(dev, LPWG_DEBUG_SWIPE,
				count[LPWG_DEBUG_SWIPE]);
	}

#if IS_ENABLED(__SUPPORT_LONGPRESS)
	if (count[LPWG_DEBUG_LONGPRESS]) { /* LongPress */
		dlkm_sw82908_print_failreason(dev, LPWG_DEBUG_LONGPRESS,
...
int dlkm_sw82908_tc_driving(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct project_param *param = &d->p_param;
	...
	if (mode != LCD_MODE_STOP) {
		ctrl = 0x04;
		dlkm_sw82908_reg_write(dev, addr, &ctrl, sizeof(ctrl));
		TOUCH_I("sw82908_tc_stop(0x%x) = LCD_MODE_STOP(%d), (0x%x)\n",
				addr, mode, ctrl);
...
*/
	...
	if (mode == LCD_MODE_U3) {
		TOUCH_I("in U3 mode(%d), set vsync driving\n", mode);
		dlkm_sw82908_vsync_set(dev, 1);
	}

	dlkm_sw82908_reg_write(dev, addr,
			&ctrl, sizeof(ctrl));
	TOUCH_I("sw82908_tc_driving(0x%x) = %s(%d), (0x%x)\n",
	...
	/* tc_status check */
	dlkm_sw82908_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
	TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);

#if 0
	dlkm_sw82908_reg_read(dev, TC_STS, (u8 *)&rdata, sizeof(u32));
	TOUCH_I("read tc_status(%x) = %x\n", TC_STS, rdata);

	dlkm_sw82908_reg_read(dev, TC_CMD, (u8 *)&rdata, sizeof(rdata));
...
static int dlkm_sw82908_release_pen_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_TRACE();
	ts->pdata.in_range = 0;
...
int dlkm_sw82908_sleep_ctrl(struct device *dev, int new_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void dlkm_sw82908_longpress_release(struct device *dev) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);

	if (d->longpress_uevent_status == TOUCH_IRQ_LONGPRESS_DOWN) {
		touch_send_uevent(ts, TOUCH_UEVENT_LONGPRESS_UP);
		d->longpress_uevent_status = TOUCH_UEVENT_LONGPRESS_UP;
	}
}
...
static int dlkm_sw82908_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg) {
			...
			ret = dlkm_sw82908_lpwg_control(dev, LPWG_DOUBLE_TAP);
			dlkm_sw82908_swipe_enable(dev, true);
			ret = dlkm_sw82908_tc_driving(dev, LCD_MODE_U0);
			return 0;
		}
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
			...
			dlkm_sw82908_longpress_release(dev);
			dlkm_sw82908_lpwg_failreason(dev);
		} else if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor == PROX_NEAR) {
			/* deep sleep */
			TOUCH_I("%s %d line - deep sleep by prox\n", __func__, __LINE__);
			dlkm_sw82908_sleep_ctrl(dev, IC_DEEP_SLEEP);
		} else if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == HALL_NEAR) {
			...
			TOUCH_D(TRACE, "knock mode %d, screen %d, proxy %d, qcover %d\n",
					ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen,
					ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover);
			...
			if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE
				...
				dlkm_sw82908_sleep_ctrl(dev, IC_NORMAL);
				ret = dlkm_sw82908_lpwg_control(dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
				dlkm_sw82908_swipe_enable(dev, true);
				dlkm_sw82908_tc_driving(dev, LCD_MODE_U0);
				if (d->lpwg_abs.enable) {
					TOUCH_I("%s: enable lpwg_abs\n", __func__);
					dlkm_sw82908_lpwg_abs_enable(dev, d->lpwg_abs.enable);
				}
				if (d->ai_pick.enable) {
				...
				if (d->lpwg_longpress.enable) {
					...
					dlkm_sw82908_longpress_enable(dev, d->lpwg_longpress.enable);
				}
				if (d->lpwg_onetap.enable) {
					TOUCH_I("%s: enable onetap\n", __func__);
					dlkm_sw82908_onetap_enable(dev, d->lpwg_onetap.enable);
				}
			}
		}
		return ret;
...
#if IS_ENABLED (CONFIG_LGE_TOUCH_PEN)
	dlkm_sw82908_release_pen_event(dev);
#endif
	touch_report_all_event(ts);
	...
	if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == HALL_NEAR) {
			...
			dlkm_sw82908_sleep_ctrl(dev, IC_NORMAL);
			dlkm_sw82908_lpwg_control(dev, LPWG_NONE);
			ret = dlkm_sw82908_tc_driving(dev, LCD_MODE_U3);
		}
	} else if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor == PROX_NEAR) {
...
static int dlkm_sw82908_setup_q_sensitivity(struct device *dev, int enable)
{
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + COVER_SENSITIVITY,
...
static int dlkm_sw82908_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
	...
	case LPWG_ACTIVE_AREA:
		ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.area.x1 = value[0];
		ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.area.x2 = value[1];
		ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.area.y1 = value[2];
		ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.area.y2 = value[3];
	...
	case LPWG_UPDATE_ALL:
		if ((ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen == 1 && value[1] == 0 &&
					ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor == PROX_FAR) ||
				(ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == 1 && value[3] == 0))
			dlkm_sw82908_setup_q_sensitivity(dev, 0);
		ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode = value[0];
		ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen = value[1];
		ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor = value[2];
		ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover = value[3];

		TOUCH_I("LPWG_UPDATE_ALL: mode[%s], screen[%s], sensor[%s], qcover[%s]\n",
				lpwg_mode_strr[ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode],
				ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen ? "ON" : "OFF",
				ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor ? "FAR" : "NEAR",
				ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover ? "CLOSE" : "OPEN");

		ret = dlkm_sw82908_lpwg_mode(dev);
		if (ret)
...
static void dlkm_sw82908_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	dlkm_sw82908_reg_write(dev, ABT_CMD + SPECIAL_CHARGER_INFO,
...
static void dlkm_sw82908_vsync_mode(struct device *dev, u32 mode)
{
	struct sw82908_data *d = to_sw49202_data(dev);

	TOUCH_TRACE();

	d->prev_vsync_mode = d->vsync_mode;
	d->vsync_mode = ((mode == 2) ? TOUCH_VSYNC_120HZ : TOUCH_VSYNC_60HZ);
	TOUCH_I("vsync_mode: %d(prev: %d)\n", d->vsync_mode, d->prev_vsync_mode);
}

static int dlkm_sw82908_check_mode(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0;
...
static int dlkm_sw82908_vsync_set(struct device *dev, int force_mode)
{
	...
	TOUCH_I("%s : force_mode = %d / prev_vsync_mode = %d, vsync_mode = %d\n", __func__, force_mode, d->prev_vsync_mode, d->vsync_mode);

	if (force_mode == 1) {
		if (d->vsync_mode == TOUCH_VSYNC_120HZ) {
			data = 0x11;
			TOUCH_I("%s : vsync 120Hz!\n", __func__);
		} else if (d->vsync_mode == TOUCH_VSYNC_60HZ) {
			data = 0x01;
			TOUCH_I("%s : vsync 60Hz!\n", __func__);
		}
	} else if (force_mode == 0){
		if (((d->prev_vsync_mode == TOUCH_VSYNC_60HZ) || (d->prev_vsync_mode == TOUCH_VSYNC_NOT_INITIALIZED)) && (d->vsync_mode == TOUCH_VSYNC_120HZ)) {
			data = 0x11;
			TOUCH_I("%s : vsync 60Hz -> 120Hz\n", __func__);
		} else if (((d->prev_vsync_mode == TOUCH_VSYNC_120HZ) || (d->prev_vsync_mode == TOUCH_VSYNC_NOT_INITIALIZED)) && (d->vsync_mode == TOUCH_VSYNC_60HZ)) {
			data = 0x01;
			TOUCH_I("%s : vsync 120Hz -> 60Hz\n", __func__);
		} else {
			TOUCH_I("%s : Touch vsync mode is not changed (vsync mode : %d, prev_vsync_mode : %d, force_mode : %d)\n", __func__, d->vsync_mode, d->prev_vsync_mode, force_mode);
		}

	}

	if (data) {
		TOUCH_I("%s : change vsync driving! (data = 0x%x)\n", __func__, data);
		dlkm_sw82908_reg_write(dev, VSYNC_DRIVING_CMD, &data, sizeof(data));
	}
	return ret;
}

#if IS_ENABLED(__SUPPORT_NOTIFY_LCD_EVENT_REG)
static void dlkm_sw82908_lcd_event_read_reg(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	...
	dlkm_sw82908_read_value(dev, TC_IC_STATUS, &rdata[0]);

	dlkm_sw82908_read_value(dev, TC_STS, &rdata[1]);

	dlkm_sw82908_read_value(dev, version_addr, &rdata[2]);

	dlkm_sw82908_read_value(dev, SPR_CHIP_ID, &rdata[3]);
...
static int dlkm_sw82908_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_TRACE();
	TOUCH_I("TA Type: %d\n", atomic_read(&ts->state.connect));
	dlkm_sw82908_connect(dev);
	return 0;
}

static int dlkm_sw82908_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82908_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82908_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);

	if (value == DEBUG_TOOL_ENABLE) {
	//	ts->driver->irq_handler = sw82908_sic_abt_irq_handler;
	} else {
		ts->driver->irq_handler = dlkm_sw82908_irq_handler;
	}

	return 0;
}

static int dlkm_sw82908_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
...
static int dlkm_sw82908_rotation(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + ROTATION_ANGLE,
...
static int dlkm_sw82908_grip_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u32 buf = 0;
	...
	if (1) //if (!ts->have_grip_area) -> The registers don't exist due to insufficient memory.
		return 0;
	...
	buf = 1;
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_ENABLE,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_START,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_END,
							&buf, sizeof(buf));
	if (ret < 0)
...
#if 0 // FW will handle it. Keep this block temporary.
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + PORTRAIT_VERTICAL_A_B_WIDTH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + PORTRAIT_VERTICAL_C_WIDTH_HEIGHT,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LANDSCAPE_VERTICAL_A_B_WIDTH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LANDSCAPE_VERTICAL_C_WIDTH_HEIGH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82908_reg_write(dev, ABT_CMD + LANDSCAPE_HORIZONTAL_A_B_HEIGHT,
							&buf, sizeof(buf));
	if (ret < 0)
...
static void dlkm_sw82908_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct sw82908_data *d =
		container_of(to_delayed_work(fb_notify_work),
				struct sw82908_data, fb_notify_work);
	int ret = 0;
...
static void dlkm_sw82908_partial_tcdriving_work_func(struct work_struct *partial_tcdriving)
{
	struct sw82908_data *d =
			container_of(to_delayed_work(partial_tcdriving),
					struct sw82908_data, partial_tcdriving);
	...
	if (atomic_read(&ts->state.fb) == FB_RESUME && !ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen
				&& ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor != PROX_NEAR && ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover != HALL_NEAR) {
		TOUCH_I("resume Partial\n");
		dlkm_sw82908_sleep_ctrl(d->dev, IC_NORMAL);
		dlkm_sw82908_lpwg_control(d->dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
		dlkm_sw82908_swipe_enable(d->dev, true);
		dlkm_sw82908_tc_driving(d->dev, LCD_MODE_U0);
		if (d->ai_pick.enable) {
			TOUCH_I("%s: enable ai pick\n", __func__);
			dlkm_sw82908_ai_pick_enable(d->dev, d->ai_pick.enable);
		}
		//sw82908_longpress_enable(d->dev, false);
	} else {
...
static int dlkm_sw82908_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct project_param *param = &d->p_param;
	...
	case LCD_EVENT_LCD_MODE:
		...
		dlkm_sw82908_lcd_mode(dev, *(u32 *)data);
		ret = dlkm_sw82908_check_mode(dev);
...
#if IS_ENABLED(__SUPPORT_NOTIFY_LCD_EVENT_REG)
		dlkm_sw82908_lcd_event_read_reg(dev);
#endif
		break;
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = dlkm_sw82908_usb_status(dev, *(u32 *)data);
		break;
	case NOTIFY_WIRELESS:
		TOUCH_I("NOTIFY_WIRELEES!\n");
		ret = dlkm_sw82908_wireless_status(dev, *(u32 *)data);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = dlkm_sw82908_earjack_status(dev, *(u32 *)data);
		break;
	case NOTIFY_FM_RADIO:
		...
		switch (d->driving_mode) {
			...
			case LCD_MODE_U3:
				...
				ret = dlkm_sw82908_reg_write(dev, ABT_CMD +
						FM_RADIO_CTRL, (u32 *)data, sizeof(u32));
				break;
		}
		dlkm_sw82908_reg_write(dev, addr,
				&ctrl, sizeof(ctrl));
		TOUCH_I("sw82908 fm radio state write(0x%x) = (0x%x)\n",
				addr,  ctrl);
		/* tc_status check */
		dlkm_sw82908_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
		TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);
...
//					}
		...
		dlkm_sw82908_reg_write(dev, addr,
				&ctrl, sizeof(ctrl));
		TOUCH_I("sw82908_pen state write(0x%x) = (0x%x)\n",
				addr,  ctrl);
		/* tc_status check */
		dlkm_sw82908_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
		TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);
	...
	case NOTIFY_DUALSCREEN_STATE:
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
			...
			dlkm_sw82908_tc_driving(dev, LCD_MODE_U3);
		}
		break;
	case LCD_EVENT_CHANGE_FPS:
		TOUCH_I("LCD_EVENT_CHANGE_FPS : %d\n", *(int *)data);
		dlkm_sw82908_vsync_mode(dev, *(u32 *)data);
		dlkm_sw82908_vsync_set(dev, 0);
		break;
	default:
...
static int dlkm_sw82908_init_pm(struct device *dev)
{
#if IS_ENABLED(CONFIG_DRM_MSM) && IS_ENABLED(CONFIG_FB)
#if IS_ENABLED(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: drm_notif change\n", __func__);
	ts->drm_notif.notifier_call = dlkm_sw82908_drm_notifier_callback;
#endif
#elif IS_ENABLED(CONFIG_FB)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: fb_notif change\n", __func__);
	ts->fb_notif.notifier_call = dlkm_sw82908_fb_notifier_callback;
#endif

	return 0;
}

static int dlkm_sw82908_init_works(struct sw82908_data *d)
{
	...
	INIT_DELAYED_WORK(&d->partial_tcdriving, dlkm_sw82908_partial_tcdriving_work_func);
	INIT_DELAYED_WORK(&d->fb_notify_work, dlkm_sw82908_fb_notify_work_func);

	return 0;
}

static void dlkm_sw82908_init_locks(struct sw82908_data *d)
{
	mutex_init(&d->io_lock);
}

static int dlkm_sw82908_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = NULL;
	int boot_mode = TOUCH_NORMAL_BOOT;

	TOUCH_TRACE();

	TOUCH_I("sw82908 probe\n");
	...
	dlkm_sw82908_init_works(d);
	dlkm_sw82908_init_locks(d);
...
//				atomic_read(&ts->state.film_mode));
	...
	dlkm_sw82908_get_tci_info(dev);
	dlkm_sw82908_get_swipe_info(dev);

	dlkm_sw82908_get_lpwg_abs_info(dev);
	dlkm_sw82908_init_ai_pick_info(dev);
	dlkm_sw82908_get_longpress_info(dev);
	dlkm_sw82908_get_onetap_info(dev);
	...
	cmds_adder(dev, (char *)ts->touch_ic_name, sw82908_cmd_lists);
#endif
	d->lcd_mode = LCD_MODE_U3;
	TOUCH_I("%s : vsync mode %d - %d(before)\n", __func__, d->prev_vsync_mode, d->vsync_mode);
	d->vsync_mode = TOUCH_VSYNC_NOT_INITIALIZED;
	d->prev_vsync_mode = TOUCH_VSYNC_NOT_INITIALIZED;
	TOUCH_I("%s : vsync mode %d - %d(after)\n", __func__, d->prev_vsync_mode, d->vsync_mode);
	d->lpwg_failreason_ctrl = LPWG_FAILREASON_ENABLE;
	d->lpwg_longpress.enable = LONGPRESS_ENABLE;


	return 0;
}

static int dlkm_sw82908_remove(struct device *dev)
{
	TOUCH_TRACE();
...
static int dlkm_sw82908_shutdown(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
static int dlkm_sw82908_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	u32 bin_ver_offset = *((u32 *)&fw->data[BIN_VER_OFFSET_POS]);
	u32 bin_pid_offset = *((u32 *)&fw->data[BIN_PID_OFFSET_POS]);
	struct sw82908_version *device = &d->ic_info.version;
	struct sw82908_version_bin *binary = NULL;
	char pid[12] = {0};
	...
	binary = (struct sw82908_version_bin *)&fw->data[bin_ver_offset];
	...
	if ((d->ic_info.img_version.major == 0) && (d->ic_info.img_version.minor == 0) && (d->ic_info.img_version.build == 0)) {
		memcpy(&d->ic_info.img_version, &fw->data[bin_ver_offset], sizeof(struct sw82908_version_bin));
...
static int dlkm_sw82908_condition_wait(struct device *dev,
		...
		u32 mask, u32 delay, u32 retry, int not_cond)
{
	...
	do {
		touch_msleep(delay);
		ret = dlkm_sw82908_read_value(dev, addr, &data);
		if (ret >= 0) {
...
#if IS_ENABLED(__SW82908_SUPPORT_CFG)
static int dlkm_sw82908_fw_verify_cfg(char *buf)
{
...
static int dlkm_sw82908_fw_verify_s_cfg(char *buf, int index)
{
	struct cfg_head *head = (struct cfg_head *)buf;
...
#endif	/* __SW82908_SUPPORT_CFG */

static int dlkm_sw82908_fw_binary_verify(struct device *dev, u8 *fw_buf, int fw_size)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	int flash_fw_size = d->p_param.flash_fw_size;
	...
	ret = dlkm_sw82908_fw_verify_cfg((char *)head);
	if (ret < 0) {
	...
	for (i = 0; i < s_cfg_cnt; i++) {
		ret = dlkm_sw82908_fw_verify_s_cfg((char *)s_cfg_base, i);
		if (ret < 0) {
...
static void sw82908_prt_reg_map(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_reg_info *info = &d->reg_info;
...
*/
/*
void sw82908_default_reg_map(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	TOUCH_I("================================================\n");
	TOUCH_I("sw82908 default register map loaded!!\n");
	TOUCH_I("================================================\n");
	sw82908_prt_reg_map(dev);
}
*/
/*
int sw82908_chip_info_load(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	u32 info_ptr_val = 0;
	u32 tc_status_val = 0;
	...
	if (sw82908_reg_read(dev, INFO_PTR_ADDR,
	...
	if (sw82908_reg_read(dev, TC_STS,
	...
	if (sw82908_reg_read(dev, chip_info_addr, (u8 *)&d->chip_info,
				sizeof(struct sw82908_chip_info)) < 0) {
		TOUCH_E("Chip info Read Error\n");
		goto error;
	}

	if (sw82908_reg_read(dev, reg_info_addr, (u8 *)&d->reg_info,
				sizeof(struct sw82908_reg_info)) < 0) {
...
error:
	sw82908_default_reg_map(dev);
	return -1;
}
*/

static int dlkm_sw82908_flash_erase_check(struct device *dev)
{
	...
	dlkm_sw82908_reg_write(dev, spr_flash_offset, &start_offset, sizeof(start_offset));
	TOUCH_I("flash read offset(start_offset) %x \n", start_offset);	//for debug

	dlkm_sw82908_reg_read(dev, flash_access_addr, &flash_read_val, sizeof(flash_read_val));
	...
	dlkm_sw82908_reg_write(dev, spr_flash_offset, &end_offset, sizeof(end_offset));
...
static int __used dlkm_sw82908_fw_flash_crc(struct device *dev, u32 *crc_val)
{
	...
	ret = dlkm_sw82908_write_value(dev, gdma_saddr, 0);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_write_value(dev, gdma_ctl, ctrl_data);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_write_value(dev, gdma_start, 1);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_read_value(dev, gdma_crc_result, &data);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_read_value(dev, gdma_crc_pass, &data);
	if (ret < 0) {
...
static int __used dlkm_sw82908_fw_flash_mass_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	ret = dlkm_sw82908_write_value(dev, fc_addr, 0);
	if (ret < 0) {
		fc_err = 1;
		goto out;
	}

	ret = dlkm_sw82908_write_value(dev, fc_ctrl, FC_CTRL_MASS_ERASE);
	if (ret < 0) {
		fc_err = 2;
		goto out;
	}

	dlkm_sw82908_reg_read(dev, fc_ctrl, &read_reg, sizeof(read_reg));
	TOUCH_I("fc_ctrl erase command read_val : %x\n", read_reg);

	ret = dlkm_sw82908_write_value(dev, fc_stat, 1);
	if (ret < 0) {
		fc_err = 3;
		goto out;
	}
	...
	chk_resp = 1;
	ret = dlkm_sw82908_condition_wait(dev, spi_flash_status, &data,
	...
	dlkm_sw82908_reg_read(dev, SYS_RST_CTL, &read_reg, sizeof(read_reg));
	TOUCH_I("1st cm3 status check read_val : %x\n", read_reg);

	//Flash Area check(first,last)
	ret = dlkm_sw82908_flash_erase_check(dev);
	if (ret < 0) {
		fc_err = 5;
		TOUCH_E("sw82908_flash_erase_check(fc_err : %d) error!!!\n", fc_err);
		goto out;
	}
	...
	ret = dlkm_sw82908_fw_flash_crc(dev, &erase_crc_val);
	if (ret < 0) {
		fc_err = 6;
		TOUCH_E("sw82908_flash_crc(fc_err : %d) error!!!\n", fc_err);
		goto out;
	}
...
out:
	...
	dlkm_sw82908_write_value(dev, fc_ctrl, 0);
...
static int __used dlkm_sw82908_fw_flash_page_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	for (i = 0; i < (FLASH_SIZE/FLASH_PAGE_SIZE); i++) {
		ret = dlkm_sw82908_write_value(dev, fc_addr, addr);
		if (ret < 0) {
			fc_err = 1;
			break;
		}

		ret = dlkm_sw82908_write_value(dev, fc_ctrl, FC_CTRL_PAGE_ERASE);
		if (ret < 0) {
			fc_err = 2;
			break;
		}
	...
	dlkm_sw82908_write_value(dev, fc_addr, 0);
	dlkm_sw82908_write_value(dev, fc_ctrl, 0);
...
static int __used dlkm_sw82908_fw_flash_write(struct device *dev, int addr, u8 *dn_buf, int dn_size)
{
	...
	ret = dlkm_sw82908_write_value(dev, fc_offset, addr);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_reg_write(dev, fc_code_access, dn_buf, dn_size);
	if (ret < 0) {
...
static int dlkm_sw82908_fw_rst_ctl(struct device *dev, int val, const char *str)
{
	...
	ret = dlkm_sw82908_write_value(dev, rst_ctl, val);
	if (ret < 0) {
...
static int dlkm_sw82908_fw_upgrade_code(struct device *dev, u8 *dn_buf, int dn_size)
{
//	struct sw82908_data *d = to_sw49202_data(dev);
//	struct project_param *param = &d->p_param;
	u32 fc_ctrl = FC_CTRL;
...
#if 0	/* not used in SW82908 */
	if (param->wa_trim_wr) {
		dlkm_sw82908_read_value(dev, SYS_OSC_CTL, &reg_buf);
		reg_buf &= ~0x7F;
		reg_buf |= 0x5A;
		dlkm_sw82908_write_value(dev, SYS_OSC_CTL, reg_buf);
	}
#endif

//	sw82908_write_value(dev, FLASH_TPROG, 560);

	dlkm_sw82908_write_value(dev, fc_ctrl, FC_CTRL_WR_EN);
	...
	do {
		touch_msleep(10);
		dlkm_sw82908_read_value(dev, BDMA_STS, &reg_buf);
		busycheck_cnt++;
	...
	for (k = 0; k <= write_cnt; k++)
	{
		...
		if(k == write_cnt){
			...
			dlkm_sw82908_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], remain_size);
			reg_buf = remain_size / sizeof(u32);
		}else{
			dlkm_sw82908_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], BDMA_TRANS_SIZE);
			reg_buf = BDMA_TRANS_SIZE / sizeof(u32);
		}
		reg_buf |= BDMA_CTL_BDMA_EN;

		dlkm_sw82908_write_value(dev, BDMA_SADDR, DATASRAM_ADDR);
		dlkm_sw82908_write_value(dev, BDMA_CTL, reg_buf);

		dlkm_sw82908_write_value(dev, BDMA_CAL_OP, BDMA_CAL_OP_CTRL);

		if(config){
			dlkm_sw82908_write_value(dev, BDMA_DADDR, 0x1FC00);
		} else {
			dlkm_sw82908_write_value(dev, BDMA_DADDR, (BDMA_TRANS_SIZE * k));
		}
		dlkm_sw82908_write_value(dev, FC_CTRL, 0x4);
		dlkm_sw82908_write_value(dev, BDMA_START, 1);
...
static int dlkm_sw82908_fw_upgrade_conf(struct device *dev, u8 *pFirm, u32 nSize)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	if (dlkm_sw82908_reg_read(dev, CHIP_INFO, (u8 *)&d->chip_info,
				sizeof(struct sw82908_chip_info)) < 0) {
		TOUCH_E("Chip info Read Error\n");
	}
	...
	dlkm_sw82908_fw_upgrade_code(dev, &pFirm[flash_fw_size], cfg_size);
	touch_msleep(100);
...
static int dlkm_sw82908_fw_upgrade_post(struct device *dev)
{
	u32 crc_val;
	int ret = 0;
	...
	dlkm_sw82908_fw_rst_ctl(dev, 0, NULL);
	touch_msleep(100);

	dlkm_sw82908_fw_rst_ctl(dev, 2, "crc");
	touch_msleep(100);

	ret = dlkm_sw82908_fw_flash_crc(dev, &crc_val);
	if (ret < 0) {
	...
	ret = dlkm_sw82908_fw_rst_ctl(dev, 1, NULL);
	if (ret < 0)
		goto out;

	touch_msleep(100);

	ret = dlkm_sw82908_fw_rst_ctl(dev, 0, NULL);
	if (ret < 0)
		goto out;
...
/*
 *	F/W upgrade Flash Direct Access for sw82908
 */
static int dlkm_sw82908_fw_upgrade(struct device *dev,
		const struct firmware *fw)
{
	...
	img_check_result = dlkm_sw82908_fw_binary_verify(dev, (unsigned char *)fwdata, fw->size);
	...
	ret = dlkm_sw82908_fw_rst_ctl(dev, 2, NULL);
	if (ret < 0)
		return ret;

	TOUCH_I("[fw flash mass erase - start]\n");
	ret = dlkm_sw82908_fw_flash_mass_erase(dev);
	if (ret < 0)
		return ret;
	TOUCH_I("[fw flash mass erase - end]\n");

	TOUCH_I("[Code update]\n");
	dlkm_sw82908_fw_upgrade_code(dev, fwdata, flash_fw_size);

#if 0	/* not fixed in SW82908 */
	dlkm_sw82908_write_value(dev, fw_boot_code_addr, FW_BOOT_LOADER_INIT);

	touch_msleep(10);
	dlkm_sw82908_fw_rst_ctl(dev, 0, NULL);
	touch_msleep(200);

	ret = dlkm_sw82908_condition_wait(dev, fw_boot_code_addr, NULL,
...
#else
//	sw82908_write_value(dev, SYS_RST_CTL, 0);
#endif
...
/*
	if (d->p_param.dynamic_reg) {
		if (sw82908_chip_info_load(dev) < 0) {
...
*/
...
#if IS_ENABLED(__SW82908_SUPPORT_CFG)
	dlkm_sw82908_read_value(dev, CHIP_INFO + 4,
	...
	if (include_conf) {
		TOUCH_I("[CFG update]\n");
		ret = dlkm_sw82908_fw_upgrade_conf(dev, fwdata, fw->size);
		if (ret < 0)
			return ret;
	}
#endif	/* __SW82908_SUPPORT_CFG */

	ret = dlkm_sw82908_fw_upgrade_post(dev);
	if (ret < 0)
		return ret;
...
static int dlkm_sw82908_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
//	struct project_param *param = &d->p_param;
...
//	u32 data = 0;
	...
	d->check_fwup = dlkm_sw82908_fw_compare(dev, fw);
	...
	if (d->check_fwup == UPGRADE_NEED || d->check_fwup == UPGRADE_FORCE) {
		...
		for (i = 0; i < 3 && ret; i++) {
			...
			if (param->wa_trim_wr) {
				if (i == 1) {
					ret = dlkm_sw82908_read_value(dev, SYS_OSC_CTL, &data);
					if (ret < 0) {
					...
					if (data == 4) {
						...
						dlkm_sw82908_write_value(dev, SYS_OSC_CTL, AVERAGE_TRIM_VAL);
					}
				}
			}
		#endif
			if ((atomic_read(&ts->state.mfts) == MFTS_DS_FLAT)
				...
				ret = dlkm_sw82908_fw_upgrade(dev, fw);
				if (ret < 0) {
					TOUCH_E("%s: sw82908_fw_upgrade failed!(ret = %d)\n", __func__, ret);
					//d->check_fwup = UPGRADE_DONE;
					dlkm_sw82908_reset_ctrl(dev, HW_RESET_ONLY);
					//goto error;
				} else {
					TOUCH_I("%s: sw82908_fw_upgrade is done!\n", __func__);
				}
			}
...
static int dlkm_sw82908_app_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	const struct firmware *fw = NULL;
...
#endif
static int dlkm_sw82908_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
	...
	TOUCH_I("sw82908_esd_recovery!!\n");

	return 0;
}

static int dlkm_sw82908_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	int boot_mode = 0;
	...
	case TOUCH_MINIOS_MFTS_DS_FLAT:
		if (!ts->mfts_lpwg) {
			...
			dlkm_sw82908_power(dev, POWER_OFF);
			return -EPERM;
		}
		break;
...
static int dlkm_sw82908_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct project_param *param = &d->p_param;
	...
	if ((d->longpress_uevent_status == TOUCH_IRQ_LONGPRESS_DOWN) && !(ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen)) {
		TOUCH_I("skip sw82908_resume\n");
		return 0;
	}
#if IS_ENABLED(CONFIG_DRM_MSM) && IS_ENABLED(CONFIG_FB)
#if IS_ENABLED(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	...
	if (d->lpwg_abs.enable) {
		...
		dlkm_sw82908_lpwg_abs_enable(d->dev, d->lpwg_abs.enable);
	}
	...
	case TOUCH_MINIOS_MFTS_CURVED:
		if (!ts->mfts_lpwg) {
			dlkm_sw82908_power(dev, POWER_ON);
			touch_msleep(ts->caps.hw_reset_delay);
			touch_interrupt_control(dev, INTERRUPT_ENABLE);
			dlkm_sw82908_ic_info(dev);
			/* TBD */
			/*
			if (sw82908_upgrade(dev) == 0) {
				sw82908_power(dev, POWER_OFF);
				sw82908_power(dev, POWER_ON);
				touch_msleep(ts->caps.hw_reset_delay);
			}
			*/
		}
	...
	case TOUCH_MINIOS_MFTS_DS_FLAT:
		if (!ts->mfts_lpwg) {
			...
			touch_msleep(ts->caps.hw_reset_delay);
			touch_interrupt_control(dev, INTERRUPT_ENABLE);
			dlkm_sw82908_init(dev);
			dlkm_sw82908_upgrade(dev);
	...
	if (param->dfc_resume_power_ctl == R_POWER_CTL) {
		...
		dlkm_sw82908_reset_ctrl(dev, HW_RESET_SYNC);
		/* "-EPERM" don't "init"because "sw82908_reset_ctrl" Performing "init" */
		ret = -EPERM;
	}
...
int dlkm_sw82908_ic_test_unit(struct device *dev, u32 data)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	struct project_param *param = &d->p_param;
	u32 data_rd;
	...
	ret = dlkm_sw82908_write_value(dev, BUS_TEST_REG, data);
	if (ret < 0) {
		TOUCH_E("ic test wr err, %08Xh, %d\n", data, ret);
		goto out;
	}

	ret = dlkm_sw82908_read_value(dev, BUS_TEST_REG, &data_rd);
	if (ret < 0) {
...
static int dlkm_sw82908_ic_test(struct device *dev)
{
	u32 data[] = {
		0x5A5A5A5A,
		0xA5A5A5A5,
	...
	for (i = 0; i < ARRAY_SIZE(data); i++) {
		ret = dlkm_sw82908_ic_test_unit(dev, data[i]);
...
/*
	if (d->p_param.dynamic_reg) {
		ret = sw82908_chip_info_load(dev);
		if (ret < 0) {
			TOUCH_E("IC_Register map load fail\n");
			touch_interrupt_control(dev, INTERRUPT_DISABLE);
			sw82908_power(dev, POWER_OFF);
...
*/
	...
	/* sw82908 special register should be controlled */
	if (ts->bus_type == HWIF_I2C) {
		ret = dlkm_sw82908_reg_write(dev, SERIAL_SPI_EN, &rdata, sizeof(rdata));
		if (ret)
			TOUCH_E("failed to write \'serial_spi_disable\', ret:%d\n", ret);
		else
			TOUCH_I("serial_spi_en : %d", rdata);
	} else if (ts->bus_type == HWIF_SPI) {
		ret = dlkm_sw82908_reg_write(dev, SERIAL_I2C_EN, &rdata, sizeof(rdata));
		if (ret)
	...
	ret = dlkm_sw82908_reg_write(dev, TC_CMD +
	...
	dlkm_sw82908_setup_q_sensitivity(dev, d->q_sensitivity);
	dlkm_sw82908_longpress_enable(dev, d->lpwg_longpress.enable);

	dlkm_sw82908_rotation(dev);
	dlkm_sw82908_grip_area(dev);
...
int dlkm_sw82908_check_abnormal_signal(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
...
int dlkm_sw82908_check_status(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0;
	int i = 0, idx = 0;
	...
	dlkm_sw82908_check_abnormal_signal(dev);
...
static void dlkm_sw82908_lpwg_abs_filter(struct device *dev, u8 touch_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
...
int dlkm_sw82908_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_touch_info *info = &d->info;
	struct sw82908_touch_data *data = info->data;
...
#if IS_ENABLED (CONFIG_LGE_TOUCH_PEN)
	struct sw82908_pen_data *pen = &info->pen;
...
//		ts->intr_status |= TOUCH_IRQ_PEN;
	...
	if (data[0].track_id == PALM_ID) {
		if (data[0].event == TOUCHSTS_DOWN) {
			...
			ret = dlkm_sw82908_reg_read(dev, DEBUG_PALM, debug_palm, sizeof(debug_palm));
			if (ret < 0) {
	...
	for (i = 0; i < touch_count; i++) {
		...
		if (data[i].event == TOUCHSTS_DOWN
			...
			if (d->lpwg_abs.enable)
				dlkm_sw82908_lpwg_abs_filter(dev, tdata->id);
...
int dlkm_sw82908_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_touch_info *info = &d->info;
	...
	return dlkm_sw82908_irq_abs_data(dev);
}

int dlkm_sw82908_irq_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	...
	case IRQ_TYPE_KNOCK_ON:
		dlkm_sw82908_get_tci_data(dev, ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.tap_count);
		ts->intr_status = TOUCH_IRQ_KNOCK;

		if (d->ai_pick.enable) {
			if (dlkm_sw82908_check_ai_pick_event(dev)) {
				ts->intr_status = TOUCH_IRQ_AI_PICK;
				TOUCH_I("%s: send ai_pick event!\n", __func__);
			} else {
				if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_PASSWORD_ONLY) {
	...
	case IRQ_TYPE_KNOCK_CODE:
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode >= LPWG_PASSWORD) {
			/*sw82908_get_tci_data(dev,
					ts->tci.info[TCI_2].tap_count);*/
			ts->intr_status = TOUCH_IRQ_PASSWD;
		}
		break;

	case IRQ_TYPE_LG_PAY_SWIPE_UP: // Pay Up
		TOUCH_I("IRQ_TYPE_LG_PAY_SWIPE_UP\n");
		dlkm_sw82908_get_swipe_data(dev);
		ts->intr_status = TOUCH_IRQ_LG_PAY_SWIPE_UP;
		break;

	case IRQ_TYPE_SWIPE_DOWN:
		TOUCH_I("IRQ_TYPE_SWIPE_DOWN\n");
		dlkm_sw82908_get_swipe_data(dev);
		ts->intr_status = TOUCH_IRQ_SWIPE_DOWN;
		break;

	case IRQ_TYPE_QUICK_TOOL_SWIPE_LEFT: // Quicktool Left
	...
	case IRQ_TYPE_QUICK_TOOL_SWIPE_RIGHT: // Quicktool Right
	...
	case IRQ_TYPE_LG_PAY_SWIPE_LEFT: // Pay Left
		TOUCH_I("IRQ_TYPE_LG_PAY_SWIPE_LEFT\n");
		dlkm_sw82908_get_swipe_data(dev);
		ts->intr_status = TOUCH_IRQ_LG_PAY_SWIPE_LEFT;
		break;

	case IRQ_TYPE_LG_PAY_SWIPE_RIGHT: // Pay Right
	...
	case IRQ_TYPE_LONG_PRESS_DOWN:
		...
		dlkm_sw82908_get_tci_data(dev, 1);
	...
	case IRQ_TYPE_CUSTOM_DEBUG:
		TOUCH_I("LPWG wakeup_type is IRQ_TYPE_CUSTOM_DEBUG\n");
		dlkm_sw82908_lpwg_failreason(dev);
		break;
	...
	case IRQ_TYPE_PEN_WAKEUP_BTN_B:
		dlkm_sw82908_get_tci_data(dev, ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.tap_count);
		TOUCH_I("LPWG wakeup_type is IRQ_TYPE_PEN_WAKEUP_BTN:%d\n", type);
...
int dlkm_sw82908_debug_info(struct device *dev) {
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0, i = 0;
	int count = 0;

	ret = dlkm_sw82908_reg_read(dev, DEBUG_INFO, &d->debug_info, sizeof(d->debug_info));
	if (ret < 0)
		goto error;
...
#define REPORT_PACKET_SIZE			(sizeof(struct sw82908_touch_data))
#define REPORT_PACKET_BASE_COUNT	(1)

static int dlkm_sw82908_irq_read_data(struct device *dev)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	struct sw82908_touch_info *info = &d->info;
	...
	if(is_flex_report) {
		ret = dlkm_sw82908_reg_read(dev, addr, (void *)buf, size);
		if(ret < 0) {
		...
		intr_type = dlkm_sw82908_tc_sts_irq_type(info->tc_status);
...
static int dlkm_sw82908_lpwg_function_info(struct device *dev, int display_area_id)
{
	return 0;
}
		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = value;
		if (dlkm_sw82908_reg_write(dev, reg_addr, &data, sizeof(u32)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (dlkm_sw82908_reg_read(dev, reg_addr, &data, sizeof(u32)) < 0)
...
static ssize_t show_lpwg_failreason(struct device *dev, char *buf)
{
	...
	if (dlkm_sw82908_reg_read(dev, TC_CMD + LPWG_FAILREASON_ON,
...
static ssize_t show_lpwg_abs(struct device *dev, char *buf)
{
	...
	dlkm_sw82908_print_lpwg_abs_info(dev);
...
static ssize_t show_ai_pick(struct device *dev, char *buf)
{
	...
	dlkm_sw82908_print_ai_pick_info(dev);
		...
		size_t count)
{
	...
	dlkm_sw82908_reset_ctrl(dev, value);
...
static ssize_t show_grip_suppression(struct device *dev, char *buf)
{
	...
	mutex_lock(&ts->lock);
	ret = dlkm_sw82908_reg_read(dev, GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = dlkm_sw82908_reg_write(dev, GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
		...
		size_t count)
{
	...
	dlkm_sw82908_setup_q_sensitivity(dev, value);
out:
	mutex_unlock(&ts->lock);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	d->lpwg_longpress.enable = true;
	TOUCH_I("%s : becuase we use ultrasonic fingerprint, we have to set d->lpwg_longpress.enable value(%d) to TRUE\n",__func__, d->lpwg_longpress.enable);
	...
	ret = dlkm_sw82908_longpress_enable(dev, d->lpwg_longpress.enable);
	if (ret < 0)
		...
		const char *buf, size_t count)
{
	...
	ret = dlkm_sw82908_onetap_enable(dev, d->lpwg_onetap.enable);
	if (ret < 0)
...
static ssize_t store_burst_test(struct device *dev, const char *buf,
		size_t count)
{
	struct sw82908_reg_info temp_info;
	u32 temp_buf[11] = {0,};
	u16 index = 0;
	int value = 0;
	...
	if (!value) {
		if (sw82908_reg_read(dev, REG_INFO, &temp_info,
					sizeof(struct sw82908_reg_info)) < 0) {
			TOUCH_E("Reg info Read Error\n");
			return -1;
		}
		...
		for (index = 0; index < 11; index++) {
			sw82908_reg_read(dev, REG_INFO + index, &temp_buf[index], sizeof(u32));
...
static struct attribute *sw82908_attribute_list[] = {
...
static const struct attribute_group sw82908_attribute_group = {
	.attrs = sw82908_attribute_list,
};

static int dlkm_sw82908_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	TOUCH_I("sw82908 sysfs reigster\n");

	ret = sysfs_create_group(&ts->kobj, &sw82908_attribute_group);
	if (ret < 0) {
		TOUCH_E("sw82908 sysfs register failed\n");
		goto error;
	}

	ret = dlkm_sw82908_prd_register_sysfs(dev);
#if IS_ENABLED(__SUPPORT_ABT)
	ret = dlkm_sw82908_abt_register_sysfs(dev);
#endif
	if (ret < 0) {
		TOUCH_E("sw82908 register failed\n");
		goto error;
	}
...
static int dlkm_sw82908_get_cmd_version(struct device *dev, char *buf)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	int offset = 0;
...
static int dlkm_sw82908_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct sw82908_data *d = to_sw49202_data(dev);
	int offset = 0;
...
static int dlkm_sw82908_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int dlkm_sw82908_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = dlkm_sw82908_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = dlkm_sw82908_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
#if IS_ENABLED (CONFIG_LGE_TOUCH_PEN)
static int dlkm_sw82908_stop_pen_sensing(struct device *dev, bool enable)
{
	...
	if (enable) {
		dlkm_sw82908_reg_read(dev, addr, (u8 *)&prev_driving, sizeof(u32));
	...
	dlkm_sw82908_reg_write(dev, addr, &stop_driving, sizeof(stop_driving));
	touch_msleep(param->sys_tc_stop_delay);
#endif

	dlkm_sw82908_reg_write(dev, addr, &curr_driving, sizeof(u32));
...
int dlkm_sw82908_write_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0;
	u32 ctrl = 0;
	...
	switch (cmd) {
		case FP_INTR2:
			TOUCH_I("FP_INTR2 write: %s\n", ts->cmd_data.cmd_buf);
			addr = 0xC00;
			if (sscanf(ts->cmd_data.cmd_buf, "%d", &d->fp_intr2_status) <= 0) {
				ret = -1;
			}
			if (d->fp_intr2_status) {
				ctrl = 0x2;
			} else {
				ctrl = 0x3;
			}
...
int dlkm_sw82908_read_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw49202_data(dev);
	int ret = 0;
	TOUCH_TRACE();
...
static int dlkm_sw82908_common_control(struct device *dev, int rw_mode, char *cmd)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	int cmd_r = -1;
	...
	if (rw_mode == WRITE_MODE) {
		ret = dlkm_sw82908_write_cmd(dev, cmd_r);
	} else if (rw_mode == READ_MODE) {
		ret = dlkm_sw82908_read_cmd(dev, cmd_r);
	} else {
	}
...
static struct touch_driver touch_driver = {
	.probe = dlkm_sw82908_probe,
	.remove = dlkm_sw82908_remove,
	.shutdown = dlkm_sw82908_shutdown,
	.suspend = dlkm_sw82908_suspend,
	.resume = dlkm_sw82908_resume,
	.init = dlkm_sw82908_init,
	.irq_handler = dlkm_sw82908_irq_handler,
	.power = dlkm_sw82908_power,
	.upgrade = dlkm_sw82908_upgrade,
#if IS_ENABLED(CONFIG_LGE_TOUCH_APP_FW_UPGRADE)
	.app_upgrade = dlkm_sw82908_app_upgrade,
#endif
	.esd_recovery = dlkm_sw82908_esd_recovery,
	.swipe_enable = dlkm_sw82908_swipe_enable,
	.lpwg = dlkm_sw82908_lpwg,
	.lpwg_function_info = dlkm_sw82908_lpwg_function_info,
	.notify = dlkm_sw82908_notify,
	.init_pm = dlkm_sw82908_init_pm,
	.register_sysfs = dlkm_sw82908_register_sysfs,
	.set = dlkm_sw82908_set,
	.get = dlkm_sw82908_get,
	.rotation = dlkm_sw82908_rotation,
	.grip_area = dlkm_sw82908_grip_area,
#if IS_ENABLED (CONFIG_LGE_TOUCH_PEN)
	.stop_pen_sensing = dlkm_sw82908_stop_pen_sensing,
#endif
#if IS_ENABLED(CONFIG_LGE_TOUCH_COMMON_CONTROL)
	.common_control = dlkm_sw82908_common_control,
#endif
};

#define MATCH_NAME			"lge,sw82908"
...
*/
	...
	if (rev_no < HW_REV_A) {
		TOUCH_I("%s, sw82908 not found HW_REV : %d\n", __func__, rev_no);
		return 0;
	}
#endif
/*
	...
	if (maker != TYPE_SW82908) {
		TOUCH_I("%s, sw82908 returned\n", __func__);
		return 0;
	}
*/
/*
