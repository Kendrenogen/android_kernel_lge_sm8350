...
static inline u32 dlkm_sw82908_tc_sts_irq_type(int status)
{
	return ((status >> 16) & 0x0F);
}

static inline u32 dlkm_sw82908_tc_sts_running_sts(int status)
{
	return (status & 0x1F);
}
...
static inline struct sw82908_data *to_sw49202_data(struct device *dev)
{
	return (struct sw82908_data *)touch_get_device(to_touch_core(dev));
}

static inline struct sw82908_data *to_sw49202_data_from_kobj(struct kobject *kobj)
{
	return (struct sw82908_data *)container_of(kobj,
			struct sw82908_data, kobj);
}

extern int dlkm_sw82908_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int dlkm_sw82908_reg_write(struct device *dev, u16 addr, void *data, int size);
extern int dlkm_sw82908_ic_info(struct device *dev);
extern int dlkm_sw82908_te_info(struct device *dev, char *buf);
extern int dlkm_sw82908_tc_driving(struct device *dev, int mode);
extern int dlkm_sw82908_irq_abs(struct device *dev);
extern int dlkm_sw82908_irq_abs_data(struct device *dev);
extern int dlkm_sw82908_irq_lpwg(struct device *dev);
extern int dlkm_sw82908_irq_handler(struct device *dev);
extern int dlkm_sw82908_check_status(struct device *dev);
extern int dlkm_sw82908_debug_info(struct device *dev);
extern int dlkm_sw82908_sleep_ctrl(struct device *dev, int new_status);
/*
extern int sw82908_chip_info_load(struct device *dev);
*/
extern int dlkm_sw82908_reset_ctrl(struct device *dev, int ctrl);
...
static inline int dlkm_sw82908_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return dlkm_sw82908_reg_read(dev, addr, value, sizeof(*value));
}

static inline int dlkm_sw82908_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return dlkm_sw82908_reg_write(dev, addr, &value, sizeof(value));
}
