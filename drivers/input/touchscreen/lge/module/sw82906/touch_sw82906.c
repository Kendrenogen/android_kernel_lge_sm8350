...
#if IS_ENABLED(__SUPPORT_ABT)
#include "touch_sw82906_abt.h"
#endif
static void dlkm_sw82906_lcd_mode(struct device *dev, u32 mode);
static int dlkm_sw82906_power(struct device *dev, int ctrl);
static int dlkm_sw82906_init(struct device *dev);
static int dlkm_sw82906_set_display_area_status(struct device *dev, int display_area_id);
static int dlkm_sw82906_set_display_area_split_info(struct device *dev, u8 type_display_area, u8 display_area_id);
static int __used dlkm_sw82906_fw_flash_crc(struct device *dev, u32 *crc_val);
static void dlkm_sw82906_set_lpwg_failreason(struct device *dev, int type, bool enable);
...
int dlkm_sw82906_reg_read(struct device *dev, u16 addr, void *data, int size)
{
	...
	ts->tx_buf[0] = ((size > 4) ? 0x20 : 0x00) | ((addr >> 8) & 0x0f);
...
int dlkm_sw82906_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	struct touch_bus_msg msg = {0, };
...
#if IS_ENABLED(CONFIG_DRM_MSM) && IS_ENABLED(CONFIG_FB)
#if IS_ENABLED(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
static int dlkm_sw82906_drm_notifier_callback(struct notifier_block *self,
...
static int dlkm_sw82906_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
static int dlkm_sw82906_sw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	...
	if (mode == SW_RESET) {
		...
		dlkm_sw82906_write_value(dev, SYS_RST_CTL, 2);
		touch_msleep(20);
		dlkm_sw82906_write_value(dev, SYS_RST_CTL, 0);
	} else if (mode == SW_RESET_CODE_DUMP) {
		/* [system reset] Touch F/W resister reset and do code flash dump */
		dlkm_sw82906_write_value(dev, SYS_RST_CTL, 1);
		touch_msleep(20);
		dlkm_sw82906_write_value(dev, SYS_RST_CTL, 0);
	} else {
...
int dlkm_sw82906_hw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
int dlkm_sw82906_only_hw_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
int dlkm_sw82906_power_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	dlkm_sw82906_power(ts->dev, POWER_OFF);
	dlkm_sw82906_power(ts->dev, POWER_ON);
...
int dlkm_sw82906_reset_ctrl(struct device *dev, int ctrl)
{
	int ret = 0;
	...
	case SW_RESET_CODE_DUMP:
		ret = dlkm_sw82906_sw_reset(dev, ctrl);
		break;

	case HW_RESET_ASYNC:
	case HW_RESET_SYNC:
		ret = dlkm_sw82906_hw_reset(dev, ctrl);
		break;
	case HW_RESET_ONLY:
		ret = dlkm_sw82906_only_hw_reset(dev);
		break;
	case HW_RESET_POWER:
		ret = dlkm_sw82906_power_reset(dev, ctrl);
		break;
	default:
...
static int dlkm_sw82906_power(struct device *dev, int ctrl)
{
	...
	case POWER_HW_RESET_ASYNC:
		TOUCH_I("%s, HW reset\n", __func__);
		dlkm_sw82906_reset_ctrl(dev, HW_RESET_ASYNC);
		break;

	case POWER_SW_RESET:
		dlkm_sw82906_reset_ctrl(dev, SW_RESET);
		break;
	case POWER_SLEEP:
	case POWER_WAKE:
	default:
...
static int dlkm_sw82906_ic_boot_check(struct device *dev, u32 *boot_st)
{
	u32 bootmode = 0;
	u32 rdata_crc = 0;
	u32 rdata_pass = ~0;
	...
	ret = dlkm_sw82906_read_value(dev, GDMA_CRC_RESULT, &rdata_crc);
	err |= (rdata_crc != CRC_FIXED_VALUE);

	ret = dlkm_sw82906_read_value(dev, GDMA_CRC_PASS, &rdata_pass);
	err |= (!rdata_pass)<<1;

	ret = dlkm_sw82906_read_value(dev, INFO_PTR_ADDR, &rdata_ptr);
	err |= (!rdata_ptr)<<2;
...
int dlkm_sw82906_ic_info(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	u32 bootmode = 0;
	...
	ret = dlkm_sw82906_reg_read(dev, CHIP_INFO + tc_version,
			&d->ic_info.version, sizeof(d->ic_info.version));

	ret = dlkm_sw82906_reg_read(dev, CHIP_INFO + tc_product_id1,
			&d->ic_info.product_id, 8);

#if !IS_ENABLED(__SW82906_STILL_BRING_UP)
	ret = dlkm_sw82906_reg_read(dev, PT_INFO + pt_info_channel,
			&d->ic_info.pt_info, sizeof(d->ic_info.pt_info));
#endif	/* __SW82906_STILL_BRING_UP */

	dlkm_sw82906_ic_boot_check(dev, &bootmode);
	...
	if (!bootmode) {
		...
		ret = dlkm_sw82906_fw_flash_crc(dev, &crc_val);
		if (ret < 0) {
...
static void dlkm_sw82906_init_udf_longpress_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_set_udf_longpress_active_area(struct device *dev, int display_area_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LONG_PRESS_INDEX_AND_ENABLE, &longpress_enable, sizeof(longpress_enable));
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LONG_PRESS_ACT_AREA_START, active_area, sizeof(active_area));
...
static int dlkm_sw82906_set_udf_longpress(struct device *dev, int display_area_id, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 longpress_enable = 0;
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LONG_PRESS_SLOPE, &longpress_data[0], sizeof(longpress_data));
...
static void dlkm_sw82906_init_show_aod_onetap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_set_show_aod_onetap_active_area(
		struct device *dev, int display_area_id, struct active_area *area_data, bool use_senseless_area)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + ONE_TAP_INDEX_AND_ENABLE, &onetap_enable, sizeof(onetap_enable));

	show_aod_onetap_area->x1 = area_data->x1 + (use_senseless_area ? SENSELESS_PIXEL : 0); // LEFT_SIDE
	show_aod_onetap_area->y1 = area_data->y1; // UP_SIDE // Do not set senseless area on upside edge.
	show_aod_onetap_area->x2 = area_data->x2 - (use_senseless_area ? SENSELESS_PIXEL : 0); // RIGHT_SIDE
	show_aod_onetap_area->y2 = area_data->y2 - (use_senseless_area ? SENSELESS_PIXEL : 0); // DOWN_SIDE
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + ONE_TAP_ACTIVE_AREA_START, active_area, sizeof(active_area));
...
static int dlkm_sw82906_set_show_aod_onetap(struct device *dev, int display_area_id, int function, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct onetap_ctrl *onetap = NULL;
	u32 onetap_enable = 0;
	u32 onetap_data[3] = {0, };
	int ret = 0;
	...
	switch (function) {
		case LPWG_SHOW_AOD:
			onetap = &ts->display_area_status[display_area_id].show_aod_onetap;
			onetap->enable = enable;
			break;
		default:
			TOUCH_E("%s: Not support function[%d]\n", __func__, function);
			break;
	}
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + ONE_TAP_TOUCH_SLOPE, &onetap_data[0], sizeof(onetap_data));

	dlkm_sw82906_set_show_aod_onetap_active_area(dev, display_area_id, &ts->display_area_status[display_area_id].show_aod_onetap.area, false);

	TOUCH_I("%s: DISPLAY_AREA_ID[%d] FUNCTION[%d], onetap->enable=%d, enable_data=0x%x\n",
			__func__, display_area_id,
			function,
			onetap->enable,
			onetap_enable);
...
static void dlkm_sw82906_init_knock_on_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_get_coordinate(struct device *dev, int count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	u8 i = 0;
...
static int dlkm_sw82906_set_knock_on_active_area(
		struct device *dev, int display_area_id, struct active_area *area_data, bool use_senseless_area)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + KNOCK_ON_INDEX_AND_ENABLE, &knock_on_enable, sizeof(knock_on_enable));

	area->x1 = area_data->x1 + (use_senseless_area ? SENSELESS_PIXEL : 0); // LEFT_SIDE
	area->y1 = area_data->y1; // UP_SIDE // Do not set senseless area on upside edge.
	area->x2 = area_data->x2 - (use_senseless_area ? SENSELESS_PIXEL : 0); // RIGHT_SIDE
	area->y2 = area_data->y2 - (use_senseless_area ? SENSELESS_PIXEL : 0); // DOWN_SIDE
	...
	if (area->y2 > ts->flexible_display.area[display_area_id].area.right_bottom.y)
		area->y2 = ts->flexible_display.area[display_area_id].area.right_bottom.y;
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + KNOCK_ON_ACTIVE_AREA_X1, area, sizeof(struct active_area));
...
static int dlkm_sw82906_set_knock_on(struct device *dev, int display_area_id, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + KNOCK_ON_INDEX_AND_ENABLE, &knock_on_buf[0], sizeof(knock_on_buf));

	dlkm_sw82906_set_knock_on_active_area(dev, display_area_id, &ts->display_area_status[display_area_id].knock_on.area, false);
	...
	dlkm_sw82906_set_lpwg_failreason(dev, LPWG_DEBUG_KNOCK_ON_FRONT, true);
	dlkm_sw82906_set_lpwg_failreason(dev, LPWG_DEBUG_KNOCK_ON_REAR, true);

	return ret;
}

static void dlkm_sw82906_init_ai_pick_double_tap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void dlkm_sw82906_set_ai_pick_double_tap(struct device *dev, int display_area_id, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (enable) {
		if (ts->display_area_status[display_area_id].lpwg.mode == LPWG_NONE) {
			...
			dlkm_sw82906_set_knock_on(dev, display_area_id, LPWG_DOUBLE_TAP);
			dlkm_sw82906_set_knock_on_active_area(dev, display_area_id,
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE) {
			...
			dlkm_sw82906_set_knock_on(dev, display_area_id, LPWG_NONE);
		} else {
...
static bool dlkm_sw82906_check_ai_pick_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_get_swipe_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	u32 rdata[3];
	...
	ts->lpwg_coord.code[0].x = rdata[1] & 0xffff;
	ts->lpwg_coord.code[0].x = rdata[1] >> 16;

	ts->lpwg_coord.code[1].x = -1;
	ts->lpwg_coord.code[1].y = -1;

	return 0;
}

static int dlkm_sw82906_set_lg_pay_swipe(struct device *dev, int display_area_id, int gesture, bool enable) {
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	TOUCH_D(LPWG, "%s - display_area_id[%d], function[%s], gesture[%s], enable[%d]\n",
            __func__, display_area_id,
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE_INDEX_AND_ENABLE, &swipe_buf[0], sizeof(u32) * 7);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE_WRONG_DIRECTION_THD, &swipe_buf[7], sizeof(u32) * 3);
...
static int dlkm_sw82906_set_quick_tool_swipe(struct device *dev, int display_area_id, int gesture, bool enable) {
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE2_INDEX_AND_ENABLE, &swipe_buf[0], sizeof(u32) * 5);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE2_WRONG_DIRECTION_THD, &swipe_buf[5], sizeof(u32) * 3);
...
static int dlkm_sw82906_set_lg_pay_swipe_active_area(
		struct device *dev, int display_area_id, int gesture, struct active_area *area_data)
{
	...
	ret = dlkm_sw82906_reg_read(dev, ABT_CMD + SWIPE_INDEX_AND_ENABLE, &swipe_enable, sizeof(swipe_enable));
	swipe_enable = (swipe_enable & ~(clear_16bit << 16)) | (display_area_id << 16);
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE_INDEX_AND_ENABLE, &swipe_enable, sizeof(swipe_enable));

	// Save last swipe active area
	swipe_active_area->x1 = ts->flexible_display.area[i].area.left_top.x;
	swipe_active_area->y1 = ts->flexible_display.area[i].area.left_top.y;
	swipe_active_area->x2 = ts->flexible_display.area[i].area.right_bottom.x;
	swipe_active_area->y2 = ts->flexible_display.area[i].area.right_bottom.y;
	if (swipe_active_area->x1 < 0)
		swipe_active_area->x1 = 0;
	if (swipe_active_area->y1 < 0)
		swipe_active_area->y1 = 0;
	if (swipe_active_area->x2 > ts->flexible_display.area[i].area.right_bottom.x)
		swipe_active_area->x2 = ts->flexible_display.area[i].area.right_bottom.x;
	if (swipe_active_area->y2 > ts->flexible_display.area[i].area.right_bottom.y)
		swipe_active_area->y2 = ts->flexible_display.area[i].area.right_bottom.y;

	// Update swipe active area register
	ret = dlkm_sw82906_reg_read(dev, addr_active_area, &swipe_buf_active_area[0], sizeof(swipe_buf_active_area));
	...
	ret = dlkm_sw82906_reg_write(dev, addr_active_area, &swipe_buf_active_area[0], sizeof(swipe_buf_active_area));
	...
	ret = dlkm_sw82906_reg_read(dev, addr_start_area, &swipe_buf_start_area[0], sizeof(swipe_buf_start_area));
	...
	ret = dlkm_sw82906_reg_write(dev, addr_start_area, &swipe_buf_start_area[0], sizeof(swipe_buf_start_area));
	...
	TOUCH_I("%s - display_area_id[%d] swipe gesture[%s]: left_top.x,y(%d, %d), right_bottom.x,y(%d, %d)\n", __func__,
			...
			area_data->x1, area_data->y1, area_data->x2, area_data->y2);

	return ret;
}

static int dlkm_sw82906_set_quick_tool_swipe_active_area(
		struct device *dev, int display_area_id, int gesture, struct active_area *area_data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct active_area *swipe_active_area = NULL, *swipe_start_area = NULL;
	...
	ret = dlkm_sw82906_reg_read(dev, ABT_CMD + SWIPE2_INDEX_AND_ENABLE, &swipe_enable, sizeof(swipe_enable));
	swipe_enable = (swipe_enable & ~(clear_16bit << 16)) | (display_area_id << 16);
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SWIPE2_INDEX_AND_ENABLE, &swipe_enable, sizeof(swipe_enable));
...
static void dlkm_sw82906_init_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.x1 = 879;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.x2 = 200;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].area.x1 = 72;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].area.x2 = 1007;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.x1 = 342;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.y1 = 2228;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.x2 = 737;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.x1 = 72;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.x2 = 1007;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.x1 = 342;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.y1 = 2228;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.x2 = 737;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.x1 = 879;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.x2 = 200;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.y2 = 0;
	...
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].distance = 15;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].ratio_thres = 58;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.x1 = 879;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_LEFT].start_border_area.y2 = 0;

	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].distance = 15;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].ratio_thres = 58;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.x2 = 200;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_RIGHT].start_border_area.y2 = 0;

	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].distance = 20;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].ratio_thres = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].area.x1 = 72;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].area.x2 = 1007;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.x1 = 342;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.y1 = 2228;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.x2 = 737;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_UP].start_border_area.y2 = 0;

	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].distance = 20;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].ratio_thres = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.x1 = 72;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.x2 = 1007;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.x1 = 342;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.y1 = 2228;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.x2 = 737;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].lg_pay_swipe[LG_PAY_SWIPE_DOWN].start_border_area.y2 = 0;

	/* Display Rear (Quick Tool) 'L, R' */
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].distance = 15;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].ratio_thres = 58;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.x1 = 879;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_border_area.x1 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_border_area.y1 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_border_area.x2 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_LEFT].start_border_area.y2 = 200;

	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].distance = 15;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].ratio_thres = 58;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].min_time = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].max_time = 150;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].wrong_dir_thres = 5;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].init_ratio_chk_dist = 4;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].init_ratio_thres = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.x2 = 1079;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].area.y2 = 2459;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.y1 = 306;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.x2 = 200;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_area.y2 = 1662;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.x1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.y1 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.x2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].border_area.y2 = 0;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_border_area.x1 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_border_area.y1 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_border_area.x2 = 100;
	ts->display_area_status[DISPLAY_AREA_ID_1].quick_tool_swipe[QUICK_TOOL_SWIPE_RIGHT].start_border_area.y2 = 200;
}

static void dlkm_sw82906_init_display_area_activation_info(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	d->display_area_split_info[DISPLAY_AREA_ID_1].area_activation = SPLIT_DISPLAY_AREA_ACTIVATION_ENABLE;
	...
	case TOUCH_MINIOS_AAT:
		d->display_area_split_info[DISPLAY_AREA_ID_0].resolustion = 1080;
		d->display_area_split_info[DISPLAY_AREA_ID_1].resolustion = 1876;
	...
	case TOUCH_MINIOS_MFTS_DS_FLAT:
		d->display_area_split_info[DISPLAY_AREA_ID_0].resolustion = 1876;
		d->display_area_split_info[DISPLAY_AREA_ID_1].resolustion = 1876;
		d->display_area_interspace_split_info[DISPLAY_AREA_INTERSPACE_0_1].resolustion = 1876;
		break;
...
static void dlkm_sw82906_init_quick_tool_abs_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_set_quick_tool_abs_active_area(struct device *dev, int display_area_id, struct active_area *area_data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LPWG_ABS_INDEX_AND_ENABLE, &abs_enable, sizeof(abs_enable));
	...
	active_area[0] = (area->y1 << 16) | area->x1;
	active_area[1] = (area->y2 << 16) | area->x2;
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LPWG_ABS_ACTIE_AREA_START, active_area, sizeof(active_area));
...
static int dlkm_sw82906_set_quick_tool_abs(struct device *dev, int display_area_id, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 abs_enable = 0;
	int ret = 0;
...
static int dlkm_sw82906_swipe_enable(struct device *dev, bool enable)
{
	return 0;
}

static void dlkm_sw82906_print_lpwg_failreason(struct device *dev, int type, int count)
{
	u64 buf = 0;
	u8 data = 0;
	u8 fail_num = 0;
	...
	case LPWG_DEBUG_KNOCK_ON_FRONT:
		ret = dlkm_sw82906_reg_read(dev, ABT_CMD + KNOCK_ON_FRONT_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_KNOCK_ON_NUM;
		break;
	case LPWG_DEBUG_KNOCK_ON_REAR:
		ret = dlkm_sw82906_reg_read(dev, ABT_CMD + KNOCK_ON_REAR_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_KNOCK_ON_NUM;
		break;
	case LPWG_DEBUG_SWIPE:
		ret = dlkm_sw82906_reg_read(dev, ABT_CMD + SWIPE_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_SWIPE_NUM;
		break;
	case LPWG_DEBUG_LONGPRESS:
		ret = dlkm_sw82906_reg_read(dev, ABT_CMD + LONGPRESS_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_LONGPRESS_NUM;
		break;
	case LPWG_DEBUG_ONETAP:
		ret = dlkm_sw82906_reg_read(dev, ABT_CMD + ONETAP_FAILREASON_BUF,
				&buf, sizeof(buf));
...
static void dlkm_sw82906_get_lpwg_failreason(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret, i = 0;
	u32 rdata[2] = {0, };
	...
	ret = dlkm_sw82906_reg_read(dev, ABT_CMD + LPWG_FAILREASON_COUNT1,
			&rdata[0], sizeof(u32));
	ret = dlkm_sw82906_reg_read(dev, ABT_CMD + LPWG_FAILREASON_COUNT2,
			&rdata[1], sizeof(u32));
	...
	if (count[LPWG_DEBUG_KNOCK_ON_FRONT]) { /* Knock-on front */
		dlkm_sw82906_print_lpwg_failreason(dev, LPWG_DEBUG_KNOCK_ON_FRONT,
				count[LPWG_DEBUG_KNOCK_ON_FRONT]);
	}

	if (count[LPWG_DEBUG_KNOCK_ON_REAR]) { /* Knock-on rear */
		dlkm_sw82906_print_lpwg_failreason(dev, LPWG_DEBUG_KNOCK_ON_REAR,
				count[LPWG_DEBUG_KNOCK_ON_REAR]);
	}

	if (count[LPWG_DEBUG_SWIPE]) { /* Swipe */
		dlkm_sw82906_print_lpwg_failreason(dev, LPWG_DEBUG_SWIPE,
				count[LPWG_DEBUG_SWIPE]);
	}

	if (count[LPWG_DEBUG_LONGPRESS]) { /* LongPress */
		dlkm_sw82906_print_lpwg_failreason(dev, LPWG_DEBUG_LONGPRESS,
				count[LPWG_DEBUG_LONGPRESS]);
	}

	if (count[LPWG_DEBUG_ONETAP]) { /* LongPress */
		dlkm_sw82906_print_lpwg_failreason(dev, LPWG_DEBUG_ONETAP,
...
static void dlkm_sw82906_set_lpwg_failreason(struct device *dev, int type, bool enable)
{
	...
	dlkm_sw82906_reg_write(dev, addr, &data, sizeof(data));
}
...
int dlkm_sw82906_tc_driving(struct device *dev, int new_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	struct project_param *param = &d->p_param;
	...
	if (new_status != LCD_MODE_STOP) {
		ctrl = 0x04;
		dlkm_sw82906_reg_write(dev, addr, &ctrl, sizeof(ctrl));
	...
	dlkm_sw82906_reg_write(dev, addr,
			&ctrl, sizeof(ctrl));
	...
	if (param->reg_debug) {
		if (new_status != LCD_MODE_STOP) {
			for (i = 0; i < 10; i++) {
				dlkm_sw82906_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
				TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);

				dlkm_sw82906_reg_read(dev, TC_STS, (u8 *)&rdata, sizeof(u32));
				TOUCH_I("read tc_status(%x) = %x\n", TC_STS, rdata);
				if ((rdata & 0x1F) == 0x7)
					return 0;
				touch_msleep(50);
...
static int dlkm_sw82906_release_pen_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_TRACE();
	ts->pdata.in_range = 0;
...
static int dlkm_sw82906_setup_q_sensitivity(struct device *dev, int enable)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;;
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + COVER_SENSITIVITY,
...
int dlkm_sw82906_sleep_ctrl(struct device *dev, int new_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_set_display_area_moving(struct device *dev, bool moving) {
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	u32 data = 0;
	...
	ret = dlkm_sw82906_reg_write(dev, SPLIT_DISPLAY_AREA_MOVING, &data, sizeof(u32));
	if (ret < 0) {
...
static int dlkm_sw82906_set_display_area_split_info(struct device *dev,
	...
	ret = dlkm_sw82906_reg_write(dev, addr, &data, sizeof(u32));
	if (ret < 0) {
	...
	if (param->reg_debug) {
		ret = dlkm_sw82906_reg_read(dev, addr, &data, sizeof(u32));
		if (ret < 0) {
...
static int dlkm_sw82906_set_display_area_status(struct device *dev, int display_area_id)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen == SCREEN_OFF &&
			...
			if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE &&
				...
				dlkm_sw82906_sleep_ctrl(dev, IC_DEEP_SLEEP);
				return ret;
				...
				TOUCH_I("%s: [GLOBAL_SET] Both of display_area_id 0 & 1, PROX_NEAR - IC_DEEP_SLEEP\n", __func__);
				dlkm_sw82906_sleep_ctrl(dev, IC_DEEP_SLEEP);
				return ret;
			} else {
				TOUCH_I("%s: [GLOBAL_SET] Both of display_area_id 0 & 1, SCREEN_OFF - TC_DRIVNG U0\n", __func__);
				if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
					dlkm_sw82906_sleep_ctrl(dev, IC_NORMAL);
				}
				dlkm_sw82906_tc_driving(dev, LCD_MODE_U0);
			}
		} else {
			...
			dlkm_sw82906_get_lpwg_failreason(dev);
			return ret;
		}
	} else {
		/* RESUME */
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen == SCREEN_OFF &&
			...
			dlkm_sw82906_tc_driving(dev, LCD_MODE_U3);
		}
	}
	...
	if (ts->display_area_status[display_area_id].lpwg.screen == SCREEN_OFF ||
		...
		if (ts->display_area_status[display_area_id].lpwg.mode == LPWG_NONE &&
			...
			dlkm_sw82906_set_knock_on(dev, display_area_id, ts->display_area_status[display_area_id].lpwg.mode);
			dlkm_sw82906_set_show_aod_onetap(dev, display_area_id, LPWG_SHOW_AOD, ts->display_area_status[display_area_id].show_aod_onetap.enable);
		}
	...
	dlkm_sw82906_set_display_area_split_info(dev, TYPE_DISPLAY_AREA, display_area_id);

	return ret;
}

static int dlkm_sw82906_display_area_status(struct device *dev, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
	...
	TOUCH_I("DISPLAY_AREA_STATUS_UPDATE_ALL : display_area_id[%d], screen[%s], mode[%s], sensor[%s], qcover[%s]\n",
			...
			ts->display_area_status[display_area_id].lpwg.qcover == 1 ? "NEAR" : "FAR");

	ret = dlkm_sw82906_set_display_area_status(dev, display_area_id);
	if (ret)
...
static int dlkm_sw82906_set_lpwg_function_info(struct device *dev, int display_area_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	...
	if (enable || d->display_area_split_info[display_area_id].area_activation == SPLIT_DISPLAY_AREA_ACTIVATION_DISABLE) {
		ret = dlkm_sw82906_set_display_area_status(dev, display_area_id);
		if (ret)
			TOUCH_E("failed to set display area status, ret:%d", ret);
	}
	...
	switch (function) {
		case LPWG_LG_PAY:
			switch (gesture) {
				...
				case SWIPE_RIGHT:
				case SWIPE_LEFT:
					dlkm_sw82906_set_lg_pay_swipe(dev, display_area_id, gesture, enable);
					dlkm_sw82906_set_lg_pay_swipe_active_area(dev, display_area_id, gesture, &area);
					break;
				default:
					goto error;
					break;
		...
		case LPWG_QUICK_TOOL:
			switch (gesture) {
				case SWIPE_LEFT:
					dlkm_sw82906_set_quick_tool_swipe(dev, display_area_id, gesture, enable);
					dlkm_sw82906_set_quick_tool_swipe_active_area(dev, display_area_id, gesture, &area);
					break;
				case SWIPE_RIGHT:
					dlkm_sw82906_set_quick_tool_swipe(dev, display_area_id, gesture, enable);
					dlkm_sw82906_set_quick_tool_swipe_active_area(dev, display_area_id, gesture, &area);
					break;
				case ABS_COORDINATE:
					dlkm_sw82906_set_quick_tool_abs(dev, display_area_id, enable);
					dlkm_sw82906_set_quick_tool_abs_active_area(dev, display_area_id, &area);
					break;
				default:
					goto error;
					break;
		...
		case LPWG_AI_PICK:
			switch (gesture) {
				case DOUBLE_TAP:
					dlkm_sw82906_set_ai_pick_double_tap(dev, display_area_id, enable);
					break;
				default:
					goto error;
					break;
		...
		case LPWG_UDF:
			switch (gesture) {
				case LONG_PRESS:
					dlkm_sw82906_set_udf_longpress(dev, display_area_id, enable);
					dlkm_sw82906_set_udf_longpress_active_area(dev, display_area_id);
					break;
				case ONE_TAP:
					dlkm_sw82906_set_show_aod_onetap(dev, display_area_id, function, enable);
					break;
				default:
					goto error;
					break;
...
static int dlkm_sw82906_lpwg_function_info(struct device *dev, int display_area_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = dlkm_sw82906_set_lpwg_function_info(dev, display_area_id);
	if (ret)
...
static void dlkm_sw82906_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	dlkm_sw82906_reg_write(dev, ABT_CMD + SPECIAL_CHARGER_INFO,
...
static int dlkm_sw82906_check_mode(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
...
#if IS_ENABLED(__SUPPORT_NOTIFY_LCD_EVENT_REG)
static void dlkm_sw82906_lcd_event_read_reg(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	...
	dlkm_sw82906_read_value(dev, TC_IC_STATUS, &rdata[0]);

	dlkm_sw82906_read_value(dev, TC_STS, &rdata[1]);

	dlkm_sw82906_read_value(dev, version_addr, &rdata[2]);

	dlkm_sw82906_read_value(dev, SPR_CHIP_ID, &rdata[3]);
...
static int dlkm_sw82906_temperature_status(struct device *dev, int new_thermal_noti)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
#endif
	if (batt_temp < 10 || (d->thermal_noti | new_thermal_noti)) {
		if (batt_temp < 0) {
		...
		if (new_thermal_noti == 1)
			wdata |= 1 << 31;
		ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SPECIAL_TEMPERATURE_INFO,
							&wdata, sizeof(u32));
		TOUCH_I("origin noti:%d, batt:%d, thermal=%s, w_val:0x%x, cam_therm_usr:%d\n",
				atomic_read(&ts->state.temperature), batt_temp,
				new_thermal_noti? "enable":"disable", wdata, cam_therm_usr);
	} else {
		TOUCH_I("batt_temp setting skip. noti:%d, batt:%d, thermal=%s, cam_therm_usr:%d\n",
				atomic_read(&ts->state.temperature), batt_temp,
				new_thermal_noti? "enable":"disable", cam_therm_usr);
	}

	return ret;
}

static int dlkm_sw82906_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_TRACE();
	TOUCH_I("TA Type: %d\n", atomic_read(&ts->state.connect));
	dlkm_sw82906_connect(dev);
	return 0;
}

static int dlkm_sw82906_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int dlkm_sw82906_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if IS_ENABLED(__SUPPORT_NOTIFY_DEBUG_OPTION)
static int dlkm_sw82906_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	//	ts->driver->irq_handler = sw82906_sic_abt_irq_handler;
		...
		ts->driver->irq_handler = dlkm_sw82906_irq_handler;
	}

	return 0;
}

static int dlkm_sw82906_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
...
static void dlkm_sw82906_fb_notify_work_func(struct work_struct *fb_notify_work)
{
...
static int dlkm_sw82906_rotation(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + ROTATION_ANGLE,
...
static int dlkm_sw82906_grip_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u32 buf = 0;
	...
	buf = 1;
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_ENABLE,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_START,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + GRIP_EXCEPTION_END,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + PORTRAIT_VERTICAL_A_B_WIDTH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + PORTRAIT_VERTICAL_C_WIDTH_HEIGHT,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LANDSCAPE_VERTICAL_A_B_WIDTH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LANDSCAPE_VERTICAL_C_WIDTH_HEIGH,
							&buf, sizeof(buf));
	if (ret < 0)
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + LANDSCAPE_HORIZONTAL_A_B_HEIGHT,
							&buf, sizeof(buf));
	if (ret < 0)
...
static int dlkm_sw82906_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	/*
	...
	*/
	...
	case LCD_EVENT_LCD_MODE:
		...
		dlkm_sw82906_lcd_mode(dev, *(u32 *)data);
		ret = dlkm_sw82906_check_mode(dev);
...
#if IS_ENABLED(__SUPPORT_NOTIFY_LCD_EVENT_REG)
		dlkm_sw82906_lcd_event_read_reg(dev);
#endif
		break;
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = dlkm_sw82906_usb_status(dev, *(u32 *)data);
		break;
	case NOTIFY_WIRELESS:
		TOUCH_I("NOTIFY_WIRELEES!\n");
		ret = dlkm_sw82906_wireless_status(dev, *(u32 *)data);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = dlkm_sw82906_earjack_status(dev, *(u32 *)data);
		break;
	case NOTIFY_IME_STATE:
		TOUCH_I("NOTIFY_IME_STATE!\n");
		ret = dlkm_sw82906_reg_write(dev, ABT_CMD +
				SPECIAL_IME_STATUS, (u32 *)data, sizeof(u32));
		break;
	case NOTIFY_CALL_STATE:
		TOUCH_I("NOTIFY_CALL_STATE!\n");
#if IS_ENABLED(__SUPPORT_NOTIFY_CALL)
		ret = dlkm_sw82906_reg_write(dev, ABT_CMD +
	...
	case NOTIFY_TEMPERATURE:
		...
		dlkm_sw82906_temperature_status(dev, d->thermal_noti);
		break;
	default:
...
static int dlkm_sw82906_init_pm(struct device *dev)
{
#if IS_ENABLED(CONFIG_DRM_MSM) && IS_ENABLED(CONFIG_FB)
#if IS_ENABLED(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: drm_notif change\n", __func__);
	ts->drm_notif.notifier_call = dlkm_sw82906_drm_notifier_callback;
#endif
#elif IS_ENABLED(CONFIG_FB)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: fb_notif change\n", __func__);
	ts->fb_notif.notifier_call = dlkm_sw82906_fb_notifier_callback;
#endif

	return 0;
}

static int dlkm_sw82906_init_works(struct sw82906_data *d)
{
	...
	INIT_DELAYED_WORK(&d->fb_notify_work, dlkm_sw82906_fb_notify_work_func);

	return 0;
}

static void dlkm_sw82906_init_locks(struct sw82906_data *d)
{
	mutex_init(&d->io_lock);
}

static int dlkm_sw82906_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	dlkm_sw82906_init_works(d);
	dlkm_sw82906_init_locks(d);
	...
	dlkm_sw82906_init_knock_on_info(dev);
	dlkm_sw82906_init_swipe_info(dev);
	dlkm_sw82906_init_quick_tool_abs_info(dev);
	dlkm_sw82906_init_ai_pick_double_tap_info(dev);
	dlkm_sw82906_init_udf_longpress_info(dev);
	dlkm_sw82906_init_show_aod_onetap_info(dev);
	dlkm_sw82906_init_display_area_activation_info(dev);
...
static int dlkm_sw82906_remove(struct device *dev)
{
	TOUCH_TRACE();
...
static int dlkm_sw82906_shutdown(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
...
static int dlkm_sw82906_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
static int dlkm_sw82906_condition_wait(struct device *dev,
		...
		u32 mask, u32 delay, u32 retry, int not_cond)
{
	...
	do {
		touch_msleep(delay);
		ret = dlkm_sw82906_read_value(dev, addr, &data);
		if (ret >= 0) {
...
static int dlkm_sw82906_fw_verify_cfg(char *buf)
{
...
static int dlkm_sw82906_fw_verify_s_cfg(char *buf, int index)
{
	struct cfg_head *head = (struct cfg_head *)buf;
...
static int dlkm_sw82906_fw_binary_verify(struct device *dev, u8 *fw_buf, int fw_size)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int flash_fw_size = d->p_param.flash_fw_size;
	...
	ret = dlkm_sw82906_fw_verify_cfg((char *)head);
	if (ret < 0) {
	...
	for (i = 0; i < s_cfg_cnt; i++) {
		ret = dlkm_sw82906_fw_verify_s_cfg((char *)s_cfg_base, i);
		if (ret < 0) {
...
int dlkm_sw82906_chip_info_load(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);

    u32 read_val = 0;

	if (dlkm_sw82906_reg_read(dev, ENGINE_BUF_ADDR,
	...
	if (dlkm_sw82906_reg_read(dev, SYS_BUF_ADDR,
			(u8 *)&read_val, sizeof(u32)) < 0) {
...
static int dlkm_sw82906_flash_erase_check(struct device *dev)
{
	...
	dlkm_sw82906_reg_write(dev, spr_flash_offset, &start_offset, sizeof(start_offset));
	TOUCH_I("flash read offset(start_offset) %x \n", start_offset);	//for debug

	dlkm_sw82906_reg_read(dev, flash_access_addr, &flash_read_val, sizeof(flash_read_val));
	...
	dlkm_sw82906_reg_write(dev, spr_flash_offset, &end_offset, sizeof(end_offset));
...
static int __used dlkm_sw82906_fw_flash_crc(struct device *dev, u32 *crc_val)
{
	...
	ret = dlkm_sw82906_write_value(dev, gdma_saddr, 0);
	if (ret < 0) {
	...
	ret = dlkm_sw82906_write_value(dev, gdma_ctl, ctrl_data);
	if (ret < 0) {
	...
	ret = dlkm_sw82906_write_value(dev, gdma_start, 1);
	if (ret < 0) {
	...
	ret = dlkm_sw82906_read_value(dev, gdma_crc_result, &data);
	if (ret < 0) {
	...
	ret = dlkm_sw82906_read_value(dev, gdma_crc_pass, &data);
	if (ret < 0) {
...
static int __used dlkm_sw82906_fw_flash_mass_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	ret = dlkm_sw82906_write_value(dev, fc_addr, 0);
	if (ret < 0) {
		fc_err = 1;
		goto out;
	}

	ret = dlkm_sw82906_write_value(dev, fc_ctrl, FC_CTRL_MASS_ERASE);
	if (ret < 0) {
		fc_err = 2;
		goto out;
	}

	dlkm_sw82906_reg_read(dev, fc_ctrl, &read_reg, sizeof(read_reg));
	TOUCH_I("fc_ctrl erase command read_val : %x\n", read_reg);

	ret = dlkm_sw82906_write_value(dev, fc_stat, 1);
	if (ret < 0) {
		fc_err = 3;
		goto out;
	}
	...
	chk_resp = 1;
	ret = dlkm_sw82906_condition_wait(dev, spi_flash_status, &data,
	...
	dlkm_sw82906_reg_read(dev, SYS_RST_CTL, &read_reg, sizeof(read_reg));
	TOUCH_I("1st cm3 status check read_val : %x\n", read_reg);

	//Flash Area check(first,last)
	ret = dlkm_sw82906_flash_erase_check(dev);
	if (ret < 0) {
		fc_err = 5;
	...
	ret = dlkm_sw82906_fw_flash_crc(dev, &erase_crc_val);
	if (ret < 0) {
		fc_err = 6;
...
out:
	...
	dlkm_sw82906_write_value(dev, fc_ctrl, 0);
...
static int __used dlkm_sw82906_fw_flash_page_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	for (i = 0; i < (FLASH_SIZE/FLASH_PAGE_SIZE); i++) {
		ret = dlkm_sw82906_write_value(dev, fc_addr, addr);
		if (ret < 0) {
			fc_err = 1;
			break;
		}

		ret = dlkm_sw82906_write_value(dev, fc_ctrl, FC_CTRL_PAGE_ERASE);
		if (ret < 0) {
			fc_err = 2;
			break;
		}
	...
	dlkm_sw82906_write_value(dev, fc_addr, 0);
	dlkm_sw82906_write_value(dev, fc_ctrl, 0);
...
static int __used dlkm_sw82906_fw_flash_write(struct device *dev, int addr, u8 *dn_buf, int dn_size)
{
	...
	ret = dlkm_sw82906_write_value(dev, fc_offset, addr);
	if (ret < 0) {
	...
	ret = dlkm_sw82906_reg_write(dev, fc_code_access, dn_buf, dn_size);
	if (ret < 0) {
...
static int dlkm_sw82906_fw_rst_ctl(struct device *dev, int val, const char *str)
{
	...
	ret = dlkm_sw82906_write_value(dev, rst_ctl, val);
	if (ret < 0) {
...
static int dlkm_sw82906_fw_upgrade_code(struct device *dev, u8 *dn_buf, int dn_size)
{
//	struct sw82906_data *d = to_sw82906_data(dev);
//	struct project_param *param = &d->p_param;
	u32 fc_ctrl = FC_CTRL;
...
//	sw82906_write_value(dev, FLASH_TPROG, 560);
	...
	dlkm_sw82906_write_value(dev, fc_ctrl, FC_CTRL_WR_EN);
	...
	do {
		touch_msleep(10);
		dlkm_sw82906_read_value(dev, BDMA_STS, &reg_buf);
		busycheck_cnt++;
	...
	for (k = 0; k <= write_cnt; k++)
	{
		...
		if(k == write_cnt){
			...
			dlkm_sw82906_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], remain_size);
			reg_buf = remain_size / sizeof(u32);
		}else{
			dlkm_sw82906_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], BDMA_TRANS_SIZE);
			reg_buf = BDMA_TRANS_SIZE / sizeof(u32);
		}
		reg_buf |= BDMA_CTL_BDMA_EN;

		dlkm_sw82906_write_value(dev, BDMA_SADDR, DATASRAM_ADDR);
		dlkm_sw82906_write_value(dev, BDMA_CTL, reg_buf);

		dlkm_sw82906_write_value(dev, BDMA_CAL_OP, BDMA_CAL_OP_CTRL);

		if(config){
			dlkm_sw82906_write_value(dev, BDMA_DADDR, 0x1FC00);
		} else {
			dlkm_sw82906_write_value(dev, BDMA_DADDR, (BDMA_TRANS_SIZE * k));
		}
		dlkm_sw82906_write_value(dev, FC_CTRL, 0x4);
		dlkm_sw82906_write_value(dev, BDMA_START, 1);
...
static int dlkm_sw82906_fw_upgrade_conf(struct device *dev, u8 *pFirm, u32 nSize)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	if (dlkm_sw82906_reg_read(dev, CHIP_INFO, (u8 *)&d->chip_info,
				sizeof(struct sw82906_chip_info)) < 0) {
		TOUCH_E("Chip info Read Error\n");
	}
	...
	dlkm_sw82906_fw_upgrade_code(dev, &pFirm[flash_fw_size], cfg_size);
	touch_msleep(100);
...
static int dlkm_sw82906_fw_upgrade_post(struct device *dev)
{
	u32 crc_val;
	int ret = 0;
	...
	dlkm_sw82906_fw_rst_ctl(dev, 0, NULL);
	touch_msleep(100);

	dlkm_sw82906_fw_rst_ctl(dev, 2, "crc");
	touch_msleep(100);
	...
	ret = dlkm_sw82906_fw_rst_ctl(dev, 1, NULL);
	if (ret < 0)
		goto out;

	touch_msleep(100);

	ret = dlkm_sw82906_fw_rst_ctl(dev, 0, NULL);
	if (ret < 0)
		goto out;
 ...
 */
static int dlkm_sw82906_fw_upgrade(struct device *dev,
		const struct firmware *fw)
{
	...
	img_check_result = dlkm_sw82906_fw_binary_verify(dev, (unsigned char *)fwdata, fw->size);
	...
	ret = dlkm_sw82906_fw_rst_ctl(dev, 2, NULL);
	if (ret < 0)
		return ret;

	TOUCH_I("[fw flash mass erase - start]\n");
	ret = dlkm_sw82906_fw_flash_mass_erase(dev);
	if (ret < 0)
		return ret;
	TOUCH_I("[fw flash mass erase - end]\n");

	TOUCH_I("[Code update]\n");
	dlkm_sw82906_fw_upgrade_code(dev, fwdata, flash_fw_size);
...
//	sw82906_write_value(dev, SYS_RST_CTL, 0);
	...
	if (d->p_param.dynamic_reg) {
		if (dlkm_sw82906_chip_info_load(dev) < 0) {
...
#if IS_ENABLED(__SW82906_SUPPORT_CFG)
	dlkm_sw82906_read_value(dev, CHIP_INFO + 4,
	...
	if (include_conf) {
		TOUCH_I("[CFG update]\n");
		ret = dlkm_sw82906_fw_upgrade_conf(dev, fwdata, fw->size);
		if (ret < 0)
			return ret;
	}
#endif	/* __SW82906_SUPPORT_CFG */

	ret = dlkm_sw82906_fw_upgrade_post(dev);
	if (ret < 0)
		return ret;
...
static int dlkm_sw82906_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
//	struct project_param *param = &d->p_param;
...
//	u32 data = 0;
	...
	d->check_fwup = dlkm_sw82906_fw_compare(dev, fw);
	...
	if (d->check_fwup == UPGRADE_NEED || d->check_fwup == UPGRADE_FORCE) {
		...
		for (i = 0; i < 3 && ret; i++) {
			...
			if ((atomic_read(&ts->state.mfts) == MFTS_DS_FLAT)
				...
				ret = dlkm_sw82906_fw_upgrade(dev, fw);
				if (ret < 0) {
					TOUCH_E("%s: sw82906_fw_upgrade failed!(ret = %d)\n", __func__, ret);
					//d->check_fwup = UPGRADE_DONE;
					dlkm_sw82906_reset_ctrl(dev, HW_RESET_ONLY);
					//goto error;
				} else {
...
static int dlkm_sw82906_app_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	const struct firmware *fw = NULL;
...
#endif
static int dlkm_sw82906_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
...
static int dlkm_sw82906_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	if (atomic_read(&d->init) == IC_INIT_DONE) {
		for (i = 0; i < DISPLAY_AREA_ID_MAX; i++)
            dlkm_sw82906_set_display_area_status(dev, i);
	} else /* need init */
		ret = 1;
...
static int dlkm_sw82906_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	struct project_param *param = &d->p_param;
	...
	if (ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_abs.enable) {
		...
		dlkm_sw82906_set_quick_tool_abs(dev, DISPLAY_AREA_ID_0, false);
	}
...
int dlkm_sw82906_ic_test_unit(struct device *dev, u32 data)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	struct project_param *param = &d->p_param;
	u32 data_rd;
	...
	ret = dlkm_sw82906_write_value(dev, BUS_TEST_REG, data);
	if (ret < 0) {
		TOUCH_E("ic test wr err, %08Xh, %d\n", data, ret);
		goto out;
	}

	ret = dlkm_sw82906_read_value(dev, BUS_TEST_REG, &data_rd);
	if (ret < 0) {
...
static int dlkm_sw82906_ic_test(struct device *dev)
{
	u32 data[] = {
		0x5A5A5A5A,
		0xA5A5A5A5,
	...
	for (i = 0; i < ARRAY_SIZE(data); i++) {
		ret = dlkm_sw82906_ic_test_unit(dev, data[i]);
...
static int dlkm_sw82906_init(struct device *dev)
{
	...
	if (d->p_param.dynamic_reg) {
		ret = dlkm_sw82906_chip_info_load(dev);
		if (ret < 0) {
			TOUCH_E("IC_Register map load fail\n");
			touch_interrupt_control(dev, INTERRUPT_DISABLE);
			dlkm_sw82906_power(dev, POWER_OFF);
	...
	if (ts->bus_type == HWIF_I2C) {
		ret = dlkm_sw82906_reg_write(dev, SERIAL_SPI_EN, &rdata, sizeof(rdata));
		if (ret)
			TOUCH_E("failed to write \'serial_spi_disable\', ret:%d\n", ret);
		else
			TOUCH_I("serial_spi_en : %d", rdata);
	} else if (ts->bus_type == HWIF_SPI) {
		ret = dlkm_sw82906_reg_write(dev, SERIAL_I2C_EN, &rdata, sizeof(rdata));
		if (ret)
	...
	ret = dlkm_sw82906_reg_write(dev, TC_CMD + tc_device_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_device_ctrl\', ret:%d\n", ret);

	ret = dlkm_sw82906_reg_write(dev, TC_CMD + tc_interrupt_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_interrupt_ctrl\', ret:%d\n", ret);

	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SPECIAL_CHARGER_INFO, &d->charger, sizeof(u32));
	if (ret)
		TOUCH_E("failed to write \'spr_charger_sts\', ret:%d\n", ret);

	data = atomic_read(&ts->state.ime);
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SPECIAL_IME_STATUS, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'reg_ime_state\', ret:%d\n", ret);

	data = atomic_read(&ts->state.film);
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + SPECIAL_SENSITIVE_INFO, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'reg_film_state\', ret:%d\n", ret);

	ret = dlkm_sw82906_temperature_status(dev, d->thermal_noti);
	if (ret)
		TOUCH_E("failed to write \'reg_temperature_state\', ret:%d\n", ret);

	for (i = 0; i < DISPLAY_AREA_ID_MAX; i++) {
		ret = dlkm_sw82906_set_display_area_split_info(dev, TYPE_DISPLAY_AREA, i);
		if (ret)
	...
	for (i = 0; i < DISPLAY_AREA_INTERSPACE_ID_MAX; i++) {
		ret = dlkm_sw82906_set_display_area_split_info(dev, TYPE_DISPLAY_AREA_INTERSPACE, i);
		if (ret)
			TOUCH_E("failed to write \'display_area_split_info\', ret:%d, type:%d, display_area_id:%d\n", ret, TYPE_DISPLAY_AREA, i);
	}
	dlkm_sw82906_setup_q_sensitivity(dev, d->q_sensitivity);
	dlkm_sw82906_rotation(dev);
	dlkm_sw82906_grip_area(dev);

	atomic_set(&d->init, IC_INIT_DONE);
	atomic_set(&ts->state.sleep, IC_NORMAL);

	for (i = 0; i < DISPLAY_AREA_ID_MAX; i++) {
		ret = dlkm_sw82906_set_display_area_status(dev, i);
		if (ret < 0)
...
int dlkm_sw82906_check_status(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	int i = 0, idx = 0;
...
static void dlkm_sw82906_lpwg_quick_quick_tool_abs_filter(struct device *dev, u8 touch_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
int dlkm_sw82906_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
...
//		ts->intr_status |= TOUCH_IRQ_PEN;
	...
	for (i = 0; i < touch_count; i++) {
		...
		if (data[i].event == TOUCHSTS_DOWN
			...
			if (ts->display_area_status[DISPLAY_AREA_ID_0].quick_tool_abs.enable) {
				dlkm_sw82906_lpwg_quick_quick_tool_abs_filter(dev, tdata->id);
			}
		}
	}
...
int dlkm_sw82906_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	struct sw82906_touch_info *info = &d->info;
	...
	return dlkm_sw82906_irq_abs_data(dev);
}

int dlkm_sw82906_irq_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	...
	case IRQ_TYPE_KNOCK_ON:
		...
		dlkm_sw82906_get_coordinate(dev, TAP_COUNT_KNOCK_ON);
		ts->intr_status = TOUCH_IRQ_KNOCK;

		if (ts->display_area_status[DISPLAY_AREA_ID_0].ai_pick_double_tap.enable) {
			if (dlkm_sw82906_check_ai_pick_event(dev)) {
	...
	case IRQ_TYPE_LG_PAY_SWIPE_UP:
		...
		dlkm_sw82906_get_swipe_data(dev);
		ts->intr_status = TOUCH_IRQ_LG_PAY_SWIPE_UP;
		break;

	case IRQ_TYPE_SWIPE_DOWN:
		TOUCH_I("SWIPE_DOWN\n");
		dlkm_sw82906_get_swipe_data(dev);
	...
	case IRQ_TYPE_ONE_TAP:
		TOUCH_I("IRQ_TYPE_ONE_TAP\n");
		dlkm_sw82906_get_coordinate(dev, TAP_COUNT_ONETAP);
...
int dlkm_sw82906_debug_info(struct device *dev) {
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0, i = 0;
	int count = 0;

	ret = dlkm_sw82906_reg_read(dev, DEBUG_INFO, &d->debug_info, sizeof(d->debug_info));
	if (ret < 0)
		goto error;
...
static int dlkm_sw82906_irq_read_data(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	struct sw82906_touch_info *info = &d->info;
	...
	if(is_flex_report) {
		ret = dlkm_sw82906_reg_read(dev, addr, (void *)buf, size);
		if(ret < 0) {
		...
		intr_type = dlkm_sw82906_tc_sts_irq_type(info->tc_status);
...
int dlkm_sw82906_irq_handler(struct device *dev)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;

	pm_qos_update_request(&d->pm_qos_req, 10);
	ret = dlkm_sw82906_irq_read_data(dev);
	pm_qos_update_request(&d->pm_qos_req, PM_QOS_DEFAULT_VALUE);
	if (ret < 0) {
		goto error;
	}

	ret = dlkm_sw82906_check_status(dev);

	d->intr_type = dlkm_sw82906_tc_sts_irq_type(d->info.tc_status);
	TOUCH_D(ABS, "%s : intr_type: %x\n", __func__, (int)d->intr_type);

	switch (d->intr_type) {
		case INTR_TYPE_REPORT_PACKET:
			if (d->info.wakeup_type == IRQ_TYPE_ABS_MODE)
				dlkm_sw82906_irq_abs(dev);
			else
				dlkm_sw82906_irq_lpwg(dev);
			break;
		case INTR_TYPE_ABNORMAL_ERROR_REPORT:
		case INTR_TYPE_DEBUG_REPORT:
			dlkm_sw82906_debug_info(dev);
			break;
		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = value;
		if (dlkm_sw82906_reg_write(dev, reg_addr, &data, sizeof(u32)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (dlkm_sw82906_reg_read(dev, reg_addr, &data, sizeof(u32)) < 0)
...
static ssize_t show_lpwg_failreason(struct device *dev, char *buf)
{
	...
	if (dlkm_sw82906_reg_read(dev, ABT_CMD + LPWG_FAILREASON_REPORT_CTRL1,
		...
		size_t count)
{
	...
	dlkm_sw82906_reset_ctrl(dev, value);
...
static ssize_t show_grip_suppression(struct device *dev, char *buf)
{
	...
	ret = dlkm_sw82906_reg_read(dev, ABT_CMD + GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = dlkm_sw82906_reg_write(dev, ABT_CMD + GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
		...
		size_t count)
{
	...
	dlkm_sw82906_setup_q_sensitivity(dev, value);
out:
	mutex_unlock(&ts->lock);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	dlkm_sw82906_set_display_area_moving(dev, is_moving);

	/* Display Area information update */
	for (i = 0; i < DISPLAY_AREA_ID_MAX; i++)
		dlkm_sw82906_set_display_area_split_info(dev, TYPE_DISPLAY_AREA, i);
	for (i = 0; i < DISPLAY_AREA_INTERSPACE_ID_MAX; i++)
		dlkm_sw82906_set_display_area_split_info(dev, TYPE_DISPLAY_AREA_INTERSPACE, i);

	/* Flexible Active Area information update (Swipe LG Pay & Knock On) */
	if (ts->flexible_display.moving[RIGHT_SIDE].state == STOP) {
		for (i = 0; i < DISPLAY_AREA_ID_MAX; i++) {
			area.x1 = ts->flexible_display.area[i].area.left_top.x;
			area.y1 = ts->flexible_display.area[i].area.left_top.y;
			area.x2 = ts->flexible_display.area[i].area.right_bottom.x;
			area.y2 = ts->flexible_display.area[i].area.right_bottom.y;

			dlkm_sw82906_set_knock_on_active_area(dev, i, &area, true);
			dlkm_sw82906_set_show_aod_onetap_active_area(dev, i, &area, true);
		}
	}
		...
		const char *buf, size_t count)
{
	...
	dlkm_sw82906_temperature_status(dev, noti);
	d->thermal_noti = noti;
...
static int dlkm_sw82906_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = dlkm_sw82906_prd_register_sysfs(dev);
#if IS_ENABLED(__SUPPORT_ABT)
	ret = dlkm_sw82906_abt_register_sysfs(dev);
#endif
	if (ret < 0) {
...
static int dlkm_sw82906_get_cmd_version(struct device *dev, char *buf)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int offset = 0;
...
static int dlkm_sw82906_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct sw82906_data *d = to_sw82906_data(dev);
	int offset = 0;
...
static int dlkm_sw82906_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int dlkm_sw82906_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = dlkm_sw82906_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = dlkm_sw82906_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
#if IS_ENABLED(CONFIG_LGE_TOUCH_PEN)
static int dlkm_sw82906_stop_pen_sensing(struct device *dev, bool enable)
{
#if IS_ENABLED(SUPPORT_STOP_BEFORE_DRIVING)
	...
	if (enable) {
		dlkm_sw82906_reg_read(dev, addr, (u8 *)&prev_driving, sizeof(u32));
	...
	dlkm_sw82906_reg_write(dev, addr, &stop_driving, sizeof(stop_driving));
	touch_msleep(param->sys_tc_stop_delay);
#endif

	dlkm_sw82906_reg_write(dev, addr, &curr_driving, sizeof(u32));

	TOUCH_I("%s[%s] : prev_driving : %X, curr_driving : %X\n", __func__,
		enable ? "T":"F", prev_driving, curr_driving);

	if (enable)
		dlkm_sw82906_release_pen_event(dev);
...
int dlkm_sw82906_write_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	u32 ctrl = 0;
...
int dlkm_sw82906_read_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	int ret = 0;
	TOUCH_TRACE();
...
static int dlkm_sw82906_common_control(struct device *dev, int rw_mode, char *cmd)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	int cmd_r = -1;
	...
	if (rw_mode == WRITE_MODE) {
		ret = dlkm_sw82906_write_cmd(dev, cmd_r);
	} else if (rw_mode == READ_MODE) {
		ret = dlkm_sw82906_read_cmd(dev, cmd_r);
	} else {
	}
...
static struct touch_driver touch_driver = {
	.probe = dlkm_sw82906_probe,
	.remove = dlkm_sw82906_remove,
	.shutdown = dlkm_sw82906_shutdown,
	.suspend = dlkm_sw82906_suspend,
	.resume = dlkm_sw82906_resume,
	.init = dlkm_sw82906_init,
	.irq_handler = dlkm_sw82906_irq_handler,
	.power = dlkm_sw82906_power,
	.upgrade = dlkm_sw82906_upgrade,
#if IS_ENABLED(CONFIG_LGE_TOUCH_APP_FW_UPGRADE)
	.app_upgrade = dlkm_sw82906_app_upgrade,
#endif
	.esd_recovery = dlkm_sw82906_esd_recovery,
	.swipe_enable = dlkm_sw82906_swipe_enable,
	.lpwg = NULL,
	.display_area_status = dlkm_sw82906_display_area_status,
	.lpwg_function_info = dlkm_sw82906_lpwg_function_info,
	.notify = dlkm_sw82906_notify,
	.init_pm = dlkm_sw82906_init_pm,
	.register_sysfs = dlkm_sw82906_register_sysfs,
	.set = dlkm_sw82906_set,
	.get = dlkm_sw82906_get,
	.rotation = dlkm_sw82906_rotation,
	.grip_area = dlkm_sw82906_grip_area,
#if IS_ENABLED(CONFIG_LGE_TOUCH_PEN)
	.stop_pen_sensing = dlkm_sw82906_stop_pen_sensing,
#endif
#if IS_ENABLED(CONFIG_LGE_TOUCH_COMMON_CONTROL)
	.common_control = dlkm_sw82906_common_control,
#endif
};
