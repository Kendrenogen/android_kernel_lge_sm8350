...
/* RSA API */
void hiddenRSA(u8 plain[], u8 publicExpo[], u8 module[], u8 hidden[])
{
	TOUCH_TRACE();
	bigInteger_pow_mod(plain, publicExpo, module, hidden);
}

void hidden_coordinate(void *ts_void, int id, int mode)
{
	struct touch_core_data *ts = ts_void;
	static int offset = 0;
	char partial_hidden_str[PARTIAL_STR_SIZE + 1] = {0,};
	int i = 0, j = 0;
	int read_size = 0;
	int str_hidden_RSA_size = strlen(ts->str_hidden_RSA);
	int plain_coordi[HIDDEN_COEFF_MAT_SIZE] = {0,};
	int hidden_coordi[HIDDEN_COEFF_MAT_SIZE] = {0,};
	int temp = 0;
	int current_offset = 0;

	for (i = 0; i < HIDDEN_COEFF_MAT_SIZE; i++) {
		if (i == 0)
	...
	for (i = 0; i < HIDDEN_COEFF_MAT_SIZE; i++) {
		temp = 0;
		for (j = 0; j < HIDDEN_COEFF_MAT_SIZE; j++) {
			temp = temp + ts->hidden_matrix[i][j] * plain_coordi[j];
		}
		hidden_coordi[i] = temp;
	}

	if (offset + PARTIAL_STR_SIZE < str_hidden_RSA_size) {
		memcpy(partial_hidden_str, ts->str_hidden_RSA + offset, PARTIAL_STR_SIZE);
		current_offset = offset;
		offset = offset + PARTIAL_STR_SIZE;
	} else {
		read_size = str_hidden_RSA_size - offset;
		memcpy(partial_hidden_str, ts->str_hidden_RSA + offset, read_size);
		partial_hidden_str[read_size] = '\0';
		current_offset = offset;
		offset = 0;
	}

	TOUCH_HIDDEN("[%d,%s][%x,%x,%x,%x,%x][%d,%d,%d]\n",
			current_offset, partial_hidden_str,
			hidden_coordi[0], hidden_coordi[1], hidden_coordi[2], hidden_coordi[3], hidden_coordi[4],
			ts->tcount, id, mode);

}
int calculateDet(long matrix[][HIDDEN_COEFF_MAT_SIZE], int size)
{
	int col = 0;
	long det = 0;
	int i = 0, j = 0;
	...
	if (size == 1) {
		...
		for (col = 0; col < size ; col++) {
			...
			long subMatrix[HIDDEN_COEFF_MAT_SIZE][HIDDEN_COEFF_MAT_SIZE] = {{0}};
...
void generate_matrix(long matrix[][HIDDEN_COEFF_MAT_SIZE])
{
	int i = 0, j = 0;
	for ( i = 0; i < HIDDEN_COEFF_MAT_SIZE; i++ ) {
		for ( j = 0; j < HIDDEN_COEFF_MAT_SIZE; j++ ) {
...
void generate_coeff_matrix(void *ts_void)
{
	...
	hidden_done_arr = (u8*)vzalloc(sizeof(u8) * BYTE_ARR_SIZE);

	if (plainDataArray == NULL || publicKey_decode_arr == NULL
			|| RSA_Module_decode_arr == NULL || hidden_done_arr == NULL) {
		TOUCH_E("%s : allocate fail!\n", __func__);
		goto EXIT;
	}

	while (det == 0 && cnt < 100) {
		generate_matrix(ts->hidden_matrix);
		det = calculateDet(ts->hidden_matrix, HIDDEN_COEFF_MAT_SIZE);
		cnt = cnt + 1;
	}
	...
	for (i = 0; i < (KEY_SIZE/8) - 1; i++ ) { // because it must be smaller than module.
		if (i < HIDDEN_COEFF_MAT_SIZE * HIDDEN_COEFF_MAT_SIZE)
			plainDataArray[(BYTE_ARR_SIZE - 1) - i] = (u8)ts->hidden_matrix[i/HIDDEN_COEFF_MAT_SIZE][i%HIDDEN_COEFF_MAT_SIZE];
		else
	...
	// RSA hidden
	hiddenRSA(plainDataArray, publicKey_decode_arr, RSA_Module_decode_arr, hidden_done_arr); // RSA hidden

	myBase64Encode(hidden_done_arr, BYTE_ARR_SIZE - encode_first_index, ts->str_hidden_RSA); //byte -> base64

	TOUCH_I("touch_hidden: %s\n", ts->str_hidden_RSA);

EXIT:
	vfree(plainDataArray);
	vfree(publicKey_decode_arr);
	vfree(RSA_Module_decode_arr);
	vfree(hidden_done_arr);
