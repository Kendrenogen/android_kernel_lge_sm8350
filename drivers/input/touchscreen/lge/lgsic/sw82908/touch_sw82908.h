...
#ifndef LGE_TOUCH_SW82908_H
#define LGE_TOUCH_SW82908_H

#include <linux/pm_qos.h>

#define __SW82908_STILL_BRING_UP
...
struct sw82908_touch_info {
	...
	u32 rsvd2:4;
	u32 palm_bit:1;
	struct sw82908_pen_data pen;
	struct sw82908_touch_data data[10];
	/* debug info */
} __packed;

static inline u32 sw82908_tc_sts_irq_type(int status)
{
	return ((status >> 16) & 0x0F);
}

static inline u32 sw82908_tc_sts_running_sts(int status)
{
	return (status & 0x1F);
}

#define TYPE_SW82908 9
...
/* v sync driving command*/
#define VSYNC_DRIVING_CMD			(0xC0D)
#define SPECIAL_TEMPERATURE_INFO		(0x90)

#define ROTATION_ANGLE				(0xF9C)
#define GRIP_EXCEPTION_ENABLE		(0xFA8)
#define GRIP_EXCEPTION_START		(0xFA9)
#define GRIP_EXCEPTION_END			(0xFAA)
...
enum {
	...
	IRQ_TYPE_QUICK_TOOL_SWIPE_LEFT = 3,
	IRQ_TYPE_QUICK_TOOL_SWIPE_RIGHT = 4,
	IRQ_TYPE_LG_PAY_SWIPE_UP = 5,
	IRQ_TYPE_SWIPE_DOWN = 6,
	IRQ_TYPE_LONG_PRESS = 7,
	IRQ_TYPE_LG_PAY_SWIPE_LEFT = 8,
	IRQ_TYPE_LG_PAY_SWIPE_RIGHT = 9,
	IRQ_TYPE_ONE_TAP = 50,
...
struct sw82908_version_bin {
	u8 major : 4;
	u8 build : 4;
	u8 minor;
};

struct sw82908_version {
	u8 minor;
	u8 major:4;
	u8 build:4;
	u8 chip_id;
...
struct sw82908_ic_info {
	struct sw82908_version version;
	u8 product_id[8+4];
	struct sw82908_pt_info pt_info;
	struct sw82908_version_bin img_version;
};

struct sw82908_chip_info {
	u32 r_version;
	u32 r_product_code;
	u32 r_product_id1;
	u32 r_product_id2;
...
struct sw82908_touch_debug_info {
	u32 info[3];
	u32 type:24;
	u32 length:8;
} __packed;

struct sw82908_fb_blank {
	int prev;
	int curr;
};

struct sw82908_active_area {
	s16 x1;
	s16 y1;
	s16 x2;
	s16 y2;
...
struct sw82908_lpwg_abs_ctrl {
	bool enable;
	struct sw82908_active_area area;
	struct sw82908_active_area border;
	s16 offset_y;
};
struct sw82908_lpwg_longpress_ctrl {
	bool enable;
	struct sw82908_active_area area;
	struct sw82908_active_area border;
	int slop;
...
struct sw82908_lpwg_onetap_ctrl {
	bool enable;
	struct sw82908_active_area area;
	struct sw82908_active_area border;
	int slop;
...
struct sw82908_ai_pick_ctrl {
	bool enable;
	bool pre_enable;
	struct sw82908_active_area area;
	struct sw82908_active_area border_area;
	struct sw82908_active_area total_area;
};

struct sw82908_lpwg_abs_buf {
	u32 enable;
	u16 start_x;
	u16 start_y;
	u16 end_x;
...
struct sw82908_lpwg_longpress_buf {
	u32 enable;
	u16 start_x;
	u16 start_y;
	u16 end_x;
...
struct sw82908_lpwg_onetap_buf {
	u32 enable;
	u16 start_x;
	u16 start_y;
	u16 end_x;
...
struct sw82908_data {
	...
	struct sw82908_touch_info info;
	struct sw82908_touch_debug_info debug_info;
	struct sw82908_ic_info ic_info;
	struct sw82908_chip_info chip_info;
//	struct sw82908_reg_info reg_info;
	struct workqueue_struct *wq_log;
	struct sw82908_fb_blank fb_blank;

	struct sw82908_lpwg_abs_ctrl lpwg_abs;
	struct sw82908_ai_pick_ctrl ai_pick;
	struct sw82908_lpwg_longpress_ctrl lpwg_longpress;
	struct sw82908_lpwg_onetap_ctrl lpwg_onetap;
...
static inline struct sw82908_data *to_sw82908_data(struct device *dev)
{
	return (struct sw82908_data *)touch_get_device(to_touch_core(dev));
}

static inline struct sw82908_data *to_sw82908_data_from_kobj(struct kobject *kobj)
{
	return (struct sw82908_data *)container_of(kobj,
			struct sw82908_data, kobj);
}

extern int sw82908_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int sw82908_reg_write(struct device *dev, u16 addr, void *data, int size);
extern int sw82908_ic_info(struct device *dev);
extern int sw82908_te_info(struct device *dev, char *buf);
extern int sw82908_tc_driving(struct device *dev, int mode);
extern int sw82908_irq_abs(struct device *dev);
extern int sw82908_irq_abs_data(struct device *dev);
extern int sw82908_irq_lpwg(struct device *dev);
extern int sw82908_irq_handler(struct device *dev);
extern int sw82908_check_status(struct device *dev);
extern int sw82908_debug_info(struct device *dev);
extern int sw82908_sleep_ctrl(struct device *dev, int new_status);
/*
extern int sw82908_chip_info_load(struct device *dev);
*/
extern int sw82908_reset_ctrl(struct device *dev, int ctrl);
...
void sw82908_irq_runtime_engine_debug(struct device *dev);

static inline int sw82908_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw82908_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw82908_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw82908_reg_write(dev, addr, &value, sizeof(value));
}
#if defined(CONFIG_LGE_TOUCH_COMMON_CONTROL)
static char *sw82908_cmd_lists[MAX_CMD_COUNTS][2] = {
		[0] = {"fp_intr2", "[high:1 low:0]"},
			{NULL},
};
enum {
		FP_INTR2,
