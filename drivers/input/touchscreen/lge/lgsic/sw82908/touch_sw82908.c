...
static int sw82908_lpwg_mode(struct device *dev);
static void sw82908_lcd_mode(struct device *dev, u32 mode);
static int sw82908_power(struct device *dev, int ctrl);
static int sw82908_init(struct device *dev);
static int sw82908_vsync_set(struct device *dev, int force_mode);

static void project_param_set(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
void sw82908_xfer_msg_ready(struct device *dev, u8 msg_cnt)
{
...
int sw82908_xfer_msg(struct device *dev, struct touch_xfer_msg *xfer)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
int sw82908_reg_read(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
int sw82908_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct touch_bus_msg msg = {0, };
...
static int sw82908_drm_notifier_callback(struct notifier_block *self,
...
static int sw82908_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
static int sw82908_sw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;
	...
	if (mode == SW_RESET) {
		...
		sw82908_write_value(dev, SYS_RST_CTL, 2);
		touch_msleep(20);
		...
		sw82908_write_value(dev, SYS_RST_CTL, 1);
		touch_msleep(20);
		sw82908_write_value(dev, SYS_RST_CTL, 0);
	} else {
...
int sw82908_hw_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
int sw82908_only_hw_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
int sw82908_power_reset(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	sw82908_power(ts->dev, POWER_OFF);
	sw82908_power(ts->dev, POWER_ON);
...
int sw82908_reset_ctrl(struct device *dev, int ctrl)
{
	int ret = 0;
	...
	case SW_RESET_CODE_DUMP:
		ret = sw82908_sw_reset(dev, ctrl);
		break;

	case HW_RESET_ASYNC:
	case HW_RESET_SYNC:
		ret = sw82908_hw_reset(dev, ctrl);
		break;
	case HW_RESET_ONLY:
		ret = sw82908_only_hw_reset(dev);
		break;
	case HW_RESET_POWER:
		ret = sw82908_power_reset(dev, ctrl);
		break;
	default:
...
static int sw82908_power(struct device *dev, int ctrl)
{
	...
	case POWER_HW_RESET_ASYNC:
		TOUCH_I("%s, HW reset\n", __func__);
		sw82908_reset_ctrl(dev, HW_RESET_ASYNC);
		break;
	case POWER_HW_RESET_SYNC:
		TOUCH_I("%s, HW reset\n", __func__);
		sw82908_reset_ctrl(dev, HW_RESET_SYNC);
		break;
	case POWER_SW_RESET:
		sw82908_reset_ctrl(dev, SW_RESET);
		break;
	case POWER_SLEEP:
	case POWER_WAKE:
	default:
...
static int sw82908_ic_boot_check(struct device *dev, u32 *boot_st)
{
	u32 bootmode = 0;
	u32 rdata_crc = 0;
	u32 rdata_pass = ~0;
	...
	ret = sw82908_read_value(dev, GDMA_CRC_RESULT, &rdata_crc);
	err |= (rdata_crc != CRC_FIXED_VALUE);

	ret = sw82908_read_value(dev, GDMA_CRC_PASS, &rdata_pass);
	err |= (!rdata_pass)<<1;

	ret = sw82908_read_value(dev, INFO_PTR_ADDR, &rdata_ptr);
	err |= (!rdata_ptr)<<2;
...
int sw82908_ic_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	u32 bootmode = 0;
	...
	ret = sw82908_reg_read(dev, CHIP_INFO + tc_version,
			&d->ic_info.version, sizeof(d->ic_info.version));

	ret = sw82908_reg_read(dev, CHIP_INFO + tc_product_id1,
			&d->ic_info.product_id, 8);

#if !defined(__SW82908_STILL_BRING_UP)
	ret = sw82908_reg_read(dev, PT_INFO + pt_info_channel,
			&d->ic_info.pt_info, sizeof(d->ic_info.pt_info));
#endif	/* __SW82908_STILL_BRING_UP */

	sw82908_ic_boot_check(dev, &bootmode);
...
static void sw82908_get_longpress_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_longpress_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	if (enable) {
		...
		ret = sw82908_reg_write(dev, ABT_CMD + LONG_PRESS_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
...
//			set_debug_reason(dev, LPWG_DEBUG_LONGPRESS);
		...
		ret = sw82908_reg_write(dev, ABT_CMD + LONG_PRESS_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
...
static void sw82908_get_onetap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_onetap_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	if (enable) {
		...
		ret = sw82908_reg_write(dev, ABT_CMD + ONE_TAP_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
			TOUCH_E("failed to write long press register (ret = %d)\n",
					ret);
		else {
...
//			set_debug_reason(dev, LPWG_DEBUG_LONGPRESS);
		...
		ret = sw82908_reg_write(dev, ABT_CMD + ONE_TAP_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
			TOUCH_E("failed to clear long press register (ret = %d)\n",
					ret);
...
static void set_debug_reason(struct device *dev, int type)
{
	...
	sw82908_reg_write(dev, ABT_CMD + LPWG_FAILREASON_ON, &data, sizeof(data));
}

static void sw82908_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw82908_get_tci_data(struct device *dev, int count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	u8 i = 0;
...
static int sw82908_tci_active_area(struct device *dev,
		u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	ret = sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_X1, &active_area[0], sizeof(u32));
	ret = sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_Y1, &active_area[1], sizeof(u32));
	ret = sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_X2, &active_area[2], sizeof(u32));
	ret = sw82908_reg_write(dev, ABT_CMD + TCI_ACTIVE_AREA_Y2, &active_area[3], sizeof(u32));

	return ret;
}

static int sw82908_tci_control(struct device *dev, int type)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	case ACTIVE_AREA_CTRL:
		ret = sw82908_tci_active_area(dev,
...
static int sw82908_tci_knock(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	//struct tci_info *info2 = &ts->tci.info[TCI_2];
	...
	ret = sw82908_reg_write(dev, ABT_CMD + TCI_ENABLE,
			&lpwg_data[0], sizeof(lpwg_data));

	ret = sw82908_tci_control(dev, ACTIVE_AREA_CTRL);

	set_debug_reason(dev, LPWG_DEBUG_TCI_1);
	return ret;
}

static int sw82908_tci_password(struct device *dev)
{
	set_debug_reason(dev, LPWG_DEBUG_TCI_2);
	return sw82908_tci_knock(dev);
}

static void sw82908_init_ai_pick_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static void sw82908_ai_pick_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	...
	if (enable) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE) {
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				...
				sw82908_sleep_ctrl(dev, IC_NORMAL);
				sw82908_tc_driving(dev, LCD_MODE_U0);
			}

			info1->enable |= 0x01;
			info1->intr_delay = 0;
			info1->tap_distance = 10;
			sw82908_tci_knock(dev);

			sw82908_tci_active_area(dev,
...
static void sw82908_print_ai_pick_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static bool sw82908_check_ai_pick_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_lpwg_control(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct knock_on_ctrl *info1 = &ts->display_area_status[DISPLAY_AREA_ID_0].knock_on;
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	case LPWG_NONE:
		info1->enable = 0;
		ret = sw82908_tci_control(dev, ENABLE_CTRL);
		if (d->ai_pick.enable)
			sw82908_ai_pick_enable(dev, d->ai_pick.enable);
		break;
...
static int sw82908_get_swipe_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	u32 rdata[3];
...
static void sw82908_get_swipe_info(struct device *dev)
{
...
static int sw82908_lpwg_abs_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	if (enable) {
		...
		ret = sw82908_reg_write(dev, ABT_CMD + LPWG_ABS_ENABLE, &buf, sizeof(buf));
		if (ret < 0)
		...
		ret = sw82908_reg_write(dev, ABT_CMD + LPWG_ABS_ENABLE, &(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
...
static void sw82908_get_lpwg_abs_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static void sw82908_print_lpwg_abs_info(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (enable) {
		...
		if (atomic_read(&ts->state.sleep) == IC_NORMAL) {
			...
			ret = sw82908_reg_write(dev, ABT_CMD + SWIPE_ON,
					&buf, sizeof(buf));
			if (ret < 0) {
			...
			ret = sw82908_reg_write(dev, ABT_CMD + SWIPE2_ON,
					&buf2, sizeof(buf2));
			if (ret < 0) {
...
static int sw82908_clock(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (onoff) {
		if (ts->bus_type == HWIF_SPI) {
			sw82908_reg_write(dev, SPI_OSC_CTL, &onoff, sizeof(onoff));
			atomic_set(&ts->state.sleep, IC_NORMAL);
			touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
		...
		//pllosc_sel
		sw82908_reg_read(dev, SYS_CLK_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(pllosc_sel);
		sw82908_reg_write(dev, SYS_CLK_CTL, &reg_val, sizeof(reg_val));
		TOUCH_I("SYS_CLK_CTL(0x%x) value(0x%x) pllosc_sel(0x%x) \n", SYS_CLK_CTL, reg_val, pllosc_sel);
		touch_msleep(1);

		//xe_en
		sw82908_reg_read(dev, SYS_PLL_CTL1, &reg_val , sizeof(reg_val));
		reg_val &= ~(xe_en);
		sw82908_reg_write(dev, SYS_PLL_CTL1, &reg_val, sizeof(reg_val));
		TOUCH_I("SYS_PLL_CTL1(0x%x) value(0x%x) xe_en(0x%x) \n", SYS_PLL_CTL1, reg_val, xe_en);
		touch_msleep(1);

		//ldo15_bias_mute
		sw82908_reg_read(dev, LDO15_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(r_ldo15_bias_mute);
		sw82908_reg_write(dev, LDO15_CTL, &reg_val, sizeof(reg_val));
		TOUCH_I("LDO15_CTL(0x%x) value(0x%x) r_ldo15_bias_mute(0x%x) \n", LDO15_CTL, reg_val, r_ldo15_bias_mute);
		touch_msleep(1);

		//ldo_bias
		sw82908_reg_read(dev, SYS_LDO_CTL, &reg_val , sizeof(reg_val));
		reg_val &= ~(ldo_bias);
		sw82908_reg_write(dev, SYS_LDO_CTL, &reg_val, sizeof(reg_val));
		...
		sw82908_reg_write(dev, SPI_OSC_CTL, &reg_val, sizeof(reg_val));
...
static inline int sw82908_clock(struct device *dev, u32 onoff)
{
	TOUCH_NOOP();
	return 0;
}
...
static void sw82908_print_failreason(struct device *dev, int type, int count)
{
	u64 buf = 0;
	u8 data = 0;
	u8 fail_num = 0;
	...
	switch (type) {
	case LPWG_DEBUG_TCI_1:
		ret = sw82908_reg_read(dev, ABT_CMD + TCI0_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_TCI_NUM;
		break;
	case LPWG_DEBUG_TCI_2:
		ret = sw82908_reg_read(dev, ABT_CMD + TCI1_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_TCI_NUM;
		break;
	case LPWG_DEBUG_SWIPE:
		ret = sw82908_reg_read(dev, ABT_CMD + SWIPE_FAILREASON_BUF,
				&buf, sizeof(buf));
		fail_num = LPWG_FAILREASON_SWIPE_NUM;
		break;
	case LPWG_DEBUG_LONGPRESS:
		ret = sw82908_reg_read(dev, ABT_CMD + LONGPRESS_FAILREASON_BUF,
				&buf, sizeof(buf));
...
static void sw82908_lpwg_failreason(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	u32 status = 0;
	int ret,i = 0;
	...
	ret = sw82908_reg_read(dev, ABT_CMD + LPWG_FAILREASON_STS,
...
int sw82908_tc_driving(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct project_param *param = &d->p_param;
	...
	if (mode != LCD_MODE_STOP) {
		ctrl = 0x04;
		sw82908_reg_write(dev, addr, &ctrl, sizeof(ctrl));
...
*/
	...
	if (mode == LCD_MODE_U3) {
		...
		sw82908_vsync_set(dev, 1);
	}

	sw82908_reg_write(dev, addr,
			&ctrl, sizeof(ctrl));
	...
	/* tc_status check */
	sw82908_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
	TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);

#if 0
	sw82908_reg_read(dev, TC_STS, (u8 *)&rdata, sizeof(u32));
	TOUCH_I("read tc_status(%x) = %x\n", TC_STS, rdata);

	sw82908_reg_read(dev, TC_CMD, (u8 *)&rdata, sizeof(rdata));
...
static int sw82908_release_pen_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_TRACE();
	ts->pdata.in_range = 0;
...
int sw82908_sleep_ctrl(struct device *dev, int new_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void sw82908_longpress_release(struct device *dev) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg) {
			...
			ret = sw82908_lpwg_control(dev, LPWG_DOUBLE_TAP);
			sw82908_swipe_enable(dev, true);
			ret = sw82908_tc_driving(dev, LCD_MODE_U0);
			return 0;
		}
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
			...
			if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE
				...
				sw82908_sleep_ctrl(dev, IC_NORMAL);
				ret = sw82908_lpwg_control(dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
				sw82908_swipe_enable(dev, true);
				sw82908_tc_driving(dev, LCD_MODE_U0);
				if (d->lpwg_abs.enable) {
					TOUCH_I("%s: enable lpwg_abs\n", __func__);
					sw82908_lpwg_abs_enable(dev, d->lpwg_abs.enable);
				}
				if (d->ai_pick.enable) {
				...
				if (d->lpwg_longpress.enable) {
					...
					sw82908_longpress_enable(dev, d->lpwg_longpress.enable);
				}
				if (d->lpwg_onetap.enable) {
					TOUCH_I("%s: enable onetap\n", __func__);
					sw82908_onetap_enable(dev, d->lpwg_onetap.enable);
				}
			}
		}
		return ret;
	...
	if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.qcover == HALL_NEAR) {
			...
			sw82908_sleep_ctrl(dev, IC_NORMAL);
			sw82908_lpwg_control(dev, LPWG_NONE);
			ret = sw82908_tc_driving(dev, LCD_MODE_U3);
		}
	} else if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.sensor == PROX_NEAR) {
...
static int sw82908_setup_q_sensitivity(struct device *dev, int enable)
{
	...
	ret = sw82908_reg_write(dev, ABT_CMD + COVER_SENSITIVITY,
...
static int sw82908_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
	...
	case LPWG_UPDATE_ALL:
		if ((ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen == 1 && value[1] == 0 &&
			...
			sw82908_setup_q_sensitivity(dev, 0);
...
static void sw82908_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	sw82908_reg_write(dev, ABT_CMD + SPECIAL_CHARGER_INFO,
...
static void sw82908_vsync_mode(struct device *dev, u32 mode)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_check_mode(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;
...
static int sw82908_vsync_set(struct device *dev, int force_mode)
{
	...
	if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
		TOUCH_I("Deep Sleep Status: skip to set vsync driving\n");
		return ret;
	}
	if (data) {
		TOUCH_I("%s : change vsync driving! (data = 0x%x)\n", __func__, data);
		sw82908_reg_write(dev, VSYNC_DRIVING_CMD, &data, sizeof(data));
	}
	return ret;
}

#if defined(__SUPPORT_NOTIFY_LCD_EVENT_REG)
static void sw82908_lcd_event_read_reg(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	...
	sw82908_read_value(dev, TC_IC_STATUS, &rdata[0]);

	sw82908_read_value(dev, TC_STS, &rdata[1]);

	sw82908_read_value(dev, version_addr, &rdata[2]);

	sw82908_read_value(dev, SPR_CHIP_ID, &rdata[3]);
...
static int get_batt_temperature(void)
{
	...
	TOUCH_I("%s: batt_temp(%d)\n", __func__, batt_temp);

	return batt_temp;
}

static int sw82908_temperature_status(struct device *dev, bool force_write_flag)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	batt_temp_change_flag = (d->batt_wdata & (1 << 16)) || (batt_temp < 0);//d->batt_wdata is condition variable to escape from -xx celsius
	...
	if (batt_temp_change_flag || force_write_flag) {
		...
		d->batt_wdata = wdata;
		ret = sw82908_reg_write(dev, ABT_CMD + SPECIAL_TEMPERATURE_INFO,
							&wdata, sizeof(u32));
		TOUCH_I("%s: origin noti:%d, batt:%d, w_val:0x%x, force_write_flag:%d\n",
				__func__, atomic_read(&ts->state.temperature), batt_temp, d->batt_wdata, force_write_flag);
	} else {
		TOUCH_I("%s: batt_temp setting skip. noti:%d, batt:%d, w_val:0x%x, force_write_flag:%d\n",
				__func__, atomic_read(&ts->state.temperature), batt_temp, d->batt_wdata, force_write_flag);
	}

	return ret;
}
static int sw82908_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw82908_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw82908_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int sw82908_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	//	ts->driver->irq_handler = sw82908_sic_abt_irq_handler;
		...
		ts->driver->irq_handler = sw82908_irq_handler;
	}

	return 0;
}

static int sw82908_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
...
static int sw82908_rotation(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = sw82908_reg_write(dev, ROTATION_ANGLE, &ts->rotation, sizeof(int));
	if (ret < 0)
...
static int sw82908_grip_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u32 buf = 0;
	...
	buf = 1;
	ret = sw82908_reg_write(dev, GRIP_EXCEPTION_ENABLE, &buf, sizeof(buf));
	if (ret < 0)
	...
	ret = sw82908_reg_write(dev, GRIP_EXCEPTION_START, &buf, sizeof(buf));
	if (ret < 0)
	...
	ret = sw82908_reg_write(dev, GRIP_EXCEPTION_END, &buf, sizeof(buf));
	if (ret < 0)
	...
	buf = ts->grip_area[IGNORE_DS_TOOL].left_top.x;
	buf |= ts->grip_area[IGNORE_DS_TOOL].left_top.y << 16;
	ret = sw82908_reg_write(dev, GRIP_EXCEPTION_START, &buf, sizeof(buf));
	if (ret < 0)
		TOUCH_E("failed to write GRIP_EXCEPTION_START");

	buf = ts->grip_area[IGNORE_DS_TOOL].right_bottom.x;
	buf |= ts->grip_area[IGNORE_DS_TOOL].right_bottom.y << 16;
	ret = sw82908_reg_write(dev, GRIP_EXCEPTION_END, &buf, sizeof(buf));
	if (ret < 0)
		TOUCH_E("failed to write GRIP_EXCEPTION_END");
...
static void sw82908_fb_notify_work_func(struct work_struct *fb_notify_work)
{
...
static void sw82908_partial_tcdriving_work_func(struct work_struct *partial_tcdriving)
{
	struct sw82908_data *d =
	...
	if (atomic_read(&ts->state.fb) == FB_RESUME && !ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen
		...
		sw82908_sleep_ctrl(d->dev, IC_NORMAL);
		sw82908_lpwg_control(d->dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
		sw82908_swipe_enable(d->dev, true);
		sw82908_tc_driving(d->dev, LCD_MODE_U0);
		if (d->ai_pick.enable) {
			TOUCH_I("%s: enable ai pick\n", __func__);
			sw82908_ai_pick_enable(d->dev, d->ai_pick.enable);
		}
...
static int sw82908_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct project_param *param = &d->p_param;
	...
	case LCD_EVENT_LCD_MODE:
		...
		sw82908_lcd_mode(dev, *(u32 *)data);
		ret = sw82908_check_mode(dev);
...
#if defined(__SUPPORT_NOTIFY_LCD_EVENT_REG)
		sw82908_lcd_event_read_reg(dev);
#endif
		break;
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = sw82908_usb_status(dev, *(u32 *)data);
		break;
	case NOTIFY_WIRELESS:
		TOUCH_I("NOTIFY_WIRELEES!\n");
		ret = sw82908_wireless_status(dev, *(u32 *)data);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = sw82908_earjack_status(dev, *(u32 *)data);
		break;
	case NOTIFY_FM_RADIO:
		...
		switch (d->driving_mode) {
			...
			case LCD_MODE_U3:
				...
				ret = sw82908_reg_write(dev, ABT_CMD +
...
//					}
	...
	case NOTIFY_DUALSCREEN_STATE:
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
			...
			sw82908_tc_driving(dev, LCD_MODE_U3);
		}
		break;
	case LCD_EVENT_CHANGE_FPS:
		TOUCH_I("LCD_EVENT_CHANGE_FPS : %d\n", *(int *)data);
		sw82908_vsync_mode(dev, *(u32 *)data);
		sw82908_vsync_set(dev, 0);
		break;
	case NOTIFY_TEMPERATURE:
		TOUCH_I("NOTIFY_TEMPERATURE!\n");
		sw82908_temperature_status(dev, false);
		break;
	default:
...
static int sw82908_init_pm(struct device *dev)
{
#if defined(CONFIG_DRM_MSM) && defined(CONFIG_FB)
#if defined(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: drm_notif change\n", __func__);
	ts->drm_notif.notifier_call = sw82908_drm_notifier_callback;
#endif
#elif defined(CONFIG_FB)
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_I("%s: fb_notif change\n", __func__);
	ts->fb_notif.notifier_call = sw82908_fb_notifier_callback;
#endif

	return 0;
}

static int sw82908_init_works(struct sw82908_data *d)
{
	...
	INIT_DELAYED_WORK(&d->partial_tcdriving, sw82908_partial_tcdriving_work_func);
	INIT_DELAYED_WORK(&d->fb_notify_work, sw82908_fb_notify_work_func);

	return 0;
}

static void sw82908_init_locks(struct sw82908_data *d)
{
	mutex_init(&d->io_lock);
}

static int sw82908_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	sw82908_init_works(d);
	sw82908_init_locks(d);
...
//				atomic_read(&ts->state.film_mode));
	...
	sw82908_get_tci_info(dev);
	sw82908_get_swipe_info(dev);

	sw82908_get_lpwg_abs_info(dev);
	sw82908_init_ai_pick_info(dev);
	sw82908_get_longpress_info(dev);
	sw82908_get_onetap_info(dev);
...
static int sw82908_remove(struct device *dev)
{
	TOUCH_TRACE();
...
static int sw82908_shutdown(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
static int sw82908_condition_wait(struct device *dev,
		...
		u32 mask, u32 delay, u32 retry, int not_cond)
{
	...
	do {
		touch_msleep(delay);
		ret = sw82908_read_value(dev, addr, &data);
		if (ret >= 0) {
...
#if defined(__SW82908_SUPPORT_CFG)
static int sw82908_fw_verify_cfg(char *buf)
{
...
static int sw82908_fw_verify_s_cfg(char *buf, int index)
{
	struct cfg_head *head = (struct cfg_head *)buf;
...
static int sw82908_fw_binary_verify(struct device *dev, u8 *fw_buf, int fw_size)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int flash_fw_size = d->p_param.flash_fw_size;
	...
	ret = sw82908_fw_verify_cfg((char *)head);
	if (ret < 0) {
	...
	for (i = 0; i < s_cfg_cnt; i++) {
		ret = sw82908_fw_verify_s_cfg((char *)s_cfg_base, i);
		if (ret < 0) {
...
static int sw82908_flash_erase_check(struct device *dev)
{
	...
	sw82908_reg_write(dev, spr_flash_offset, &start_offset, sizeof(start_offset));
	TOUCH_I("flash read offset(start_offset) %x \n", start_offset);	//for debug

	sw82908_reg_read(dev, flash_access_addr, &flash_read_val, sizeof(flash_read_val));
	...
	sw82908_reg_write(dev, spr_flash_offset, &end_offset, sizeof(end_offset));
...
static int __used sw82908_fw_flash_crc(struct device *dev, u32 *crc_val)
{
	...
	ret = sw82908_write_value(dev, gdma_saddr, 0);
	if (ret < 0) {
	...
	ret = sw82908_write_value(dev, gdma_ctl, ctrl_data);
	if (ret < 0) {
	...
	ret = sw82908_write_value(dev, gdma_start, 1);
	if (ret < 0) {
	...
	ret = sw82908_read_value(dev, gdma_crc_result, &data);
	if (ret < 0) {
	...
	ret = sw82908_read_value(dev, gdma_crc_pass, &data);
	if (ret < 0) {
...
static int __used sw82908_fw_flash_mass_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	ret = sw82908_write_value(dev, fc_addr, 0);
	if (ret < 0) {
		fc_err = 1;
		goto out;
	}

	ret = sw82908_write_value(dev, fc_ctrl, FC_CTRL_MASS_ERASE);
	if (ret < 0) {
		fc_err = 2;
		goto out;
	}

	sw82908_reg_read(dev, fc_ctrl, &read_reg, sizeof(read_reg));
	TOUCH_I("fc_ctrl erase command read_val : %x\n", read_reg);

	ret = sw82908_write_value(dev, fc_stat, 1);
	if (ret < 0) {
		fc_err = 3;
		goto out;
	}
	...
	chk_resp = 1;
	ret = sw82908_condition_wait(dev, spi_flash_status, &data,
	...
	sw82908_reg_read(dev, SYS_RST_CTL, &read_reg, sizeof(read_reg));
	TOUCH_I("1st cm3 status check read_val : %x\n", read_reg);

	//Flash Area check(first,last)
	ret = sw82908_flash_erase_check(dev);
	if (ret < 0) {
		fc_err = 5;
	...
	ret = sw82908_fw_flash_crc(dev, &erase_crc_val);
	if (ret < 0) {
		fc_err = 6;
...
out:
	...
	sw82908_write_value(dev, fc_ctrl, 0);
...
static int __used sw82908_fw_flash_page_erase(struct device *dev)
{
	u32 fc_addr = FC_ADDR;
	u32 fc_ctrl = FC_CTRL;
	u32 fc_stat = FC_STAT;
	...
	for (i = 0; i < (FLASH_SIZE/FLASH_PAGE_SIZE); i++) {
		ret = sw82908_write_value(dev, fc_addr, addr);
		if (ret < 0) {
			fc_err = 1;
			break;
		}

		ret = sw82908_write_value(dev, fc_ctrl, FC_CTRL_PAGE_ERASE);
		if (ret < 0) {
			fc_err = 2;
			break;
		}
	...
	sw82908_write_value(dev, fc_addr, 0);
	sw82908_write_value(dev, fc_ctrl, 0);
...
static int __used sw82908_fw_flash_write(struct device *dev, int addr, u8 *dn_buf, int dn_size)
{
	...
	ret = sw82908_write_value(dev, fc_offset, addr);
	if (ret < 0) {
	...
	ret = sw82908_reg_write(dev, fc_code_access, dn_buf, dn_size);
	if (ret < 0) {
...
static int sw82908_fw_rst_ctl(struct device *dev, int val, const char *str)
{
	...
	ret = sw82908_write_value(dev, rst_ctl, val);
	if (ret < 0) {
...
static int sw82908_fw_upgrade_code(struct device *dev, u8 *dn_buf, int dn_size)
{
//	struct sw82908_data *d = to_sw82908_data(dev);
//	struct project_param *param = &d->p_param;
	u32 fc_ctrl = FC_CTRL;
	...
	if (param->wa_trim_wr) {
		sw82908_read_value(dev, SYS_OSC_CTL, &reg_buf);
		reg_buf &= ~0x7F;
		reg_buf |= 0x5A;
		sw82908_write_value(dev, SYS_OSC_CTL, reg_buf);
	}
#endif

//	sw82908_write_value(dev, FLASH_TPROG, 560);

	sw82908_write_value(dev, fc_ctrl, FC_CTRL_WR_EN);
	...
	do {
		touch_msleep(10);
		sw82908_read_value(dev, BDMA_STS, &reg_buf);
		busycheck_cnt++;
	...
	for (k = 0; k <= write_cnt; k++)
	{
		...
		if(k == write_cnt){
			...
			sw82908_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], remain_size);
			reg_buf = remain_size / sizeof(u32);
		}else{
			sw82908_fw_flash_write(dev, 0, &dn_buf[k*BDMA_TRANS_SIZE], BDMA_TRANS_SIZE);
			reg_buf = BDMA_TRANS_SIZE / sizeof(u32);
		}
		reg_buf |= BDMA_CTL_BDMA_EN;

		sw82908_write_value(dev, BDMA_SADDR, DATASRAM_ADDR);
		sw82908_write_value(dev, BDMA_CTL, reg_buf);

		sw82908_write_value(dev, BDMA_CAL_OP, BDMA_CAL_OP_CTRL);

		if(config){
			sw82908_write_value(dev, BDMA_DADDR, 0x1FC00);
		} else {
			sw82908_write_value(dev, BDMA_DADDR, (BDMA_TRANS_SIZE * k));
		}
		sw82908_write_value(dev, FC_CTRL, 0x4);
		sw82908_write_value(dev, BDMA_START, 1);
...
static int sw82908_fw_upgrade_conf(struct device *dev, u8 *pFirm, u32 nSize)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	if (sw82908_reg_read(dev, CHIP_INFO, (u8 *)&d->chip_info,
				sizeof(struct sw82908_chip_info)) < 0) {
		TOUCH_E("Chip info Read Error\n");
	}
	...
	sw82908_fw_upgrade_code(dev, &pFirm[flash_fw_size], cfg_size);
	touch_msleep(100);
...
static int sw82908_fw_upgrade_post(struct device *dev)
{
	u32 crc_val;
	int ret = 0;
	...
	sw82908_fw_rst_ctl(dev, 0, NULL);
	touch_msleep(100);

	sw82908_fw_rst_ctl(dev, 2, "crc");
	touch_msleep(100);

	ret = sw82908_fw_flash_crc(dev, &crc_val);
	if (ret < 0) {
	...
	ret = sw82908_fw_rst_ctl(dev, 1, NULL);
	if (ret < 0)
		goto out;

	touch_msleep(100);

	ret = sw82908_fw_rst_ctl(dev, 0, NULL);
	if (ret < 0)
		goto out;
 ...
 */
static int sw82908_fw_upgrade(struct device *dev,
		const struct firmware *fw)
{
	...
	sw82908_tc_driving(dev, LCD_MODE_STOP);
	...
	img_check_result = sw82908_fw_binary_verify(dev, (unsigned char *)fwdata, fw->size);
	...
	ret = sw82908_fw_rst_ctl(dev, 2, NULL);
	if (ret < 0)
		return ret;

	TOUCH_I("[fw flash mass erase - start]\n");
	ret = sw82908_fw_flash_mass_erase(dev);
	if (ret < 0)
		return ret;
	TOUCH_I("[fw flash mass erase - end]\n");

	TOUCH_I("[Code update]\n");
	sw82908_fw_upgrade_code(dev, fwdata, flash_fw_size);

#if 0	/* not fixed in SW82908 */
	sw82908_write_value(dev, fw_boot_code_addr, FW_BOOT_LOADER_INIT);

	touch_msleep(10);
	sw82908_fw_rst_ctl(dev, 0, NULL);
	touch_msleep(200);

	ret = sw82908_condition_wait(dev, fw_boot_code_addr, NULL,
...
*/
...
#if defined(__SW82908_SUPPORT_CFG)
	sw82908_read_value(dev, CHIP_INFO + 4,
	...
	if (include_conf) {
		TOUCH_I("[CFG update]\n");
		ret = sw82908_fw_upgrade_conf(dev, fwdata, fw->size);
		if (ret < 0)
			return ret;
	}
#endif	/* __SW82908_SUPPORT_CFG */

	ret = sw82908_fw_upgrade_post(dev);
	if (ret < 0)
		return ret;
...
static int sw82908_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
//	struct project_param *param = &d->p_param;
...
//	u32 data = 0;
	...
	d->check_fwup = sw82908_fw_compare(dev, fw);
	...
	if (d->check_fwup == UPGRADE_NEED || d->check_fwup == UPGRADE_FORCE) {
		...
		for (i = 0; i < 3 && ret; i++) {
			...
			if (param->wa_trim_wr) {
				if (i == 1) {
					ret = sw82908_read_value(dev, SYS_OSC_CTL, &data);
					if (ret < 0) {
					...
					if (data == 4) {
						...
						sw82908_write_value(dev, SYS_OSC_CTL, AVERAGE_TRIM_VAL);
					}
				}
			}
		#endif
...
static int sw82908_app_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	const struct firmware *fw = NULL;
...
#endif
static int sw82908_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
...
static int sw82908_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	int boot_mode = 0;
...
static int sw82908_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct project_param *param = &d->p_param;
	...
	if (d->lpwg_abs.enable) {
		...
		sw82908_lpwg_abs_enable(d->dev, d->lpwg_abs.enable);
	}
...
int sw82908_ic_test_unit(struct device *dev, u32 data)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	struct project_param *param = &d->p_param;
	u32 data_rd;
	...
	ret = sw82908_write_value(dev, BUS_TEST_REG, data);
	if (ret < 0) {
		TOUCH_E("ic test wr err, %08Xh, %d\n", data, ret);
		goto out;
	}

	ret = sw82908_read_value(dev, BUS_TEST_REG, &data_rd);
	if (ret < 0) {
...
static int sw82908_ic_test(struct device *dev)
{
	u32 data[] = {
		0x5A5A5A5A,
		0xA5A5A5A5,
	...
	for (i = 0; i < ARRAY_SIZE(data); i++) {
		ret = sw82908_ic_test_unit(dev, data[i]);
...
*/
	...
	if (ts->bus_type == HWIF_I2C) {
		ret = sw82908_reg_write(dev, SERIAL_SPI_EN, &rdata, sizeof(rdata));
		if (ret)
			TOUCH_E("failed to write \'serial_spi_disable\', ret:%d\n", ret);
		else
			TOUCH_I("serial_spi_en : %d", rdata);
	} else if (ts->bus_type == HWIF_SPI) {
		ret = sw82908_reg_write(dev, SERIAL_I2C_EN, &rdata, sizeof(rdata));
		if (ret)
	...
	ret = sw82908_reg_write(dev, TC_CMD +
	...
	ret = sw82908_temperature_status(dev, true);
	if (ret)
		TOUCH_E("failed to write \'reg_temperature_state\', ret:%d\n", ret);

	sw82908_setup_q_sensitivity(dev, d->q_sensitivity);
	sw82908_longpress_enable(dev, d->lpwg_longpress.enable);

	sw82908_rotation(dev);
	sw82908_grip_area(dev);
...
int sw82908_check_abnormal_signal(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
...
int sw82908_check_status(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;
	int i = 0, idx = 0;
	...
	sw82908_check_abnormal_signal(dev);
...
static void sw82908_lpwg_abs_filter(struct device *dev, u8 touch_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
int sw82908_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
...
//		ts->intr_status |= TOUCH_IRQ_PEN;
	...
	if (data[0].track_id == PALM_ID) {
		if (data[0].event == TOUCHSTS_DOWN) {
			...
			ret = sw82908_reg_read(dev, DEBUG_PALM, debug_palm, sizeof(debug_palm));
			if (ret < 0) {
	...
	for (i = 0; i < touch_count; i++) {
		...
		if (data[i].event == TOUCHSTS_DOWN
			...
			if (d->lpwg_abs.enable)
				sw82908_lpwg_abs_filter(dev, tdata->id);
...
int sw82908_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct sw82908_touch_info *info = &d->info;
	...
	return sw82908_irq_abs_data(dev);
}

int sw82908_irq_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	...
	case IRQ_TYPE_KNOCK_ON:
		sw82908_get_tci_data(dev, ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.tap_count);
		ts->intr_status = TOUCH_IRQ_KNOCK;

		if (d->ai_pick.enable) {
			if (sw82908_check_ai_pick_event(dev)) {
...
int sw82908_debug_info(struct device *dev) {
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0, i = 0;
	int count = 0;

	ret = sw82908_reg_read(dev, DEBUG_INFO, &d->debug_info, sizeof(d->debug_info));
	if (ret < 0)
		goto error;
...
static int sw82908_irq_read_data(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	struct sw82908_touch_info *info = &d->info;
	...
	if(is_flex_report) {
		ret = sw82908_reg_read(dev, addr, (void *)buf, size);
		if(ret < 0) {
		...
		intr_type = sw82908_tc_sts_irq_type(info->tc_status);
...
int sw82908_irq_handler(struct device *dev)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;

	pm_qos_update_request(&d->pm_qos_req, 10);
	ret = sw82908_irq_read_data(dev);
	pm_qos_update_request(&d->pm_qos_req, PM_QOS_DEFAULT_VALUE);
	if (ret < 0) {
		goto error;
	}

	ret = sw82908_check_status(dev);

	d->intr_type = sw82908_tc_sts_irq_type(d->info.tc_status);
...
static int sw82908_lpwg_function_info(struct device *dev, int display_area_id)
{
	return 0;
}
		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = value;
		if (sw82908_reg_write(dev, reg_addr, &data, sizeof(u32)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (sw82908_reg_read(dev, reg_addr, &data, sizeof(u32)) < 0)
...
static ssize_t show_lpwg_failreason(struct device *dev, char *buf)
{
	...
	if (sw82908_reg_read(dev, TC_CMD + LPWG_FAILREASON_ON,
...
static ssize_t show_lpwg_abs(struct device *dev, char *buf)
{
	...
	sw82908_print_lpwg_abs_info(dev);
...
static ssize_t show_ai_pick(struct device *dev, char *buf)
{
	...
	sw82908_print_ai_pick_info(dev);
...
static ssize_t show_grip_suppression(struct device *dev, char *buf)
{
	...
	mutex_lock(&ts->lock);
	ret = sw82908_reg_read(dev, GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = sw82908_reg_write(dev, GRAB_TOUCH_CTRL, &data, sizeof(data));
	if (ret < 0 ) {
		...
		size_t count)
{
	...
	sw82908_setup_q_sensitivity(dev, value);
out:
	mutex_unlock(&ts->lock);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	ret = sw82908_longpress_enable(dev, d->lpwg_longpress.enable);
	if (ret < 0)
		...
		const char *buf, size_t count)
{
	...
	ret = sw82908_onetap_enable(dev, d->lpwg_onetap.enable);
	if (ret < 0)
...
static int sw82908_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = sw82908_prd_register_sysfs(dev);
#if defined(__SUPPORT_ABT)
	ret = sw82908_abt_register_sysfs(dev);
#endif
	if (ret < 0) {
...
static int sw82908_get_cmd_version(struct device *dev, char *buf)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int offset = 0;
...
static int sw82908_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct sw82908_data *d = to_sw82908_data(dev);
	int offset = 0;
...
static int sw82908_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int sw82908_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = sw82908_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = sw82908_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
#ifdef CONFIG_LGE_TOUCH_PEN
static int sw82908_stop_pen_sensing(struct device *dev, bool enable)
{
	...
	if (enable) {
		sw82908_reg_read(dev, addr, (u8 *)&prev_driving, sizeof(u32));
	...
	sw82908_reg_write(dev, addr, &stop_driving, sizeof(stop_driving));
	touch_msleep(param->sys_tc_stop_delay);
#endif

	sw82908_reg_write(dev, addr, &curr_driving, sizeof(u32));
...
int sw82908_write_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;
	u32 ctrl = 0;
...
int sw82908_read_cmd(struct device *dev, int cmd) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	int ret = 0;
	TOUCH_TRACE();
...
static int sw82908_common_control(struct device *dev, int rw_mode, char *cmd)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	int cmd_r = -1;
	...
	if (rw_mode == WRITE_MODE) {
		ret = sw82908_write_cmd(dev, cmd_r);
	} else if (rw_mode == READ_MODE) {
		ret = sw82908_read_cmd(dev, cmd_r);
	} else {
	}
...
static struct touch_driver touch_driver = {
	.probe = sw82908_probe,
	.remove = sw82908_remove,
	.shutdown = sw82908_shutdown,
	.suspend = sw82908_suspend,
	.resume = sw82908_resume,
	.init = sw82908_init,
	.irq_handler = sw82908_irq_handler,
	.power = sw82908_power,
	.upgrade = sw82908_upgrade,
#if defined(CONFIG_LGE_TOUCH_APP_FW_UPGRADE)
	.app_upgrade = sw82908_app_upgrade,
#endif
	.esd_recovery = sw82908_esd_recovery,
	.swipe_enable = sw82908_swipe_enable,
	.lpwg = sw82908_lpwg,
	.lpwg_function_info = sw82908_lpwg_function_info,
	.notify = sw82908_notify,
	.init_pm = sw82908_init_pm,
	.register_sysfs = sw82908_register_sysfs,
	.set = sw82908_set,
	.get = sw82908_get,
	.rotation = sw82908_rotation,
	.grip_area = sw82908_grip_area,
#ifdef CONFIG_LGE_TOUCH_PEN
	.stop_pen_sensing = sw82908_stop_pen_sensing,
#endif
#if defined(CONFIG_LGE_TOUCH_COMMON_CONTROL)
	.common_control = sw82908_common_control,
#endif
};
