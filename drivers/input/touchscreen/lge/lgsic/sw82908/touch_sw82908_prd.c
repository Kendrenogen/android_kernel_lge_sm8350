...
static const char * const test_name_str[TEST_TOTAL_NUM] = {
	...
	"FLASH_INFO_CHECK",
	"PT_INFO_CHECK",
	"U3_SINGLE_JITTER_TEST",
	"U3_SINGLE_JITTER_SELF_TEST",
	"U0_SINGLE_JITTER_TEST",
	"U0_SINGLE_JITTER_SELF_TEST",
};
...
static void prd_param_set(struct device *dev)
{
	...
	param->sd_test_set = (
			...
			|(1<<U3_P_JITTER_TEST)|(1<<U3_M1_JITTER_TEST)
			|(1<<U3_SINGLE_JITTER_TEST)|(1<<U3_SINGLE_JITTER_SELF_TEST));

//	param->lpwg_sd_test_set = ((1<<U0_M2_RAWDATA_TEST)|(1<<U0_M2_JITTER_TEST)|(1<<U0_M1_RAW_SELF_TEST));
	param->lpwg_sd_test_set = (
			(1<<U0_M2_RAWDATA_TEST)|(1<<U0_M2_JITTER_TEST)
			|(1<<U0_M1_JITTER_TEST)
				|(1<<U0_SINGLE_JITTER_TEST)|(1<<U0_SINGLE_JITTER_SELF_TEST));
...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	switch (type) {
		...
		case U3_P_JITTER_TEST:
			...
			testmode = ((disp_mode << 8) + type_temp);
			break;
		case U3_SINGLE_JITTER_TEST:
			type_temp = 0x11;
			testmode = ((disp_mode << 8) + type_temp);
			waiting_time = 800;
			break;
		case U3_SINGLE_JITTER_SELF_TEST:
			type_temp = 0x12;
			testmode = ((disp_mode << 8) + type_temp);
			waiting_time = 800;
			break;
		case U0_SINGLE_JITTER_TEST:
			type_temp = 0x11;
			testmode = type_temp;
			waiting_time = 800;
			break;
		case U0_SINGLE_JITTER_SELF_TEST:
			type_temp = 0x12;
			testmode = type_temp;
			waiting_time = 800;
			break;
	...
	/* TestType Set */
	sw82908_reg_write(dev, TC_PT_TEST_CTL,
	...
	do {
		touch_msleep(100);
		sw82908_reg_read(dev, addr, (u8 *)&rdata, sizeof(rdata));
...
static int prd_read_self_print(struct device *dev, int offset, int row, int col, u8 type)
{
	...
	sw82908_write_value(dev, tc_tsp_test_data_offset, offset);

	sw82908_reg_read(dev, data_access_addr,
...
static int prd_open_short_test(struct device *dev)
{
	...
	if (param->os_test_type == DATA_IC_COMPARE) {
		sw82908_reg_read(dev, result_addr, (u8 *)&open_result, sizeof(open_result));
	...
	if (param->os_test_type == DATA_IC_COMPARE) {
		sw82908_reg_read(dev, result_addr, (u8 *)&short_result, sizeof(short_result));
...
static int prd_compare_rawdata(struct device *dev, u8 type, int *result)
{
	...
	case U3_M2_JITTER_TEST:
	case U0_M2_JITTER_TEST:
		TOUCH_I("U3(%d)JITTER spec lower:%d uppper:%d", type, LowerImage[0][0], UpperImage[0][0]);
		break;
	case U0_M1_RAWDATA_TEST:
		col_size = M1_COL_SIZE;
		break;
	...
	case U3_SINGLE_JITTER_TEST:
	case U0_SINGLE_JITTER_TEST:
		TOUCH_I("U%d Single Jitter spec lower:%d uppper:%d", type, SimpleImage[0], SimpleImage[1]);
	...
	case U3_M2_JITTER_SELF_TEST:
		...
		return ret;
	case U3_SINGLE_JITTER_SELF_TEST:
		TOUCH_I("U3_SINGLE_JITTER_SELF spec lower:%d uppper:%d", SimpleImage[0], SimpleImage[1]);
		for (i = 0; i < ROW_SIZE + col_size ; i++) {
			if ((self_data[i] < SimpleImage[0]) ||
					(self_data[i] > SimpleImage[1])) {
		...
		return ret;
	case U0_SINGLE_JITTER_SELF_TEST:
		TOUCH_I("U0_SINGLE_JITTER_SELF spec lower:%d uppper:%d", SimpleImage[0], SimpleImage[1]);
		for (i = 0; i < ROW_SIZE + col_size ; i++) {
			if ((self_data[i] < SimpleImage[0]) ||
					(self_data[i] > SimpleImage[1])) {
...
int sw82908_tune_u8_print(int num_ch, u8 *tune_val, char *log_buf, int offset)
{
	int i;
	int temp = 0;
	for (i = 0; i < num_ch; i++) {
...
static void tune_display(struct device *dev, struct tune_data_sel *sel, int type)
{
	...
	switch (type) {
		case U3_M1_RAWDATA_TEST:;
			...
			offset += sw82908_tune_u8_print(LOFT_CH_NUM, sel->m1.m1_tune_sel_code, log_buf, offset);
		...
		case U3_M2_RAW_SELF_TEST:
			...
			offset += sw82908_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_high_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune low: ");
			offset += sw82908_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_low_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self1: ");
			offset += sw82908_tune_u8_print(LOFT_CH_NUM, sel->m2.self1_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self2: ");
			offset += sw82908_tune_u8_print(LOFT_CH_NUM, sel->m2.self2_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);
			break;
...
//	int m2_offset_size = m2_tune_size/4;
	...
	sw82908_reg_read(dev, tune_code_addr + offset, data, size);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	int u3_single_jitter_ret = 0;
	int u3_single_jitter_self_ret = 0;
	...
	*/
	...
	/*
	   ...
	   pass : 0, fail : 1
	*/
	if(param->sd_test_set & (1<<U3_SINGLE_JITTER_TEST)) {
		sic_get_simple_limit(dev, "U3_SINGLE_JITTER", SimpleImage);
		u3_single_jitter_ret = prd_rawdata_test(dev, U3_SINGLE_JITTER_TEST);
	}
	...
	*/
	if(param->sd_test_set & (1<<U3_M2_JITTER_SELF_TEST)) {
		...
		pass : 0, fail : 1
	*/
	if(param->sd_test_set & (1<<U3_SINGLE_JITTER_SELF_TEST)) {
		sic_get_simple_limit(dev, "U3_SINGLE_JITTER_SELF", SimpleImage);
		u3_single_jitter_self_ret = prd_rawdata_test(dev, U3_SINGLE_JITTER_SELF_TEST);
	}
        /*
		U3_P_JITTER_TEST
		pass : 0, fail : 1
	...
	*/
	...
	if ((u3_m1_raw_ret + u3_m2_raw_ret + u3_m1_jitter_ret + u3_m2_jitter_ret
				+ u3_m2_raw_self_ret + u3_m2_jitter_self_ret+u3_p_jitter_ret
				+ u3_single_jitter_ret + u3_single_jitter_self_ret) == 0) {
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Pass\n");
		TOUCH_I("Raw Data : Pass\n");
	} else {
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Fail (r1:%d/r2:%d/j1:%d/j2:%d/pj:%d/sj:%d)\n",
				u3_m1_raw_ret, u3_m2_raw_ret|u3_m2_raw_self_ret ,
				u3_m1_jitter_ret, u3_m2_jitter_ret|u3_m2_jitter_self_ret,u3_p_jitter_ret,u3_single_jitter_ret|u3_single_jitter_self_ret);
		TOUCH_I("Raw Data : Fail (r1:%d/r2:%d/j1:%d/j2:%d/pj:%d/sj:%d)\n",
				u3_m1_raw_ret, u3_m2_raw_ret|u3_m2_raw_self_ret ,
				u3_m1_jitter_ret, u3_m2_jitter_ret|u3_m2_jitter_self_ret,u3_p_jitter_ret,u3_single_jitter_ret|u3_single_jitter_self_ret);
	}
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw82908_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
	ret = sw82908_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw82908_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw82908_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw82908_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do {
		...
		sw82908_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
		touch_msleep(10);
...
static int prd_show_prd_get_data_raw_core(struct device *dev,
	...
	ret = sw82908_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&offset, sizeof(offset));
	if (ret < 0) {
		goto out;
	}

	ret = sw82908_reg_read(dev, data_access_addr, (void *)buf, size);
	if (ret < 0) {
		goto out;
	}
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata, int mode)
{
	...
	case CMD_RAWDATA_S:
		...
		sw82908_reg_write(dev, tc_tsp_test_data_offset,
				(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw82908_reg_read(dev, data_access_addr, (int16_t *)buf, r_size);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DEBUG_P_POS:
		...
		sw82908_reg_read(dev, data_access_addr, (int16_t *)buf,
...
static int check_osc_status(struct device *dev)
{
	...
	data = 0x8018;
	addr =  0x3c;
	if (sw82908_reg_write(dev, addr, &data, sizeof(u32)) < 0) {
	...
	/* read data */
	addr =  0xfd8;
	if (sw82908_reg_read(dev, addr, &value, sizeof(u32)) < 0) {
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	int m1_jitter_ret = 0;
	int m2_jitter_ret = 0;
	int u0_single_jitter_ret = 0;
	int u0_single_jitter_self_ret = 0;
	int ret = 0;
	int file_exist = 0;
	...
	*/
	...
	/*
	   ...
	   pass : 0, fail : 1
        */
	if(param->lpwg_sd_test_set & (1<<U0_SINGLE_JITTER_TEST)) {
		sic_get_simple_limit(dev, "U0_SINGLE_JITTER", SimpleImage);
		u0_single_jitter_ret = prd_rawdata_test(dev, U0_SINGLE_JITTER_TEST);
	}
	...
	/*
	   ...
	   pass : 0, fail : 1
	*/
	if(param->lpwg_sd_test_set & (1<<U0_SINGLE_JITTER_SELF_TEST)) {
		sic_get_simple_limit(dev, "U0_SINGLE_JITTER_SELF", SimpleImage);
		u0_single_jitter_self_ret = prd_rawdata_test(dev, U0_SINGLE_JITTER_SELF_TEST);
	}
	ret = touch_snprintf(buf + ret, PAGE_SIZE, "\n========RESULT=======\n");
	TOUCH_I("========RESULT=======\n");

	if (!m1_rawdata_ret && !m2_rawdata_ret && !m1_jitter_ret && !m2_jitter_ret 
        && !u0_single_jitter_ret && !u0_single_jitter_self_ret && !osc_status_ret) {
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret,
				"LPWG RawData : %s\n", "Pass");
		TOUCH_I("LPWG RawData : %s\n", "Pass");
	} else {
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret,
				"LPWG RawData : %s (r1:%d/r2:%d/j1:%d/j2:%d/sj:%d/osc:%d)\n", "Fail",
				m1_rawdata_ret, m2_rawdata_ret, m1_jitter_ret, m2_jitter_ret,
				u0_single_jitter_ret|u0_single_jitter_self_ret,
				osc_status_ret);
		TOUCH_I("LPWG RawData : %s (r1:%d/r2:%d/j1:%d/j2:%d/sj:%d/osc:%d)\n", "Fail",
				m1_rawdata_ret, m2_rawdata_ret, m1_jitter_ret, m2_jitter_ret,
				u0_single_jitter_ret|u0_single_jitter_self_ret,
				osc_status_ret);
	}
...
int sw82908_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82908_data *d = to_sw82908_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
