...
static uint32_t abt_ksocket_rcv_from_pctool(uint8_t *buf, uint32_t len)
{
	...
	if (abt_comm.recv_packet->hdr.cmd == TCP_REG_READ) {
		ret = sw82908_reg_read(abt_comm.dev,
		...
		ret = sw82908_reg_write(abt_comm.dev,
					abt_comm.recv_packet->hdr.addr,
...
static struct attribute *sw82908_abt_attribute_list[] = {
	&touch_attr_raw_report.attr,
	NULL,
};

static const struct attribute_group sw82908_abt_attribute_group = {
	.attrs = sw82908_abt_attribute_list,
};

int sw82908_abt_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = sysfs_create_group(&ts->kobj, &sw82908_abt_attribute_group);
...
/*
void sw82908_abt_unregister_sysfs(struct device *dev)
{
	sysfs_remove_group(&ts->kobj, &sw82908_abt_attribute_group);
}
*/
