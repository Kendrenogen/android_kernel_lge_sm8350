...
int sw42902_tune_u8_print(int num_ch, u8 *tune_val, char *log_buf, int offset)
{
	int i;
	int temp = 0;
	for (i = 0; i < num_ch; i++) {
...
static void tune_display(struct device *dev, struct tune_data_sel *sel, int type)
{
	...
	switch (type) {
		case U3_M1_RAWDATA_TEST:;
			...
			offset += sw42902_tune_u8_print(LOFT_CH_NUM, sel->m1.m1_tune_sel_code, log_buf, offset);
		...
		case U3_M2_RAW_SELF_TEST:
			...
			offset += sw42902_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_high_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune low: ");
			offset += sw42902_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_low_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self1: ");
			offset += sw42902_tune_u8_print(LOFT_CH_NUM, sel->m2.self1_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self2: ");
			offset += sw42902_tune_u8_print(LOFT_CH_NUM, sel->m2.self2_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);
			break;
