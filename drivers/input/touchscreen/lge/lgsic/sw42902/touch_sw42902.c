...
static int sw42902_longpress_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw42902_data *d = to_sw49202_data(dev);
	struct sw42902_lpwg_longpress_buf buf = {0, };
...
static int sw42902_onetap_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw42902_data *d = to_sw49202_data(dev);
	struct sw42902_lpwg_onetap_buf buf = {0, };
	struct sw42902_active_area area = {0, };
	int ret = 0;
...
static int sw42902_swipe_enable(struct device *dev, bool enable)
{
	...
	struct sw42902_swipe_buf buf = {0, };
	struct sw42902_swipe2_buf buf2 = {0, };
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen) {
			...
			if (ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode == LPWG_NONE
				...
				sw42902_sleep_ctrl(dev, IC_NORMAL);
				ret = sw42902_lpwg_control(dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
				sw42902_swipe_enable(dev, true);
				sw42902_tc_driving(dev, LCD_MODE_U0);
...
static void sw42902_partial_tcdriving_work_func(struct work_struct *partial_tcdriving)
{
	...
	if (atomic_read(&ts->state.fb) == FB_RESUME && !ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.screen
		...
		sw42902_lpwg_control(d->dev, ts->display_area_status[DISPLAY_AREA_ID_0].lpwg.mode);
...
int sw42902_irq_lpwg(struct device *dev)
{
	...
	case IRQ_TYPE_KNOCK_ON:
		sw42902_get_tci_data(dev, ts->display_area_status[DISPLAY_AREA_ID_0].knock_on.tap_count);
...
static int sw42902_lpwg_function_info(struct device *dev, int display_area_id)
{
	return 0;
}
...
static struct touch_driver touch_driver = {
	...
	.lpwg = sw42902_lpwg,
	.lpwg_function_info = sw42902_lpwg_function_info,
