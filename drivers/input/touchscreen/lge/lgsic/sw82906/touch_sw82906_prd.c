...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	/* TestType Set */
	sw82906_reg_write(dev, TC_PT_TEST_CTL,
	...
	do {
		touch_msleep(100);
		sw82906_reg_read(dev, addr, (u8 *)&rdata, sizeof(rdata));
...
static int prd_read_self_print(struct device *dev, int offset, int row, int col, u8 type)
{
	...
	sw82906_write_value(dev, tc_tsp_test_data_offset, offset);

	sw82906_reg_read(dev, data_access_addr,
...
static int __used prd_os_xline_result_read(struct device *dev, int type)
{
	...
	case OPEN_NODE_TEST:
		...
		write_file(dev, "[OPEN NODE DATA]\n", TIME_INFO_SKIP);
		prd_open_result_get(dev);
		break;
	case SHORT_NODE_TEST:
		TOUCH_I("[SHORT NODE DATA]\n");
		write_file(dev, "[SHORT NODE DATA]\n", TIME_INFO_SKIP);
...
static int sdcard_spec_file_read(struct device *dev)
{
	...
	if (fd >= 0) {
		...
		if (line) {
			TOUCH_I("%s: line is already allocated. vfree line\n",
					__func__);
			vfree(line);
			line = NULL;
		}
		...
		line = vzalloc(size);
		if (line == NULL) {
			TOUCH_E("failed to vzalloc line\n");
			ksys_close(fd);
			set_fs(old_fs);
			return -ENOMEM;
		}
...
static int prd_open_short_test(struct device *dev)
{
	...
	if (param->os_test_type == DATA_IC_COMPARE) {
		sw82906_reg_read(dev, result_addr, (u8 *)&open_result, sizeof(open_result));
	...
	if (param->os_test_type == DATA_IC_COMPARE) {
		sw82906_reg_read(dev, result_addr, (u8 *)&short_result, sizeof(short_result));
...
int sw82906_tune_u8_print(int num_ch, u8 *tune_val, char *log_buf, int offset)
{
	int i;
	int temp = 0;
	for (i = 0; i < num_ch; i++) {
...
static void tune_display(struct device *dev, struct tune_data_sel *sel, int type)
{
	...
	switch (type) {
		case U3_M1_RAWDATA_TEST:;
			...
			offset += sw82906_tune_u8_print(LOFT_CH_NUM, sel->m1.m1_tune_sel_code, log_buf, offset);
		...
		case U3_M2_RAW_SELF_TEST:
			...
			offset += sw82906_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_high_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune low: ");
			offset += sw82906_tune_u8_print(LOFT_CH_NUM, sel->m2.m2_tune_low_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self1: ");
			offset += sw82906_tune_u8_print(LOFT_CH_NUM, sel->m2.self1_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);

			offset = 0;
			offset += touch_snprintf(log_buf+offset, tc_tune_code_size-offset, "U3 M2 tune self2: ");
			offset += sw82906_tune_u8_print(LOFT_CH_NUM, sel->m2.self2_tune_sel_code, log_buf, offset);
			write_file(dev, log_buf, TIME_INFO_SKIP);
			TOUCH_I("%s", log_buf);
			break;
...
//	int m2_offset_size = m2_tune_size/4;
	...
	sw82906_reg_read(dev, tune_code_addr + offset, data, size);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	sw82906_write_value(dev, TC_PT_TEST_CTL, pt_command);
	*/
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw82906_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
	ret = sw82906_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw82906_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw82906_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw82906_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do {
		...
		sw82906_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
		touch_msleep(10);
...
static int prd_show_prd_get_data_raw_core(struct device *dev,
	...
	ret = sw82906_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&offset, sizeof(offset));
	if (ret < 0) {
		goto out;
	}

	ret = sw82906_reg_read(dev, data_access_addr, (void *)buf, size);
	if (ret < 0) {
		goto out;
	}
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata, int mode)
{
	...
	case CMD_RAWDATA_S:
		...
		sw82906_reg_write(dev, tc_tsp_test_data_offset,
				(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw82906_reg_read(dev, data_access_addr, (int16_t *)buf, r_size);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DEBUG_P_POS:
		...
		sw82906_reg_read(dev, data_access_addr, (int16_t *)buf,
...
static int check_osc_status(struct device *dev)
{
	...
	data = 0x8018;
	addr =  0x3c;
	if (sw82906_reg_write(dev, addr, &data, sizeof(u32)) < 0) {
	...
	/* read data */
	addr =  0xfd8;
	if (sw82906_reg_read(dev, addr, &value, sizeof(u32)) < 0) {
...
int sw82906_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw82906_data *d = to_sw82906_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
