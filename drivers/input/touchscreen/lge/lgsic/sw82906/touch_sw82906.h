...
enum {
	...
	IRQ_TYPE_LG_PAY_SWIPE_DOWN = 6,
	IRQ_TYPE_UDF_LONG_PRESS = 7,
	IRQ_TYPE_QUICK_TOOL_SWIPE_LEFT = 8,
	IRQ_TYPE_QUICK_TOOL_SWIPE_RIGHT = 9,
	IRQ_TYPE_SHOW_AOD_ONE_TAP = 50,
	IRQ_TYPE_LONG_PRESS_DOWN = 51,
	IRQ_TYPE_LONG_PRESS_UP = 52,
	IRQ_TYPE_FRONT_PALM_DOWN = 53,
	IRQ_TYPE_FRONT_PALM_UP = 54,
	IRQ_TYPE_REAR_PALM_DOWN = 55,
	IRQ_TYPE_REAR_PALM_UP = 56,
	IRQ_TYPE_FAKE_OFF_SWIPE_LEFT = 57,
	IRQ_TYPE_FAKE_OFF_SWIPE_RIGHT = 58,
	IRQ_TYPE_FAKE_OFF_SWIPE_UP = 59,
	IRQ_TYPE_FAKE_OFF_SWIPE_DOWN = 60,
	IRQ_TYPE_RESERVED_SWIPE_LEFT = 61,
	IRQ_TYPE_RESERVED_SWIPE_RIGHT = 62,
	IRQ_TYPE_FAKE_OFF_ONE_TAP = 63,
...
extern int sw82906_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int sw82906_reg_write(struct device *dev, u16 addr, void *data, int size);
extern int sw82906_ic_info(struct device *dev);
extern int sw82906_te_info(struct device *dev, char *buf);
extern int sw82906_tc_driving(struct device *dev, int mode);
extern int sw82906_irq_abs(struct device *dev);
extern int sw82906_irq_abs_data(struct device *dev);
extern int sw82906_irq_lpwg(struct device *dev);
extern int sw82906_irq_handler(struct device *dev);
extern int sw82906_check_status(struct device *dev);
extern int sw82906_debug_info(struct device *dev);
extern int sw82906_sleep_ctrl(struct device *dev, int new_status);
/*
extern int sw82906_chip_info_load(struct device *dev);
*/
extern int sw82906_reset_ctrl(struct device *dev, int ctrl);
...
static inline u32 sw82906_tc_sts_irq_type(int status)
{
	return ((status >> 16) & 0x0F);
}

static inline u32 sw82906_tc_sts_running_sts(int status)
{
	return (status & 0x1F);
}

static inline int sw82906_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw82906_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw82906_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw82906_reg_write(dev, addr, &value, sizeof(value));
}

void sw82906_irq_runtime_engine_debug(struct device *dev);
