...
static const unsigned char InitCodeVal[CNT_INITCODE] =  { 0x00, 0x12, 0x00, 0x19, 0x11, 0x9A, 0x33, 0x13, 0x13, 0x13, 0x0F, 0x72, 0x82, 0x64, 0x82, 0x64, 0x82, 0x64, 0xD0, 0x89, 0x20, 0x10, 0x0B, 0x07, 0x0B, 0x07, 0x0B, 0x07, 0x33, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0xEE, 0x4F, 0x03, 0x33, 0x03, 0x07, 0x04 };

#elif defined (CONFIG_MACH_LAHAINA_RAINBOWLM)
#define CNT_INITCODE					42
static const unsigned char InitCodeAddr[CNT_INITCODE] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B };
static const unsigned char InitCodeVal[CNT_INITCODE] =  { 0x11, 0x9A, 0x11, 0x9A, 0x00, 0x29, 0x33, 0x13, 0x13, 0x13, 0x0F, 0x74, 0x82, 0x64, 0x82, 0x64, 0x9B, 0x64, 0xD0, 0x89, 0x20, 0x10, 0x0B, 0x07, 0x0B, 0x07, 0x0B, 0x07, 0x33, 0x00, 0xFF, 0x00, 0xFF, 0x0D, 0x4B, 0xF0, 0x4F, 0x03, 0x2B, 0x02, 0x04, 0x04 };
...
void sar_controller_ots_notify_connect(u32 targetCount)
{
    if( (&sar_controller_atomic_notifier)->head != NULL ){
        PINFO("notifier_callback function is registered targetCount %d", (int)targetCount );
        atomic_notifier_call_chain(&sar_controller_atomic_notifier, OTS_CHANGED, &targetCount);
    }
    else
        PINFO("notifier_callback function isn't registered %d", (int)targetCount );
}
EXPORT_SYMBOL(sar_controller_ots_notify_connect);
#endif
...
#ifdef CONFIG_LITO_WINGLM_SEPERATE_INITCODE
static void initialize_init_code()
{
    int sku = -1;
    PINFO("set initcode as winglm");
    InitCodeAddr = InitCodeAddr_WINGLM;
    ...
    switch(sku)
    {
        ...
        case HW_SKU_AU_OPEN:
        case HW_SKU_GLOBAL_CERT:
            PINFO("global's init code %d", sku);
            InitCodeVal = InitCodeVal_WINGLM_GLOBAL;
            break;
        case HW_SKU_NA_GSM:
        case HW_SKU_NA_GSM_ATT:
        case HW_SKU_NA_GSM_TMUS:
        case HW_SKU_NA_CDMA:
        case HW_SKU_NA_CDMA_VZW:
        case HW_SKU_NA_CDMA_SPR:
        case HW_SKU_NA:
            PINFO("na's init code %d", sku);
            InitCodeVal = InitCodeVal_WINGLM_NA;
            break;
        default:
            PINFO("default init code %d", sku);
            InitCodeVal = InitCodeVal_WINGLM_DEFAULT;
            break;
    }
}
#endif
		...
		const struct i2c_device_id *id)
{
	...
	if (!data->input_dev_cap) {
		PERR("ALMF04 Failed to allocate input device cap !\n");
                err = -ENOMEM;
	...
	PINFO("interrupt is hooked\n");
        ...
        if (err < 0){
            PERR("almf04 failed to register almf04 atomic_notify callback\n");
        }
